...
export const scannerAPI = {
  ...
  handleQuickScan: (address: string) => scannerAPI.analyzeContract(address, 'quick'),
  handleDeepScan: (address: string) => scannerAPI.analyzeContract(address, 'deep'),
...
export const timeMachineAPI = {
  getBlocks: (startBlock: number, endBlock: number) =>
    apiRequest(`/time-machine/blocks?start_block=${startBlock}&end_block=${endBlock}`),
  ...
  handleAnalyzeTx: (txHash: string) => timeMachineAPI.analyzeTransaction(txHash),
...
export const reportsAPI = {
  generateReport: (reportType: string) => apiRequest(`/reports/generate/${reportType}`),

  getHistory: () => apiRequest('/reports/history'),
  downloadReport: (reportId: string) => apiRequest(`/reports/download/${reportId}`),
