...
export const config: AppConfig = {
  ...
  app: {
    ...
    environment:
      ...
      'development',
    logLevel:
      (import.meta.env.VITE_LOG_LEVEL as AppConfig['app']['logLevel']) || 'info',
  },
};
...
export const getScannerUrl = (path: string) => `${config.api.scannerUrl}${path}`;
export const getMempoolUrl = (path: string) => `${config.api.mempoolUrl}${path}`;
