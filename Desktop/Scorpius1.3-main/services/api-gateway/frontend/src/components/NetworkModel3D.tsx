...
import { Shield, Zap, AlertTriangle, CheckCircle, Activity, Globe } from 'lucide-react';
...
export const NetworkModel3D = ({
  ...
  const calculateNodePosition = (index: number, total: number) => {
    const radius = 120;
    const angle = (index / total) * Math.PI * 2 + (animationFrame * Math.PI) / 180;
  ...
  const generateConnections = (): NetworkConnection[] => {
    ...
    nodes.forEach((node, nodeIndex) => {
      node.connections.forEach(connectionId => {
        const connectedNodeIndex = nodes.findIndex(n => n.id === connectionId);
        if (connectedNodeIndex !== -1 && connectedNodeIndex > nodeIndex) {
          const startPos = calculateNodePosition(nodeIndex, nodes.length);
          const endPos = calculateNodePosition(connectedNodeIndex, nodes.length);
              ...
              <div
                ...
                style={{
                  transform: `translate3d(${position.x}px, ${position.y}px, ${
                    position.z
                  }px) ${isHovered ? 'scale(1.2)' : 'scale(1)'}`,
                  transformStyle: 'preserve-3d',
                }}
