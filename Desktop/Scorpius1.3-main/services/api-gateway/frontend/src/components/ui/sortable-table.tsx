    ...
    return (
      ...
      >
        {direction === 'asc' ? (
          ...
          <ArrowUpDown size={14} className="text-gray-500 group-hover:text-gray-300" />
        )}
      </motion.div>
    );
  };
  ...
  const TableRow = ({ item, index }: { item: T; index: number }) => {
    ...
    return (
      <Reorder.Item
        ...
        style={{
          gridTemplateColumns: `${enableDrag ? 'auto ' : ''}${columns
      ...
      >
        ...
        {columns.map(column => (
          <div key={String(column.key)} className="flex items-center min-h-[40px]">
            {column.render
                ...
                >
                  {enableDrag && (
                    <div className="flex items-center justify-center w-8">
                      <motion.div whileHover={{ scale: 1.1 }} className="text-gray-600">
    ...
    },
    {
      ...
      render: item => (
        <div className="flex items-center">
          ...
          />
          <span
            className={item.status === 'Active' ? 'text-green-400' : 'text-gray-500'}
          >
            {item.status}
          </span>
        </div>
