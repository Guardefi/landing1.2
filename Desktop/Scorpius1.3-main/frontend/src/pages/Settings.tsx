...
const Settings = () => {
  ...
  const {
    ...
    initialData: StorageManager.getSystemConfig(),
    onSuccess: (data) => {
      if (data) {
        StorageManager.setSystemConfig(data);
      }
    },
    onError: () => {
      // Fallback to local storage
      console.warn("Using local system config");
    },
  });

  const [config, setConfig] = useState<SystemConfig>(
    systemConfig || StorageManager.getSystemConfig(),
  );
  ...
  const updateSettingsMutation = useMutation({
    mutationFn: (newConfig: SystemConfig) =>
      apiClient.updateSystemConfig(newConfig),
    onSuccess: () => {
