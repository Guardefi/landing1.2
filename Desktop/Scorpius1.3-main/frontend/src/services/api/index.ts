...
class ApiClient {
  ...
  async scanContract(target: string, options?: any): Promise<any> {
    try {
      const response = await ScannerService.startScan(target, options);
  ...
  async getScanResults(scanId: string): Promise<any> {
    try {
      const response = await ScannerService.getScanResults(scanId);
      return response.data;
    } catch (error) {
      console.error("Scanner results API error:", error);
      throw error;
    }
  }
  ...
  async getRecentScans(limit: number = 10): Promise<any> {
    try {
      const response = await ScannerService.getRecentScans(limit);
      return response.data;
    } catch (error) {
  ...
  async uploadAndScan(file: File, options?: any): Promise<any> {
    try {
      const response = await ScannerService.uploadContract(file);
      return response.data;
    } catch (error) {
      console.error("Upload and scan API error:", error);
      throw error;
    }
  }

  // Honeypot-specific methods
  async analyzeHoneypot(address: string): Promise<any> {
    try {
      const response = await HoneypotService.analyzeContract(address);
      return response.data;
    } catch (error) {
      console.error("Honeypot analyze API error:", error);
      throw error;
    }
  }

  async getHoneypotDetections(): Promise<any> {
    try {
      const response = await HoneypotService.getDetections();
      return response.data;
    } catch (error) {
      console.error("Honeypot detections API error:", error);
      throw error;
    }
  }
  ...
  async getMempoolData(): Promise<any> {
    try {
      const response = await MempoolService.getTransactions();
      return response.data;
    } catch (error) {
...
// Scanner Service - Uses available vulnerability scanners (slither, mythril, etc.)
export class ScannerService {
  static async startScan(
    contractAddress: string,
    options?: any,
  ): Promise<ApiResponse<ScanResult>> {
    // Use slither as the primary scanner since it's available in the backend
    try {
      const response = await apiClient.post("/api/slither/scan", { 
        contract_address: contractAddress, 
        scan_type: options?.scanType || "basic",
        ...options 
      });
      ...
      return {
        success: true,
        data: {
          id: response.data?.scan_id || `scan_${Date.now()}`,
          contractAddress,
          vulnerabilities: response.data?.vulnerabilities || [],
          score: response.data?.security_score || 0,
      ...
      console.error("Slither scan failed, trying mythril:", error);
      
      // Fallback to mythril if slither fails
      try {
        const response = await apiClient.post("/api/mythril/scan", { 
          contract_address: contractAddress,
          ...options 
        });
        ...
        console.error("Both scanners failed:", mythrilError);
        throw new Error("Vulnerability scanning services are currently unavailable");
      }
    }
  }
  ...
  static async getScanResults(
    ...
    // Try to get results from slither first, then mythril
    try {
      return await apiClient.get(`/api/slither/results/${scanId}`);
    } catch (error) {
      return await apiClient.get(`/api/mythril/results/${scanId}`);
    }
  }

  static async getRecentScans(limit = 10): Promise<ApiResponse<ScanResult[]>> {
    // Return mock data for now since we don't have a unified scan history endpoint
    return {
      success: true,
      data: [
        {
  ...
  static async uploadContract(
    ...
    // For now, return a mock response since file upload endpoints aren't implemented
    return {
      success: true,
      data: {
        scanId: `upload_scan_${Date.now()}`
      }
    };
  }
}
...
export class AuthService {
  ...
  static async register(email: string, password: string, full_name?: string): Promise<ApiResponse<any>> {
  ...
  static async upgradeSubscription(tier: string): Promise<ApiResponse<any>> {
...
export class HealthService {
  ...
  static async getServices(): Promise<ApiResponse<any>> {
