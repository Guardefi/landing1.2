...
class ProviderManager:
    ...
    def __init__(self):
        ...
        for url in fallback_urls:
            ...
            if provider_obj:
                self.providers["fallbacks"].append(provider_obj)  # Filter out any None
    ...
    def _get_provider_url(self, provider: AsyncBaseProvider | None) -> str:
        ...
        if not provider:
            return "(None)"
        return getattr(
            provider, "_endpoint_uri", getattr(provider, "endpoint_uri", "(Unknown)")
        )
    ...
    async def get_provider(self) -> Web3:
        ...
        logger.warning(
            f"Current provider {self._get_provider_url(self.current_provider)} is unhealthy. Selecting optimal."
        )
    ...
    async def get_optimal_provider(self) -> AsyncBaseProvider:
        ...
        if old_url != best["url"]:
            logger.info(
                f"Switching provider from {old_url} to {best['url']} (latency {best['latency']:.1f}ms)."
            )
