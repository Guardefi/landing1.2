...
def install_dependencies():
    ...
    try:
        subprocess.run([sys.executable, "-m", "pip", "install",
                        "-r", "docs-requirements.txt"], check=True, )
...
def serve_docs():
    ...
    try:
        subprocess.run(["mkdocs", "serve", "--dev-addr",
                       "127.0.0.1:8080"], check=True)
    except subprocess.CalledProcessError as e:
...
def main():
    ...
    if not os.path.exists("mkdocs.yml"):
        print(
            "❌ Error: mkdocs.yml not found. Please run from the project root directory."
        )
        sys.exit(1)
    ...
    if command == "install":
        ...
        if install_dependencies():
            if build_docs():
                ...
                print(
                    "📖 Open site/index.html in your browser to view the documentation"
                )
            else:
