...
class ServiceRegistrationClient:
    ...
    async def register(self, config: ServiceRegistrationConfig) -> bool:
        ...
        try:
            ...
            response = await self.http_client.post(
                f"{self.gateway_url}/services/register", json=payload, timeout=10.0
            )
            ...
            if response.status_code == 200:
                ...
                if self.redis_client:
                    await self.redis_client.publish(
                        "scorpius.service_registration", json.dumps(payload)
                    )
    ...
    async def unregister(self, service_name: str) -> bool:
        ...
        try:
            response = await self.http_client.delete(
                f"{self.gateway_url}/services/{service_name}", timeout=10.0
            )

            if response.status_code == 200:
                logger.info(
                    f"Successfully unregistered service: {service_name}")
                ...
                logger.error(
                    f"Failed to unregister service: {
    ...
    async def heartbeat(self, service_name: str) -> bool:
        ...
        try:
            response = await self.http_client.post(
                f"{self.gateway_url}/services/{service_name}/heartbeat", timeout=5.0
            )
