...
class SecretsManager:
        ...
        if self._client is None:
            try:
                ...
                if self.config.fail_fast:
                    ...
                    raise RuntimeError(
                        f"AWS Secrets Manager connection failed: {e}"
                    ) from e
    ...
    def get_secret(
        ...
        if self.client:
            try:
                ...
                if error_code == "ResourceNotFoundException":
                    logger.warning(
                        f"Secret '{full_secret_name}' not found in AWS Secrets Manager")
                else:
                    logger.error(
                        f"Error retrieving secret '{full_secret_name}': {e}")
        ...
        if env_value is None and self.config.fail_fast:
            raise RuntimeError(
                f"Secret '{secret_name}' not found in AWS Secrets Manager or environment")

        if env_value:
            logger.debug(
                f"Retrieved secret '{secret_name}' from environment variable")
            self._cache[cache_key] = env_value

        return env_value

    def get_secret_json(
        self, secret_name: str, default: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.warning(
                f"Secret '{secret_name}' contains invalid JSON, using default"
            )
            return default or {}
    ...
    def rotate_secret(self, secret_name: str) -> bool:
        ...
        if not self.client:
            logger.warning(
                "AWS Secrets Manager not available, cannot rotate secret")
            return False
...
def get_secret_json(
    secret_name: str, default: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
