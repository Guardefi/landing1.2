...
from core.models import ScanConfig, ScanType, Target, VulnerabilityLevel
...
class ScannerCLI:
    ...
    def _create_parser(self) -> argparse.ArgumentParser:
        ...
        scan_parser = subparsers.add_parser(
            "scan", help="Run a vulnerability scan")
        ...
        scan_parser.add_argument(
            "--scan-type",
            "-s",
            choices=["static", "dynamic", "deployed", "comprehensive"],
            default="comprehensive",
            help="Type of scan to perform",
        )
        scan_parser.add_argument(
            "--config", "-c", help="Path to scan configuration file (JSON)"
        )
        scan_parser.add_argument(
            "--output-dir", "-o", help="Directory for scan results"
        )
        scan_parser.add_argument(
            "--report-formats",
            "-r",
        ...
        status_parser = subparsers.add_parser(
            "status", help="Check status of a scan")
        status_parser.add_argument(
            "--scan-id", "-i", required=True, help="ID of the scan to check"
        )
        ...
        if args.wait:
            ...
            while not completed:
                ...
                if scan_result.status.name in [
                    ...
                    if (
                        hasattr(scan_result, "progress")
                        and scan_result.progress is not None
                    ):
    ...
    async def _handle_status_command(self, args: argparse.Namespace) -> None:
        ...
        if hasattr(
                scan_result,
                "progress") and scan_result.progress is not None:
