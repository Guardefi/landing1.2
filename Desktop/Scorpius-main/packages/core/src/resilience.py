...
class AsyncCircuitBreaker:
    ...
    async def __call__(self, func: Callable[..., Any], *args, **kwargs) -> Any:
        ...
        try:
            ...
            logger.error(
                f"Function failed in circuit breaker {
                    self.name}: {e}"
            )
            raise
    ...
    def _on_failure(self, cb, exception):
        ...
        logger.warning(
            f"Circuit breaker {
                self.name} recorded failure: {exception}"
        )
    ...
    def _on_state_change(self, cb, old_state, new_state):
        ...
        logger.info(
            f"Circuit breaker {
                self.name} state changed: {old_state} -> {new_state}"
        )
            ...
            with attempt:
                try:
                    ...
                    if attempt.retry_state.attempt_number >= self.config.max_attempts:
                        ...
                        logger.error(
                            f"Max retries ({
                                self.config.max_attempts}) exceeded for function")
                        raise

                    logger.warning(
                        f"Attempt {
                            attempt.retry_state.attempt_number} failed: {e}"
                    )
                    raise
...
class ResilientAsyncClient:
    ...
    async def execute(self, func: Callable[..., Any], *args, **kwargs) -> Any:
        ...
        try:
            ...
            logger.debug(
                f"Function executed successfully in {
                    execution_time:.2f}s"
            )
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "retrier": {"stats": self.retrier.stats.__dict__},
...
def resilient(
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        client = ResilientAsyncClient(
            retry_config, circuit_breaker_config, name)
