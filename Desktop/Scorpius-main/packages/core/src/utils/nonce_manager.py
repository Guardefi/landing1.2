...
class NonceManager:
        ...
        try:
            ...
            async with self._account_locks[account]:
                ...
                if not tracker.reserve_nonce(nonce):
                    if self.explain:
                        self.logger.warning(
                            f"Nonce {nonce} already reserved for {account}"
                        )
                    return None
    ...
    async def confirm_nonce(
        ...
        try:
            if transaction_id not in self._reservations:
                self.logger.warning(
                    f"No reservation found for transaction {transaction_id}"
                )
                return False
            ...
            async with self._account_locks[account]:
                ...
                if not tracker:
                    self.logger.error(
                        f"No tracker found for account {account}")
                    return False
    ...
    async def get_replacement_nonce(
            ...
            new_gas_price: int) -> NonceReservation | None:
        """
        ...
        try:
            ...
            if new_gas_price <= original_reservation.gas_price:
                self.logger.error(
                    "Replacement gas price must be higher than original")
                return None
    ...
    async def sync_account_nonce(self, account: str) -> bool:
        ...
        try:
            ...
            current_nonce = await asyncio.to_thread(
                self.web3.eth.get_transaction_count, account, "pending"
            )
            ...
            async with self._account_locks[account]:
                ...
                if not tracker:
                    ...
                    tracker.confirmed_nonces = {
                        n for n in tracker.confirmed_nonces if n >= current_nonce}
        ...
        try:
            ...
            async with self._account_locks[account]:
                ...
                for nonce in range(current, current + self.max_nonce_gap):
                    if (
                        nonce not in tracker.pending_nonces
                        and nonce not in tracker.confirmed_nonces
                        and nonce not in tracker.failed_nonces
                    ):
                        gaps.append(nonce)

                if gaps and self.explain:
                    self.logger.warning(
                        f"Detected nonce gaps for {account}: {gaps}")
        ...
        while self._running:
            try:
                ...
                for account in list(self._account_trackers.keys()):
                    ...
                    if gaps:
                        self.logger.warning(
                            f"Account {account} has nonce gaps: {gaps}")
    ...
    def get_account_status(self, account: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "pending_nonces": sorted(tracker.pending_nonces),
