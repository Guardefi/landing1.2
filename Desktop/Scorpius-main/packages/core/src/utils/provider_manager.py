...
from web3 import AsyncHTTPProvider, Web3, WebsocketProvider
...
provider_health_cache = TTLCache(
    maxsize=100, ttl=CONFIG.get("CACHE_PROVIDER_HEALTH_TTL_S", 10)
)
...
class ProviderManager:
    ...
    def __init__(self):
        ...
        self.providers: dict[str, Any] = {
            "primary": self._create_async_provider(
                primary_url, is_websocket_preferred=True
            ),
            "fallbacks": [],
        }
        ...
        if CONFIG["RPC_URL"] and CONFIG["RPC_URL"] != primary_url:
            main_http_provider = self._create_async_provider(
                CONFIG["RPC_URL"], is_websocket_preferred=False
            )
        ...
        for url in fallback_urls:
            provider_obj = self._create_async_provider(
                url, is_websocket_preferred=False
            )
            if provider_obj:
                self.providers["fallbacks"].append(
                    provider_obj)  # Filter out any None
        self.providers["fallbacks"] = [
            p for p in self.providers["fallbacks"] if p]
        ...
        if not self.providers["primary"]:
            if self.providers["fallbacks"]:
                ...
                raise ConnectionError(
                    "No valid primary provider and no fallbacks available"
                )

        self.current_provider: AsyncBaseProvider | None = self.providers["primary"]
        if not self.current_provider:
            raise ConnectionError(
                "ProviderManager: No valid provider after init.")
    ...
    def _get_provider_url(self, provider: AsyncBaseProvider | None) -> str:
    ...
    def _create_async_provider(
        self, url: str, is_websocket_preferred: bool
        ...
        try:
            ...
            if is_websocket_preferred and url.startswith("ws"):
                ...
                return Web3(
                    WebsocketProvider(
                        url, websocket_timeout=request_kwargs["timeout"] * 2
                    )
                ).provider
                ...
                logger.debug(
                    f"Got WS URL but not preferred; falling back to AsyncHTTPProvider for {url}.")
                return AsyncHTTPProvider(url, request_kwargs=request_kwargs)
            else:
    ...
    async def _check_single_provider_latency(
            self, provider: AsyncBaseProvider):
        ...
        try:
            await with_retry_async(
                f"HealthCheck({url})", check_block_number, max_retries=2
            )
        except Exception:
            logger.warning(
                f"Provider health check failed for {url}: 'coroutine' object is not callable")
            raise
    ...
    async def get_provider(self) -> Web3:
        ...
        if self.initial_provider and (
                "127.0.0.1" in self.initial_provider or "localhost" in self.initial_provider):
            try:
                ...
                logger.error(
                    f"Local node {self.initial_provider} connection error: {e}"
                )
                # fall through to standard approach

        if self.current_provider and await self.is_provider_healthy(
        ...
        logger.warning(
            f"Current provider {
                self._get_provider_url(
                    self.current_provider)} is unhealthy. Selecting optimal."
        )
    ...
    async def is_provider_healthy(self, provider: AsyncBaseProvider) -> bool:
        ...
        try:
            ...
            await with_retry_async(f"HealthCheck({url})", health_check, max_retries=1)
            provider_health_cache[url] = True
            return True
        except Exception as e:
    ...
    async def get_optimal_provider(self) -> AsyncBaseProvider:
        ...
        logger.info(
            "ProviderManager: searching best provider by health/latency...")
        candidates = []
        ...
        if primary and await self.is_provider_healthy(primary):
            ...
            candidates.append(
                {"provider": primary, "latency": lat, "url": p_url, "isPrimary": True}
            )
        ...
        for p in self.providers["fallbacks"]:
            if p and await self.is_provider_healthy(p):
                ...
                candidates.append(
                    {"provider": p, "latency": lat, "url": f_url, "isPrimary": False}
                )
        ...
        if old_url != best["url"]:
            logger.info(
                f"Switching provider from {old_url} to {
                    best['url']} (latency {
                    best['latency']:.1f}ms)."
            )
    ...
    async def get_network(
        self, provider_to_check: AsyncBaseProvider | None = None
    ) -> dict[str, Any] | None:
        ...
        try:
            chain_id = await with_retry_async(
                f"getChainId({self._get_provider_url(provider)})", fetch_chain_id
            )
            net_map = {
                1: "mainnet",
                11155111: "sepolia",
