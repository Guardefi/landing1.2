...
class RustArbitrageEngine:
    ...
    async def find_arbitrage_paths(
        ...
        try:
            ...
            cache_key = self._create_cache_key(
                chain_configs, token_pairs, min_profit_usd
            )

            # Check cache first
            cached_result = self._get_cached_result(cache_key)
            if cached_result:
                logger.debug("Cache hit for arbitrage search")
            ...
            input_data = {
                ...
                "config": {
                    ...
                    or self.config.min_profit_threshold_usd,
    ...
    async def _execute_rust_engine(
        self, input_data: dict[str, Any], operation: str
    ) -> list[Any]:
        """
        ...
        input_file = self.temp_dir / \
            f"input_{operation}_{int(time.time() * 1000)}.json"
        output_file = (self.temp_dir /
                       f"output_{operation}_{int(time.time() * 1000)}.json")
        ...
        try:
            ...
            try:
                ...
                await process.wait()
                raise Exception(
                    f"Rust engine execution timed out after {
                        self.config.max_execution_time_seconds}s"
                ) from None
            ...
            if process.returncode != 0:
                ...
                raise Exception(
                    f"Rust engine failed with code {
            ...
            if output_file.exists():
                ...
                if operation == "find_arbitrage":
                    return [
                        self._parse_arbitrage_path(path_data)
                        for path_data in output_data.get("paths", [])
                    ]
                ...
                logger.warning(
                    f"No output file generated for operation: {operation}")
                return []
    ...
    def _parse_arbitrage_path(
            self, path_data: dict[str, Any]) -> ArbitragePath:
        ...
        return ArbitragePath(
            ...
            slippage_tolerance=path_data.get(
                "slippage_tolerance", self.config.slippage_tolerance
            ),
    ...
    def _get_cached_result(self, cache_key: str) -> list[ArbitragePath] | None:
    ...
    def _cache_result(
        ...
        if len(self._path_cache) > 100:
            oldest_key = min(
                self._path_cache.keys(), key=lambda k: self._path_cache[k][1]
            )
    ...
    def _update_metrics(self, execution_time_ms: float, success: bool) -> None:
        ...
        if self.total_searches == 1:
            ...
            self.average_search_time_ms = (
                self.average_search_time_ms * (self.total_searches - 1)
                + execution_time_ms
            ) / self.total_searches
        ...
        cache_hits = len(
            [
                k
                for k, (_, ts) in self._path_cache.items()
...
class RustEngineManager:
    ...
    async def find_arbitrage_paths(
            self, *args, **kwargs) -> list[ArbitragePath]:
    ...
    async def optimize_execution_order(
            self, *args, **kwargs) -> list[dict[str, Any]]:
    ...
    def _get_next_engine(self) -> RustArbitrageEngine:
        ...
        self.current_engine_index = (
            self.current_engine_index + 1) % len(self.engines)
        return engine
    ...
    def get_aggregate_metrics(self) -> dict[str, Any]:
        ...
        total_successful = sum(
            engine.successful_searches for engine in self.engines)
        ...
        avg_cache_hit_rate = sum(
            engine.cache_hit_rate for engine in self.engines
        ) / len(self.engines)
        ...
        return {
            ...
            "engine_metrics": [
                engine.get_performance_metrics() for engine in self.engines
            ],
        }
