...
from ..core.types import BundleRequest, BundleStatus
...
class RelayClient(ABC):
    ...
    def __init__(
        self, endpoint: RelayEndpoint, private_key: str, explain: bool = False
    ):
        """
    ...
    async def submit_bundle(
        self, bundle: BundleRequest, bundle_id: str
    ...
    async def check_bundle_status(
            self, bundle_id: str) -> BundleSubmissionResult:
        """
...
class FlashbotsRelay(RelayClient):
    ...
    async def submit_bundle(
        ...
        try:
            ...
            async with self.session.post(
                self.endpoint.url, data=body, headers=headers
            ) as response:
                ...
                if response.status == 200 and "error" not in response_data:
                    if self.explain:
                        self.logger.info(
                            f"Bundle {bundle_id} submitted successfully")
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200 and "result" in response_data:
                    ...
                    included = stats.get("isSimulated", False) and stats.get(
                    ...
                    return BundleSubmissionResult(
                        ...
                        relay_name=self.endpoint.name,
                        status=(
                            BundleStatus.INCLUDED if included else BundleStatus.PENDING),
                        block_number=bundle.block_number,
                        submitted_at=time.time(),
                        included=included,
    ...
    async def _build_flashbots_bundle(
            self, bundle: BundleRequest) -> dict[str, Any]:
...
class MEVShareRelay(RelayClient):
    ...
    async def submit_bundle(
        ...
        try:
            ...
            # Build MEV-Share bundle (similar to Flashbots but with privacy
            # params)
    ...
    async def _build_mev_share_bundle(
            self, bundle: BundleRequest) -> dict[str, Any]:
...
class BundleSubmitter:
    ...
    async def start(self) -> None:
        ...
        for name, config in self.relay_configs.items():
            if config.relay_type == RelayType.FLASHBOTS:
                ...
                client = FlashbotsRelay(
                    config, self.private_key, self.explain
                )  # Fallback
    ...
    async def submit_bundle(
        ...
        bundle_id: str,
        preferred_relays: list[str] | None = None,
            ...
            for relay_name in selected_relays:
                if relay_name in self.relays:
                    task = asyncio.create_task(
                        self.relays[relay_name].submit_bundle(
                            bundle, bundle_id))
    ...
    async def _select_relays(
        self, preferred_relays: list[str] | None = None
    ) -> list[str]:
        """
