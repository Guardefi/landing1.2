...
class ValidatorNode:
    ...
    async def validate_transfer(self, transfer: BridgeTransfer) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Validator {
                    self.validator_id} validated transfer {
                    transfer.transfer_id}: {validation_result}"
            )
            ...
            logger.error(
                f"Validation error for validator {
                    self.validator_id}: {e}"
            )
            self.metrics.failed_validations += 1
            return False
    ...
    async def _perform_validation(self, transfer: BridgeTransfer) -> bool:
        ...
        if transfer.from_chain.value not in settings.supported_chains:
            logger.warning(
                f"Source chain {
                    transfer.from_chain.value} not supported"
            )
            return False
        ...
        if transfer.to_chain.value not in settings.supported_chains:
            logger.warning(
                f"Destination chain {
                    transfer.to_chain.value} not supported"
            )
            return False
        ...
        if transfer.status in [
            ...
            logger.warning(
                f"Transfer {
                    transfer.id} is in invalid status: {
    ...
    def get_voting_power(self) -> float:
        ...
        reputation_multiplier = max(
            0.1, min(2.0, self.metrics.reputation_score))
