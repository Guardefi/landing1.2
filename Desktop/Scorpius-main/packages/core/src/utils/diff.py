...
class FindingChange:
    ...
    finding_id: str
    change_type: ChangeType
    old_finding: dict[str, Any] | None = None
    new_finding: dict[str, Any] | None = None
...
class ReportDiffEngine:
    ...
    async def compare_reports(
        ...
        if include_details:
            metric_changes = await self._compare_metrics(
                base_report.get("metrics", {}), compare_report.get("metrics", {})
            )

            metadata_changes = await self._compare_metadata(
                base_report.get("metadata", {}), compare_report.get("metadata", {})
            )

        # Generate recommendations
        recommendations = self._generate_recommendations(
            finding_changes, summary)
    ...
    async def _compare_findings(
        ...
        compare_map = {self._generate_finding_key(
            f): f for f in compare_findings}
        ...
        for key in base_map.keys() & compare_map.keys():
            ...
            field_changes = self._compare_finding_fields(
                base_finding, compare_finding)
    ...
    def _compare_finding_fields(
        self, base_finding: dict[str, Any], compare_finding: dict[str, Any]
    ) -> list[FieldChange]:
        """
    ...
    async def _compare_metrics(
        self, base_metrics: dict[str, Any], compare_metrics: dict[str, Any]
    ) -> list[FieldChange]:
        """
    ...
    async def _compare_metadata(
        self, base_metadata: dict[str, Any], compare_metadata: dict[str, Any]
    ) -> list[FieldChange]:
        """
    ...
    def _generate_summary(
        ...
        findings_added = len(
            [c for c in finding_changes if c.change_type == ChangeType.ADDED]
        )
        findings_removed = len(
            [c for c in finding_changes if c.change_type == ChangeType.REMOVED]
        )
        findings_modified = len(
            [c for c in finding_changes if c.change_type == ChangeType.MODIFIED]
        )

        total_changes = findings_added + findings_removed + findings_modified

        # Calculate risk score change
        base_risk = base_report.get(
            "metadata", {}).get(
            "overall_risk_score", 0)
        compare_risk = compare_report.get(
            "metadata", {}).get(
            "overall_risk_score", 0)
    ...
    def _calculate_severity_changes(
        self, finding_changes: list[FindingChange]
    ) -> dict[str, int]:
        """
        ...
        """
        severity_changes = {"critical": 0, "high": 0, "medium": 0, "low": 0}
    ...
    def _generate_recommendations(
        self, finding_changes: list[FindingChange], summary: DiffSummary
    ) -> list[str]:
        """
        ...
        if summary.total_changes == 0:
            recommendations.append(
                "No changes detected. Security posture remains stable."
            )
        ...
        for change in finding_changes:
            if change.change_type in [ChangeType.ADDED, ChangeType.MODIFIED]:
                ...
                if finding:
                    ...
                    function_changes[func_name] = function_changes.get(
                        func_name, 0) + 1

        # Find functions with multiple new issues
        problematic_functions = [
            func for func, count in function_changes.items() if count >= 3
        ]
    ...
    def _render_finding_changes(
            self, finding_changes: list[FindingChange]) -> str:
