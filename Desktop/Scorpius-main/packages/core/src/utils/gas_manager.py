...
class GasPriceOracle:
    ...
    async def get_current_gas_prices(self) -> GasPriceData:
        ...
        try:
            ...
            priority_fee = max(
                2 * 10**9, gas_price - base_fee)  # Minimum 2 gwei
    ...
    async def analyze_network_congestion(self) -> float:
        ...
        try:
            ...
            if isinstance(pending_count, dict):
                self._pending_tx_count = int(
                    pending_count.get("pending", "0x0"), 16)
    ...
    def predict_gas_price_trend(
            self, minutes_ahead: int = 5) -> tuple[int, float]:
        """
        ...
        try:
            ...
            if len(recent_prices) >= 5:
                recent_trend = statistics.mean(
                    recent_prices[-5:]) - statistics.mean(recent_prices[-10:-5])

                # Project trend forward
                current_price = recent_prices[-1]
                predicted_price = current_price + \
                    (recent_trend * minutes_ahead / 5)

                # Confidence based on trend consistency
                price_variance = (statistics.variance(
                    recent_prices) if len(recent_prices) > 1 else 0)
                confidence = max(0.1,
                                 1.0 - (price_variance / (current_price**2)))

                return int(max(1 * 10**9, predicted_price)
                           ), min(confidence, 0.9)
...
class GasManager:
    ...
    async def optimize_for_mev_profit(
        ...
        try:
            ...
            estimate = await self.estimate_gas(
                tx_params, GasStrategy.MEV_OPTIMIZED, expected_profit_wei
            )
            ...
            if estimate.total_cost_wei > max_gas_cost:
                if self.explain:
                    self.logger.info(
                        "Gas cost too high, trying adaptive strategy")

                estimate = await self.estimate_gas(
                    tx_params, GasStrategy.ADAPTIVE, expected_profit_wei
                )

                if estimate.total_cost_wei > max_gas_cost:
                    if self.explain:
                        self.logger.info(
                            "Still too high, trying conservative strategy")

                    estimate = await self.estimate_gas(
                        tx_params, GasStrategy.CONSERVATIVE, expected_profit_wei
                    )
            ...
            if estimate.total_cost_wei > max_gas_cost:
                ...
                if max_affordable_gas < 21000:  # Minimum gas for basic tx
                    if self.explain:
                        self.logger.warning(
                            "MEV opportunity not profitable after gas costs"
                        )
                    estimate.confidence = 0.0
                else:
                    estimate.gas_limit = int(max_affordable_gas)
                    estimate.total_cost_wei = (
                        max_affordable_gas * estimate.max_fee_per_gas
                    )
    ...
    async def _estimate_gas_limit(self, tx_params: TxParams) -> int:
        ...
        try:
            ...
            estimated = await asyncio.to_thread(self.web3.eth.estimate_gas, tx_params)
    ...
    async def _calculate_optimal_gas_prices(
        ...
        try:
            ...
            congestion_multiplier = 1.0 + \
                (congestion * 0.5)  # Up to 50% increase
            ...
            max_priority_fee = int(
                current_gas.priority_fee *
                priority_multiplier)
            max_fee = int(current_gas.base_fee *
                          base_multiplier) + max_priority_fee
            ...
            gas_price = min(
                max_fee, int(
                    current_gas.gas_price * base_multiplier))
    ...
    async def _calculate_gas_confidence(
        self, current_gas: GasPriceData, strategy: GasStrategy
    ) -> float:
        """
        ...
        try:
            ...
            if "gas_price" in historical_stats:
                ...
                price_deviation = abs(
                    current_price_gwei - mean_price) / mean_price
    ...
    def get_strategy_recommendation(
        self, expected_profit_wei: int, urgency: float = 0.5
    ) -> GasStrategy:
        """
        ...
        try:
            ...
            if expected_profit_wei > 10 * 10**18:  # > 10 ETH profit
                return (
                    GasStrategy.AGGRESSIVE
                    if urgency > 0.7
                    else GasStrategy.MEV_OPTIMIZED
                )

            # Medium profit transactions
            elif expected_profit_wei > 1 * 10**18:  # > 1 ETH profit
                return (GasStrategy.MEV_OPTIMIZED if urgency >
                        0.5 else GasStrategy.ADAPTIVE)

            # Lower profit transactions
            else:
                return (GasStrategy.ADAPTIVE if urgency >
                        0.7 else GasStrategy.CONSERVATIVE)
