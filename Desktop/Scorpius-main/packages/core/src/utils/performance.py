...
class PerformanceTracker:
    ...
    def __init__(self):
        ...
        base_decimals_found = token_decimals_map.get(
            self.base_token_symbol.upper())
        self.base_token_decimals = (
            base_decimals_found if isinstance(base_decimals_found, int) else 6
        )
        ...
        logger.info(
            f"PerformanceTracker Initialized. Base: {
                self.base_token_symbol} (dec={
                self.base_token_decimals}), Hist size={
    ...
    def track_execution(
        ...
        profit_token_decimals: int | None = None,
        ...
        if success:
            ...
            if (
                profit_token_symbol.upper() == self.base_token_symbol.upper()
                and profit_token_decimals == self.base_token_decimals
            ):
                ...
                logger.warning(
                    f"Non-base profit {amt_f:.6f} {profit_token_symbol} not added to base total."
                )
        else:
    ...
    def track_scan_cycle(self, latency_ms: float, last_block: int):
        ...
        if (
            isinstance(last_block, int)
            and last_block > self.metrics["last_block_processed"]
        ):
    ...
    def get_report(self) -> dict:
        ...
        m = self.metrics
        total_success = m["successful_arbitrages"] + \
            m["successful_liquidations"]
        win_rate = self._safe_division(total_success, m["total_attempts"])
        fb_inclusion = self._safe_division(
            m["flashbots_included"], m["flashbots_submissions"]
        )
        direct_inclusion = self._safe_division(
            m["direct_included"], m["direct_submissions"]
        )
        ...
        total_profit_base_dec = Web3.from_wei(
            m["total_profit_base_token_wei"], "ether"
        ) * (
            10 ** (18 - self.base_token_decimals)
        )  # Adjust if base token != 18 decimals
