...
async def initiate_transfer(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Unsupported chain: {
                    request.from_chain} or {
                    request.to_chain}",
            )
        ...
        try:
            SecurityLevel(request.security_level.lower())
        except ValueError:
...
async def get_transfer_status(
    ...
    try:
        ...
        if not transfer:
            raise HTTPException(
                status_code=404,
                detail="Transfer not found") from e
...
async def estimate_bridge_fee(
    ...
    try:
        # Validate chain types
        try:
            ChainType(from_chain.lower())
            ChainType(to_chain.lower())
        except ValueError:
            raise HTTPException(
                status_code=400,
                detail=f"Unsupported chain: {from_chain} or {to_chain}") from e
...
async def unregister_validator(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=404,
                detail="Validator not found") from e
...
async def lock_swap_funds(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400,
                detail="Failed to lock funds") from e
...
async def complete_atomic_swap(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400,
                detail="Cannot complete swap") from e
