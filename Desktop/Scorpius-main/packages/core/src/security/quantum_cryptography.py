...
class LatticeBasedCrypto:
    ...
    def decrypt(
        ...
        try:
            ...
            ciphertext_poly = self._decode_polynomial(
...
class HashBasedSignatures:
        ...
        try:
            ...
            private_key = QuantumKey(
                ...
                private_key=json.dumps(
                    ots_keys,
                    default=lambda x: x.hex() if isinstance(
                        x,
                        bytes) else x).encode(),
                parameters=self.params.copy(),
            )
    ...
    def sign(
        ...
        try:
            ...
            ots_signature = self._create_ots_signature(
                message_hash, ots_keys, index)
            ...
            signature_bytes = json.dumps(
                signature_data, default=lambda x: x.hex() if isinstance(
                    x, bytes) else x).encode()
    ...
    def _generate_ots_keys(self) -> list[dict[str, Any]]:
        ...
        for i in range(num_keys):
            ...
            ots_keys.append(
                {"private": private_key, "public": public_key, "index": i})
        ...
        while len(current_level) > 1:
            ...
            for i in range(0, len(current_level), 2):
                ...
                parent = hashlib.sha256(
                    left + right).digest()[: self.params["n"]]
    ...
    def _get_authentication_path(
        ...
        return [secrets.token_hex(self.params["n"])
...
class QuantumCryptographyEngine:
    ...
    async def generate_keypair(
        ...
        try:
            if algorithm == QuantumAlgorithm.LATTICE_BASED:
                ...
                raise ValueError(
                    f"Unsupported algorithm: {algorithm}") from None
            ...
            self.logger.info(
                f"Generated {
                    algorithm.value} keypair: {public_key_id}"
            )
    ...
    async def encrypt_message(self, message: bytes, public_key_id: str) -> str:
        ...
        try:
            ...
            if public_key.algorithm == QuantumAlgorithm.LATTICE_BASED:
                ...
                raise ValueError(
                    f"Encryption not supported for {
    ...
    async def verify_signature(
        ...
        try:
            ...
            if signature.algorithm == QuantumAlgorithm.HASH_BASED:
                result = self.hash_signatures.verify(
    ...
    async def get_performance_stats(self) -> dict[str, Any]:
        ...
        if stats["key_generations"] > 0:
            stats["avg_keygen_time"] = stats["total_time"] / \
                stats["key_generations"]

        if stats["encryptions"] > 0:
            stats["avg_encryption_time"] = stats["total_time"] / \
                stats["encryptions"]

        if stats["signatures"] > 0:
            stats["avg_signing_time"] = stats["total_time"] / \
                stats["signatures"]
