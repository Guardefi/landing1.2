...
SLITHER_CONFIDENCE_TO_SCORE = {"High": 0.9, "Medium": 0.7, "Low": 0.4}
...
class SlitherPlugin(BaseScannerPlugin):
    ...
    async def scan(
        ...
        if not self.initialized:
            ...
            if not self.initialized:
                logger.error(
                    "Cannot perform scan, Slither plugin not initialized")
                return []
        ...
        if target.source_code:
            ...
            logger.error(
                "Target does not provide source code, address, or bytecode")
            return []
    ...
    async def _scan_source_files(
        ...
        try:
            # Source code can be a dict of file paths to content or a single
            # string
            if isinstance(target.source_code, dict):
                ...
                # Find the main contract file (usually the one with the
                # contract name)
                main_file = next(
    ...
    async def _scan_deployed_contract(
        self, target: Target, config: ScanConfig
    ) -> List[VulnerabilityFinding]:
        ...
        """
        logger.info(
            f"Scanning deployed contract at {
        ...
        try:
            ...
            # Not fully implemented here as it requires API keys and network
            # setup

            # Example command:
            # slither {target.identifier} --etherscan-apikey
            # {config.custom_options.get('etherscan_api_key')}

            # For now, we'll log a warning
            logger.warning(
                "Scanning deployed contracts not fully implemented. Provide source code for best results."
            )
            return []
    ...
    async def _scan_bytecode(
        ...
        try:
            ...
            # For better results, we should use another tool like Mythril for
            # bytecode
            logger.warning(
                "Slither has limited support for bytecode analysis. Bytecode analysis might not be comprehensive."
            )
    ...
    async def _run_slither(
        self, file_path: str, target: Target, config: ScanConfig
    ) -> List[VulnerabilityFinding]:
        ...
        cmd = ["slither", file_path, "--json", output_file]
        ...
        try:
            ...
            if os.path.exists(output_file):
                ...
                logger.warning(
                    "Slither completed but no output file was created")
                return []
    ...
    def _parse_slither_output(
        self, slither_data: Dict[str, Any], target: Target
    ) -> List[VulnerabilityFinding]:
        ...
        for detector_result in slither_data.get(
            ...
            severity = SLITHER_IMPACT_TO_LEVEL.get(
                impact, VulnerabilityLevel.MEDIUM)
            ...
            if detector_result.get("elements") and len(
                    detector_result["elements"]) > 0:
                element = detector_result["elements"][0]
                if element.get("source_mapping") and element["source_mapping"].get(
                        "filename_absolute"):
                    file_path = element["source_mapping"]["filename_absolute"]
                    location = os.path.basename(file_path)

                    if (
                        element["source_mapping"].get("lines")
                        and len(element["source_mapping"]["lines"]) > 0
                    ):
            ...
            finding = VulnerabilityFinding(
                id=finding_id,
                vulnerability_type=detector_result.get(
                    "check", "unknown_vulnerability"
                ),
                title=detector_result.get("check", "Unknown Vulnerability"),
                description=detector_result.get(
                ...
                tags=[
                    "static-analysis",
                    "solidity",
                    detector_result.get("check", "").lower().replace(" ", "-"),
                ],
