...
class ManticorePlugin(BaseScannerPlugin):
    ...
    async def scan(
        ...
        if not self.initialized:
            ...
            if not self.initialized:
                logger.error(
                    "Cannot perform scan, Manticore plugin not initialized")
                return []
    ...
    async def _run_manticore(
        ...
        try:
            ...
            cmd = [
                ...
                file_path,
                "--solc-solcs-bin",
                config.custom_options.get("solc_path", "/usr/bin/solc"),
                "--core.procs",
                str(os.cpu_count() or 2),
                "--detect-all",
            ]
    ...
    async def _parse_manticore_output(
        self, output_dir: str, target: Target, config: ScanConfig
    ) -> List[VulnerabilityFinding]:
        ...
        try:
            ...
            for finding in manticore_findings:
                ...
                severity = MANTICORE_LEVEL_TO_SEVERITY.get(
