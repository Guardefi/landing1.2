...
class RealTimeThreatMonitor:
    ...
    async def _monitor_smart_contracts(self):
        ...
        while self.is_running:
            try:
                ...
                # 5 second interval for contract monitoring
                    ...
                    if avg_response > 1000:  # >1 second average
                        self.logger.warning(
                            f"Performance degradation: {
                                avg_response:.2f}ms avg response"
                        )
    ...
    async def _analyze_transaction_threats(
            self, tx: dict[str, Any]) -> dict[str, Any]:
    ...
    async def _apply_automatic_mitigation(self, alert: SecurityAlert):
        ...
        if alert.threat_type == "high_value_transaction":
            ...
            mitigation_actions.append(
                "Applied rate limiting to affected addresses")
