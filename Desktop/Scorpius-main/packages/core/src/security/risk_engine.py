...
import numpy as np
from arbitrage_math import Token, get_base_token_price, get_eth_price
...
try:
    ...
    # pandas_ta imported but not used in current implementation

    PANDAS_TA_AVAILABLE = True
except ImportError as e:
    PANDAS_TA_AVAILABLE = False
    logger.warning(
        f"Pandas TA library not installed. ATR/volatility calculations disabled. Error: {e}"
    )
...
class RiskEngine:
    ...
    def __init__(self):
        ...
        self.position_sizing_strategy: str = CONFIG.get(
            "POSITION_STRATEGY", "FIXED")
        self.max_risk_per_trade: float = CONFIG.get("MAX_RISK_PER_TRADE", 0.01)
        self.fixed_size_base_str: str = CONFIG.get(
            "FIXED_POSITION_SIZE", "1000")
        self.max_loan_size_usd: float = CONFIG.get("MAX_LOAN_SIZE_USD", 50000)
        self.min_eth_balance_wei: int = Web3.to_wei(
            CONFIG.get("MIN_ETH_BALANCE", "0.05"), "ether"
        )

        # If using pandas_ta for historical data
        self.historical_data: dict[str, pd.DataFrame] = (
            {} if PANDAS_TA_AVAILABLE else {}
        )

        logger.info(
            f"RiskEngine Initialized: Strategy={
                self.position_sizing_strategy}, "
            f"MaxRisk={
            ...
            f"MinETH={
                    Web3.from_wei(
                        self.min_eth_balance_wei,
                        'ether')}"
        )

    def update_market_data(self, pair_symbol: str,
        ...
        if not PANDAS_TA_AVAILABLE:
            logger.debug(
                "Cannot update market data - pandas_ta not available.")
            return
        if not data_points:
            return
        ...
        try:
            ...
            if not all(col in new_data_df.columns for col in required_cols):
                logger.warning(
                    f"Market data update for {pair_symbol} missing required columns.")
                return

            for col in ["open", "high", "low", "close", "volume"]:
                new_data_df[col] = pd.to_numeric(
                    new_data_df[col], errors="coerce")

            new_data_df.dropna(subset=required_cols, inplace=True)

            # Convert timestamp to datetime index
            if not pd.api.types.is_datetime64_any_dtype(
                    new_data_df["timestamp"]):
                new_data_df["timestamp"] = pd.to_datetime(
                    new_data_df["timestamp"], unit="s"
                )
            ...
            self.historical_data[pair_symbol] = combined_df.tail(
                required_length)

        except Exception as e:
            logger.error(
                f"Error updating market data for {pair_symbol}: {e}",
                exc_info=True)
    ...
    def calculate_historical_volatility(self, pair_symbol: str) -> float:
        ...
        try:
            ...
            vol_series = log_returns.rolling(
                window=self.volatility_window).std()
            last_vol = vol_series.iloc[-1]
            return float(last_vol) if pd.notna(last_vol) else 0.0
        except Exception as e:
            logger.error(
                f"Error calculating historical volatility for {pair_symbol}: {e}")
            return 0.0

    def get_pair_volatility(
            self,
            pair_symbol: str,
    ...
    async def calculate_max_position_size(
        ...
        if self.position_sizing_strategy == "FIXED":
            fixed_size_base = int(
                float(self.fixed_size_base_str) * (10**base_decimals))
            ...
            try:
                ...
                if base_price_usd <= 0:
                    raise ValueError(
                        f"Invalid base token price: {base_price_usd}")
                position_size_base_wei = int(
                    (capped_size_usd / base_price_usd) * (10**base_decimals)
                )
            except Exception as e:
            ...
            logger.info(
                f"Risk (VOL_ADJ): BalUSD={
                    account_balance_usd:.2f}, AdjRisk={
                    adjusted_max_risk *
                    100:.1f}%, "
                f"Vol={
                    safe_volatility:.4f}, Confidence={
                    confidence_factor:.2f} => "
                f"{
                        position_size_base_wei /
                        (
                            10**base_decimals):,.{base_decimals}f} {
                                base_token.symbol}"
            )
            ...
            logger.warning(
                f"Risk: Unknown strategy {
                    self.position_sizing_strategy}, using simple FIXED."
            )
    ...
    async def run_preflight_simulation(
        self, opportunity: dict, loan_amount: int, w3: Web3
    ) -> bool:
        """
