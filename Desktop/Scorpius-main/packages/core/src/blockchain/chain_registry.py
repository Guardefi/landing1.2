...
class ChainRegistry:
    def __init__(self, config_path=DEFAULT_CHAIN_CONFIG_PATH):
        ...
        try:
            ...
            logger.info(
                f"ChainRegistry initialized. Loaded {
                    len(
                        self.chains)} chain configs from '{config_path}'."
            )
        except Exception as e:
            logger.error(
                f"Failed to initialize ChainRegistry: {e}",
                exc_info=True)
            ...
            if filename.endswith(".json"):
                ...
                try:
                    with open(file_path) as f:
                        ...
                        chain_id_str = chain_config.get(
                            "chainId") or chain_config.get("chain_id")
                        if chain_id_str:
                            ...
                            logger.warning(
                                f"Skipping config {filename}: Missing 'chainId' or 'chain_id'.")
                except Exception as e:
                    logger.error(
                        f"Error loading chain config {filename}: {e}",
                        exc_info=True)
        # logger.debug(f"_load_chain_configs finished. Loaded {loaded_count}
        # configs.")

    def get_chain(self, chain_id: int) -> dict | None:
        return self.chains.get(chain_id)

    def get_rpc_url(self, chain_id: int) -> str | None:
        ...
        if rpc_url:
            ...
            if rpc_url:
                return rpc_url
        env_var_key = chain_config.get(
            "rpcEnvVar") or chain_config.get("rpc_env_var")
        if env_var_key:
            rpc_url = os.getenv(env_var_key)
            if rpc_url:
                logger.debug(
                    f"Using RPC URL from environment variable {env_var_key} for chain {chain_id}.")
                return rpc_url
            else:
                logger.warning(
                    f"Environment variable {env_var_key} specified for chain {chain_id}, but it's not set.")
        ...
        if rpc_url:
            return rpc_url
        logger.error(
            f"Could not determine RPC URL for chain ID {chain_id} (Name: {
                chain_config.get(
                    'name', 'N/A')})."
        )
        return None

    def get_web3_provider(self, chain_id: int) -> Web3 | None:
        if chain_id in self.providers:
            if self.providers[chain_id].isConnected():
                return self.providers[chain_id]
            else:
                logger.warning(
                    f"Cached provider for chain {chain_id} disconnected. Creating new one.")
                del self.providers[chain_id]
        rpc_url = self.get_rpc_url(chain_id)
        if not rpc_url:
            logger.error(
                f"Cannot create Web3 provider for chain {chain_id}: No RPC URL found.")
            return None
        try:
            ...
            if provider.isConnected():
                ...
                logger.info(
                    f"Created and cached Web3 provider for chain {chain_id} at {rpc_url}")
                return provider
            else:
                logger.error(
                    f"Failed to connect Web3 provider for chain {chain_id} at {rpc_url}.")
                return None
        except Exception as e:
            logger.error(
                f"Error creating Web3 provider for chain {chain_id} ({rpc_url}): {e}",
                exc_info=True,
            )
            return None
    ...
    def add_chain(self, chain_config: dict) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Cannot add chain: Invalid chain ID format '{chain_id}'. Must be integer.")
            return False
        self.chains[chain_id] = chain_config
        ...
        try:
            ...
            logger.info(
                f"Added/Updated chain {chain_id} config to {filepath}.")
            if chain_id in self.providers:
                del self.providers[chain_id]
                logger.debug(
                    f"Invalidated provider cache for updated chain {chain_id}."
                )
            return True
        except Exception as e:
            logger.error(
                f"Error saving chain config file {filepath}: {e}",
                exc_info=True)
    ...
    def update_chain(self, chain_id: int, chain_config: dict) -> bool:
        if chain_id not in self.chains:
            logger.warning(
                f"Chain {chain_id} not found for updating. Use add_chain instead.")
            return False
...
class ProtocolRegistry:
    def __init__(self, config_path=DEFAULT_PROTOCOL_CONFIG_PATH):
        ...
        try:
            ...
            logger.info(
                f"ProtocolRegistry initialized. Loaded {
                    len(
                        self.protocols)} protocol configs from '{
                    self.config_path}'."
            )
        except Exception as e:
            logger.error(
                f"Failed to initialize ProtocolRegistry: {e}",
                exc_info=True)
                ...
                try:
                    with open(file_path) as f:
                        ...
                        if protocol_name:
                            ...
                            logger.warning(
                                f"Skipping protocol config {filename}: Missing 'name'.")
                except Exception as e:
                    logger.error(
                        f"Error loading protocol config {filename}: {e}",
                        exc_info=True)
        # logger.debug(f"_load_protocol_configs finished. Loaded {loaded_count}
        # configs.")

    def get_protocol(self, protocol_name: str) -> dict | None:
    ...
    def get_protocol_abis(self, protocol_name: str) -> dict[str, list]:
        ...
        if not protocol:
            logger.warning(
                f"Cannot load ABIs: Protocol '{protocol_name}' not found.")
            return {}
        abis = {}
        contracts_info = protocol.get("contracts", {})
        if not contracts_info:
            logger.warning(
                f"Protocol '{protocol_name}' config has no 'contracts' section.")
            return {}
        for contract_name, contract_details in contracts_info.items():
            abi_path = contract_details.get("abiPath") or contract_details.get(
                "abi_path"
            )
            if abi_path:
                try:
                    ...
                    with open(abi_full_path) as f:
                        ...
                        logger.debug(
                            f"Loaded ABI for {protocol_name}/{contract_name} from {abi_full_path}")
                except Exception as e:
                    logger.error(
                        f"Error loading ABI for {protocol_name}/{contract_name} from {abi_path}: {e}",
                        exc_info=True,
                    )
            else:
                logger.debug(
                    f"No 'abiPath' defined for contract '{contract_name}' in protocol '{protocol_name}'.")
        return abis

    def get_protocol_chain_config(
        self, protocol_name: str, chain_id: int
    ) -> dict | None:
        protocol = self.get_protocol(protocol_name)
        if not protocol:
    ...
    def add_protocol(self, protocol_config: dict) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Added/Updated protocol '{protocol_name}' config to {filepath}.")
            return True
        except Exception as e:
            logger.error(
                f"Error saving protocol config file {filepath}: {e}",
                exc_info=True)
    ...
    def update_protocol(
            self,
            protocol_name: str,
            protocol_config: dict) -> bool:
        protocol_key = protocol_name
        if protocol_key not in self.protocols:
            logger.warning(
                f"Protocol '{protocol_name}' not found for updating. Use add_protocol instead.")
            return False
    ...
    def get_protocols_for_chain(self, chain_id: int) -> dict[str, dict]:
        ...
        for protocol_name, protocol_config in self.protocols.items():
            chain_config = protocol_config.get(
                "chains", {}).get(target_chain_id_str)
            if chain_config:
                chain_protocols[protocol_name] = chain_config
        logger.debug(
            f"Found {
                len(chain_protocols)} protocols for chain {chain_id}.")
...
class MultiChainMonitor:
    ...
    async def initialize(self, event_bus, ml_model):
        ...
        start_tasks = [
            self.start_chain_monitor(chain_id) for chain_id in enabled_chains
        ]
        results = await asyncio.gather(*start_tasks, return_exceptions=True)
        for chain_id, result in zip(enabled_chains, results, strict=False):
            if isinstance(result, Exception):
                self.logger.error(
                    f"Failed monitor for chain {chain_id}: {result}",
                    exc_info=result)
            elif not result:
                self.logger.error(
                    f"Monitor for chain {chain_id} did not start successfully (returned False).")
    ...
    async def start_chain_monitor(self, chain_id: int) -> bool:
        if chain_id in self.active_monitors:
            self.logger.warning(
                f"Monitor for chain {chain_id} is already active.")
            return True
        ...
        self.logger.info(
            f"Starting monitor for chain {chain_id} ({chain_name})...")
        provider = self.chain_registry.get_web3_provider(chain_id)
        if not provider:
            return False
        chain_protocols = self.protocol_registry.get_protocols_for_chain(
            chain_id)
        protocol_names = list(chain_protocols.keys())
        if not protocol_names:
            self.logger.warning(
                f"No protocols configured for chain {chain_id}.")
        try:
        except ImportError:
        ...
        try:
            monitor = ProductionMempoolMonitor(
                monitor_config, self.ml_model, self.event_bus
            )
            monitor_task = asyncio.create_task(monitor.start())
            self.monitor_tasks[chain_id] = monitor_task
            self.active_monitors[chain_id] = monitor
            self.logger.info(
                f"Started monitor for chain {chain_id} ({chain_name}). Protocols: {protocol_names}")
            return True
        except Exception as e:
            self.logger.error(
                f"Failed to start monitor for chain {chain_id}: {e}",
                exc_info=True)
            return False
    ...
    async def add_chain_monitor(self, chain_id: int) -> bool:
        ...
        if chain_id in self.active_monitors:
            self.logger.warning(
                f"Monitor for chain {chain_id} already exists.")
            return False
        success = await self.start_chain_monitor(chain_id)
        if success:
            self.logger.info(
                f"Dynamically added monitor for chain {chain_id}.")
        else:
    ...
    async def remove_chain_monitor(self, chain_id: int) -> bool:
        ...
        try:
            if hasattr(
                    monitor,
                    "stop") and asyncio.iscoroutinefunction(
                    monitor.stop):
                await monitor.stop()
            else:
                monitor_task.cancel()
                try:
                    ...
                    self.logger.debug(
                        f"Monitor task for chain {chain_id} cancelled.")
                except Exception as e:
                    self.logger.error(
                        f"Error cancelling monitor task for chain {chain_id}: {e}")
            del self.active_monitors[chain_id]
            del self.monitor_tasks[chain_id]
            self.logger.info(
                f"Successfully removed monitor for chain {chain_id}.")
            return True
        except Exception as e:
            self.logger.error(
                f"Exception while stopping monitor for chain {chain_id}: {e}",
                exc_info=True,
            )
            return False
    ...
    async def stop_all(self):
        ...
        stop_tasks = [self.remove_chain_monitor(
        ...
        self.logger.info(
            f"Stopped {stopped_count} monitors. Failed to stop {failed_count}."
        )
