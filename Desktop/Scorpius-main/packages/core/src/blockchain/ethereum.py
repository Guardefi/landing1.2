...
class EthereumChain(BaseChain):
    ...
    def __init__(
        ...
        if not WEB3_AVAILABLE:
            self.logger.warning(
                "Web3.py not available. Install with: pip install web3")
    ...
    async def get_transaction_receipt(
            self, tx_hash: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get transaction receipt {tx_hash}: {e}")
            return None
    ...
    async def get_transaction_confirmations(self, tx_hash: str) -> int:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get confirmations for {tx_hash}: {e}")
            return 0
