...
# import asyncpg  # Commented out for now - will be added when asyncpg is
# available
...
class CostOptimizationEngine:
    ...
    def __init__(self):
        ...
        self.opencost_url = os.getenv(
            "OPENCOST_URL", "http://opencost.opencost.svc.cluster.local:9003"
        )
        self.database_url = os.getenv(
            "DATABASE_URL",
            "postgresql://scorpius:password@postgres:5432/scorpius")
    ...
    async def collect_metrics(self) -> List[CostMetric]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                # Combine metrics
                metrics = await self._combine_metrics(
                    cpu_metrics, memory_metrics, cost_data
                )
    ...
    async def _combine_metrics(
        self, cpu_data: Dict, memory_data: Dict, cost_data: Dict
    ) -> List[CostMetric]:
        ...
        if "data" in cost_data:
            for allocation in cost_data["data"]:
                ...
                cpu_efficiency = self._calculate_cpu_efficiency(
                    cpu_data, namespace, pod
                )
                memory_efficiency = self._calculate_memory_efficiency(
                    memory_data, namespace, pod
                )

                # Overall efficiency (weighted average)
                overall_efficiency = (cpu_efficiency * 0.6) + \
    ...
    def _calculate_cpu_efficiency(
        self, cpu_data: Dict, namespace: str, pod: str
    ) -> float:
        ...
        for result in cpu_data["data"]["result"]:
            ...
            if metric.get("namespace") == namespace and metric.get(
                    "pod") == pod:
    ...
    def _calculate_memory_efficiency(
        self, memory_data: Dict, namespace: str, pod: str
    ) -> float:
    ...
    async def analyze_costs(
        ...
        for metric in metrics:
            ...
            if metric.efficiency < self.memory_efficiency_threshold:
                memory_recommendation = await self._generate_memory_recommendation(
                    metric
                )
    ...
    async def _generate_cpu_recommendation(
        ...
        return OptimizationRecommendation(
            ...
            risk_level="low",
            description=f"CPU over-provisioned. Current efficiency: {
                metric.efficiency:.2%}. Recommended: Reduce CPU allocation by {
                    (
                        1 - optimal_cpu) * 100:.1f}%",
    ...
    async def _generate_memory_recommendation(
        ...
        return OptimizationRecommendation(
            ...
            implementation_difficulty="medium",
            risk_level="medium",
            description=f"Memory over-provisioned. Current efficiency: {
                metric.efficiency:.2%}. Recommended: Reduce memory allocation by {
                    (
                        1 - optimal_memory) * 100:.1f}%",
            implementation_steps=[
    ...
    async def _generate_idle_recommendation(
        self, metric: CostMetric
    ) -> Optional[OptimizationRecommendation]:
        ...
        return OptimizationRecommendation(
            ...
            risk_level="low",
            description=f"Idle resource detected. Efficiency: {
                metric.efficiency:.2%}. Consider scaling down or removing.",
            implementation_steps=[
    ...
    async def _get_current_spend(self, period: str) -> float:
        ...
        async with aiohttp.ClientSession() as session:
            if period == "daily":
                url = f"{
                    self.opencost_url}/model/allocation?window=1d&aggregate=cluster"
            elif period == "monthly":
                url = f"{
                    self.opencost_url}/model/allocation?window=30d&aggregate=cluster"
            else:
                return 0.0
    ...
    async def send_notifications(
        ...
        if not self.slack_webhook:
            logger.warning(
                "Slack webhook not configured, skipping notifications")
            return

        # Prepare notification message
        message = self._prepare_notification_message(
            recommendations, budget_alerts)

        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self.slack_webhook, json={"text": message}
                ) as response:
                    if response.status == 200:
    ...
    def _prepare_notification_message(
        ...
        if recommendations:
            total_savings = sum(
                rec.projected_savings for rec in recommendations)
            message_parts.append(
                f"💰 *Optimization Opportunities: ${total_savings:.2f}/month potential savings*"
            )
    ...
    try:
        ...
        print(
            f"Potential Monthly Savings: ${
                report['summary']['total_potential_savings']:.2f}"
        )
        print(f"Budget Alerts: {report['summary']['budget_alerts']}")

        # Save report to file
        with open(
            f"/tmp/cost-optimization-report-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}.json",
            "w",
        ) as f:
