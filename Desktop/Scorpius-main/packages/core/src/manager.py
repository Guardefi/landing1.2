...
class ValidatorManager:
    ...
    async def register_validator(
        ...
        try:
            ...
            if stake_amount <= 0:
                logger.error(
                    f"Invalid stake amount for validator {validator_id}")
                return False
    ...
    async def validate_transfer(
            self, transfer: BridgeTransfer) -> ConsensusResult:
        ...
        try:
            ...
            if len(active_validators) < settings.min_validators:
                logger.error(
                    f"Not enough active validators: {
                        len(active_validators)} < {
                        settings.min_validators}"
                )
            ...
            logger.info(
                ...
                f"transfer_id={
                    transfer.transfer_id}, consensus={
                    result.reached}"
            )
            ...
            try:
                ...
                for validator in self.validators.values():
                    if validator.status == ValidatorStatus.ACTIVE:
                        ...
                        logger.debug(
                            f"Heartbeat from {
                                validator.validator_id}: {heartbeat}"
                        )
                        ...
                        if not validator.is_healthy():
                            logger.warning(
                                f"Validator {
                                    validator.validator_id} became unhealthy"
                            )
