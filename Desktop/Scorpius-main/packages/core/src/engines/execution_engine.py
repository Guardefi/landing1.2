...
from .bundle_submitter import BundleSubmissionResult, BundleSubmitter
from .gas_manager import GasManager, GasPriceOracle
...
class ExecutionResult:
    ...
    actual_profit_wei: int | None = None
    gas_cost_wei: int = 0
    net_profit_wei: int | None = None
    ...
    @property
    def profit_percentage(self) -> float | None:
...
class ExecutionEngine:
    ...
    async def execute_opportunity(
        self, opportunity: MEVOpportunity, strategy_result: StrategyResult
    ) -> ExecutionResult:
        """
        ...
        try:
            ...
            if execution.expected_profit_wei < self.profit_threshold_wei:
                ...
                execution.error = f"Profit below threshold: {
                    execution.expected_profit_wei / 10**18:.6f} ETH"
                return execution
            ...
            optimized_bundle = await self._optimize_bundle_gas(
                bundle, execution.expected_profit_wei
            )
            ...
            nonce_reservations = await self._reserve_bundle_nonces(
                optimized_bundle, execution_id
            )
            ...
            if self.explain:
                successful_submissions = sum(
                    1 for r in submission_results if r.status == BundleStatus.SUBMITTED)
                self.logger.info(
    ...
    async def _optimize_bundle_gas(
        self, bundle: BundleRequest, expected_profit_wei: int
    ) -> BundleRequest | None:
        """
    ...
    async def _reserve_bundle_nonces(
        self, bundle: BundleRequest, execution_id: str
    ) -> list[Any]:
        """
        ...
        try:
            ...
            for i, tx in enumerate(bundle.transactions):
                nonce_reservation = await self.nonce_manager.reserve_nonce(
                    self.wallet_address, f"{execution_id}_tx_{i}", tx.gas_price
                )
        ...
        for reservation in reservations:
            if hasattr(reservation, "transaction_id"):
                await self.nonce_manager.confirm_nonce(
                    reservation.transaction_id, success=False
                )
        ...
        while self._running:
            try:
                ...
                for execution_id, execution in self.active_executions.items():
                    # Check for timeouts
                    if (
                        current_time - execution.created_at > self.execution_timeout
                    ...
                    if (
                        execution.bundle_id
                        and execution.status == ExecutionStatus.SUBMITTED
                    ):
                        updated_results = (
                            await self.bundle_submitter.check_bundle_inclusion(
                                execution.bundle_id
                            )
                        )
                        ...
                        included_results = [
                            r for r in updated_results if r.included]
                        if included_results:
                            execution.status = ExecutionStatus.INCLUDED
                            execution.included_at = current_time

                            # Calculate actual profit from first included
                            # result
                            ...
                            if best_result.profit_wei:
                                ...
                                execution.net_profit_wei = (
                                    best_result.profit_wei - execution.gas_cost_wei)
                    ...
                    if execution.status == ExecutionStatus.FAILED:
                        ...
                        if execution.net_profit_wei and execution.net_profit_wei < 0:
                            self.metrics["worst_loss_wei"] = min(
                                self.metrics["worst_loss_wei"], execution.net_profit_wei)

                # Update average execution time
                completed_executions_with_time = [
                    ex for ex in self.execution_history if ex.execution_time is not None]

                if completed_executions_with_time:
                    total_time = sum(
                        ex.execution_time for ex in completed_executions_with_time)
                    self.metrics["avg_execution_time"] = total_time / len(
                        completed_executions_with_time
                    )
    ...
    def get_execution_status(
            self,
            execution_id: str) -> ExecutionResult | None:
        """
    ...
    def get_performance_metrics(self) -> dict[str, Any]:
        ...
        success_rate = (
            self.metrics["successful_executions"] / max(total_executions, 1)
        ) * 100
