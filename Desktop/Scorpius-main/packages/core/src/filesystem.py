...
class ExportFilter:
    ...
    def __init__(
        ...
        """
        self.excludes = (
            excludes if excludes is not None else self.DEFAULT_EXCLUDES.copy()
        )
    ...
    def should_include(self, file_path: str, relative_path: str) -> bool:
        ...
        if self.includes_only:
            included = any(
                ...
                for pattern in self.includes_only
            )
            if not included:
                return False
        ...
        for pattern in self.excludes:
            if fnmatch.fnmatch(relative_path, pattern):
                logger.debug(
                    f"Excluding {relative_path} (matches pattern: {pattern})")
                return False
...
class FileSystemExporter:
    ...
    def create_zip_export(
        ...
        with zipfile.ZipFile(output_path, "w", compression=compression) as zipf:
            for root, dirs, files in os.walk(source_path):
                # Remove excluded directories from dirs list to prevent walking
                # into them
                dirs[:] = [
                    d
                    for d in dirs
                    if self._should_include_dir(os.path.join(root, d), source_path)
                ]
                ...
                for file in files:
                    ...
                    if self.filter.should_include(file_path, relative_path):
                        try:
                            ...
                            logger.warning(
                                f"Failed to add {relative_path} to archive: {
                                    str(e)}"
                            )
        ...
        logger.info(
            f"ZIP export completed: {
                stats['files_included']} files, "
            f"{
    ...
    def create_tar_export(
        self, source_path: str, output_path: str, compression: str = "gz"
    ) -> Dict[str, Union[str, int]]:
        """
    ...
    def calculate_directory_hash(self, directory_path: str) -> str:
        ...
        for root, dirs, files in os.walk(directory_path):
            ...
            dirs[:] = [
                d
                for d in dirs
                if self._should_include_dir(os.path.join(root, d), directory_path)
            ]
    ...
    def clean_directory(
        self, directory_path: str, dry_run: bool = True
    ) -> Dict[str, List[str]]:
        """
        ...
        result = {"files_removed": [], "directories_removed": [], "errors": []}

        logger.info(
            f"Cleaning directory: {directory_path} (dry_run={dry_run})")
        ...
        logger.info(
            f"Cleaning completed: {len(result['files_removed'])} files, "
