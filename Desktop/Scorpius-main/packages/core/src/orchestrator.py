...
class CoreOrchestrator:
    ...
    async def _get_redis(self) -> redis.Redis:
        ...
        if self._redis_client is None:
            try:
                ...
                logger.warning(
                    f"Could not connect to Redis for service registry: {e}")
    ...
    def register_service(
        ...
        if name in self.services:
            logger.warning(
                f"Service {name} already registered, updating registration")
    ...
    async def _store_service_in_redis(
            self, name: str, service_info: ServiceInfo):
        ...
        try:
            ...
            if redis_client:
                ...
                await redis_client.hset(
                    "scorpius:services", name, json.dumps(service_data)
                )
    ...
    async def start_service(self, name: str) -> bool:
        ...
        try:
            ...
            if hasattr(module, service.entry_point):
                ...
                logger.error(
                    f"Entry point {
                        service.entry_point} not found in {
                        service.module_path}"
                )
    ...
    async def stop_service(self, name: str) -> bool:
        ...
        if service.status != "running":
            logger.warning(
                f"Service {name} is not running (status: {
                    service.status})"
            )
            return True
    ...
    async def check_health(self, service_name: str = None) -> Dict[str, str]:
        ...
        health_status = {}
        services_to_check = [
            service_name] if service_name else self.services.keys()
        ...
        for name in services_to_check:
            ...
            if service.health_check and service.instance:
                ...
                # No health check defined, assume running
    ...
    async def start_all_services(self) -> bool:
        ...
        while remaining and self._running:
            ...
            if not progress:
                logger.error(
                    "Dependency cycle detected, cannot start remaining services"
                )
    ...
    async def stop_all_services(self) -> None:
        ...
        running = [
            name for name,
            svc in self.services.items() if svc.status == "running"]
        stopped = set()
    ...
    def get_services(self) -> Dict[str, Dict[str, Any]]:
        ...
        for name, service_info in self.services.items():
            ...
            result[name] = {
                "host": service_info.metadata.get(
                    "host",
                    f"{name}-service"),
                "port": service_info.metadata.get(
                    "port",
                    8000),
                "health": service_info.metadata.get(
                    "health_endpoint",
                    "/health"),
                "status": service_info.status.value,
                "dependencies": service_info.dependencies,
            ...
            if redis_client:
                ...
                for name, service_json in services_data.items():
                    ...
                    try:
                        ...
                        service_info.status = ServiceStatus(
                            service_data.get("status", "registered")
                        )
                        self.services[name] = service_info
                        logger.debug(
                            f"Loaded/updated service {name} from Redis")
                    except Exception as e:
                        logger.warning(
                            f"Failed to load service {name} from Redis: {e}")
        except Exception as e:
