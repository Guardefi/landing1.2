...
from flashbots_helper import (
    build_flashbots_bundle,
    send_flashbots_bundle,
    simulate_bundle,
)
...
async def main():
    ...
    logger.info("MLClient loaded model from model_161.joblib")
    ...
    while True:
        ...
        results = await run_scan(
            RPC_URL, None, offset=latest_block, ml_client=ml_client
        )
        ...
        if results and isinstance(results, list):
            for opp in results:
                ...
                if profit > 50:
                    logger.info(
                        f"🚀 Triggering flashloan for {
                            opp['pair']} | Profit: {profit}"
                    )
                    log_ml_prediction(
                        opp,
                        opp.get(
                    ...
                    try:
                        ...
                        tx = contract.functions.executeArb(
                            opp["token0"], opp["token1"], opp["amount_in"]
                        ).build_transaction(
                            {
                        ...
                        sim_result = simulate_bundle(
                            w3, bundle, latest_block + 1)

                        if "result" in sim_result:
                            send_result = send_flashbots_bundle(
                                w3, bundle, latest_block + 1
                            )
                            logger.info(
                                f"✅ Bundle submitted: {
                            ...
                            logger.warning(
                                "⚠️ Simulation failed, skipping submission.")

                    except Exception as e:
                        logger.warning(
                            f"⚠️ Error during bundle build/send: {e}")
