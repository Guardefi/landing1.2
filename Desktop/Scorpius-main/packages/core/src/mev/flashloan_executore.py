...
class FlashLoanArbitrageExecutor:
    ...
    def __init__(self, config):
        ...
        if not config.get("PRIVATE_KEY"):
            raise ValueError(
                "PRIVATE_KEY is required in config for FlashLoanArbitrageExecutor"
            )
        ...
        if not self.aggregator_contract_address:
            raise ValueError(
                "ARBITRAGE_CONTRACT address is required in config")
        try:
            self.aggregator_contract_address = Web3.toChecksumAddress(
                self.aggregator_contract_address
            )
        except ValueError:
            raise ValueError(
                f"Invalid ARBITRAGE_CONTRACT address format: {
                    self.aggregator_contract_address}"
            )
        ...
        self.logger.info(
            f"Executor configured for aggregator contract: {
                self.aggregator_contract_address}"
        )
    ...
    def _encode_aggregator_call(
        ...
        for action in strategy_sequence:
            if action.get("type") == "swap":
                ...
                self.logger.warning(
                    f"Unsupported action type in strategy: {
                        action.get('type')}"
                )
                continue
    ...
    async def _tenderly_gas_simulate(self, tx_params: dict) -> int | None:
        """
    ...
    async def execute_arbitrage(self, opportunity: dict):
        ...
        self.logger.info(
            f"Executing arbitrage. Estimated Profit: {
                opportunity.get(
                    'estimated_profit',
                    0):.6f} ETH"
        )
        try:
            ...
            self.logger.info(
                f"Selected flash loan provider: {provider_name} (ID: {provider_id})")

            gas_settings = await asyncio.to_thread(
                self.gas_optimizer.estimate_optimal_gas_eip1559
            )
            self.logger.info(
                f"Gas Settings: MaxFeePerGas={
                    gas_settings['maxFeePerGas'] /
                    1e9:.2f} Gwei, "
                + f"MaxPriorityFeePerGas={
                    gas_settings['maxPriorityFeePerGas'] /
                    1e9:.2f} Gwei"
            )

            tx_calldata = self._encode_aggregator_call(
                provider_id,
                opportunity["token_address"],
                opportunity["loan_amount"],
                opportunity["strategy_sequence"],
            )
            ...
            try:
                estimated_gas = await self._estimate_gas_with_fallback(
                    tx_params_for_estimate
                )
                ...
                self.logger.warning(
                    f"Gas estimation failed: {estimate_e}. Using default gas limit.")
            ...
            self.account.sign_transaction(full_tx)
            self.logger.info(
                "Transaction signed. Submitting via MEV protection.")
            protection_result = await self.mev_protection_manager.protect_transaction(
                full_tx
            )
            if not protection_result or not protection_result.get("success"):
                self.logger.error(
                    f"MEV protection submission failed: {
            ...
            self.logger.info(
                f"Tx submitted via {service_used}. Tx Hash: {tx_hash}")
            ...
            if receipt and receipt.status == 1:
                ...
                self.logger.info(
                    f"Tx Confirmed! Gas Used: {gas_used}, Gas Cost: {
                        gas_cost_eth:.6f} ETH, Profit: {
                        actual_profit_eth:.6f} ETH, Net: {
                        net_profit_eth:.6f} ETH"
                )
                ...
                fail_reason = (
                    "Transaction reverted or status is 0."
                    if receipt
                    else f"Receipt timeout for tx {tx_hash}."
                )
            ...
            self.logger.critical(
                f"Critical error during arbitrage execution: {e}",
                exc_info=True)
            return False, f"Critical error: {e}"

    async def _wait_for_transaction_receipt(
            self, tx_hash, timeout=120, poll_latency=1):
        self.logger.info(
            f"Waiting for receipt for tx {tx_hash} (timeout={timeout}s)..."
        )
        try:
            ...
            self.logger.info(
                f"Receipt received for tx {tx_hash}. Status: {receipt.status}"
            )
            return receipt
            ...
            self.logger.warning(
                f"Error or timeout waiting for receipt for tx {tx_hash}: {e}"
            )
            return None
    ...
    def _parse_profit_from_logs(self, logs: list) -> float:
        ...
        """
        profit_wei = 0
        profit_event_signature = Web3.keccak(
            text="ProfitReported(address,uint256)"
        ).hex()
        for log in logs:
            if (
                log.address == self.aggregator_contract_address
                and log.topics[0].hex() == profit_event_signature
            ):
                try:
                    profit_wei = Web3.toInt(hexstr=log.data.hex())
                    self.logger.info(
                        f"ProfitReported event found. Profit (Wei): {profit_wei}")
                    break
                except Exception as e:
                    self.logger.error(
                        f"Error decoding ProfitReported event: {e}.")
    ...
    def _parse_revert_reason(self, error: Exception) -> str:
        ...
        """
        try:
            # Extract hex data from error message (this may depend on your
            # provider)
            hex_data = error.args[0].get("data", "")[2:]
            decoded = self.aggregator_contract.decode_function_input(
                "0x" + hex_data)
    ...
    async def _handle_failed_tx(self, tx_hash: str):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                self.logger.error(
                    f"Failed to analyze failed tx {tx_hash}: {e}")
                return {"error": str(e)}
...
async def _run_test_scenario(executor, scenario):
    ...
    try:
        ...
        receipt = await executor.w3.eth.wait_for_transaction_receipt(
            tx_result[1].get("tx_hash")
        )
        return {
