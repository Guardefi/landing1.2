...
class MEVMetrics:
    ...
    def __init__(self):
        self.metrics = {
            ...
            "public_fallback": {"count": 0, "success_rate": 0},
        }

    def update_provider_metrics(
        self, provider: str, success: bool, latency: float, gas_used: int
    ):
        data = self.metrics["providers"][provider]
        data["submissions"] += 1
        data["successes"] += int(success)
        data["avg_latency"] = (
            (data["avg_latency"] * (data["submissions"] - 1)) + latency
        ) / data["submissions"]
        if success:
...
class MEVProtectionManager:
    ...
    def __init__(self, config, w3: Web3):
        ...
        self.providers = {
            "flashbots": FlashbotsProvider(
                w3,
                signature_account=Account.from_key(
                    config["FLASHBOTS_SIGNER_KEY"]),
                endpoint=config.get(
                    "FLASHBOTS_ENDPOINT",
    ...
    async def submit_bundle(self, bundle, target_block) -> dict:
        ...
        try:
            result = await self.providers["flashbots"].send_bundle(
                bundle, target_block_number=target_block, opts={"simulation": True}
            )
            return {
                "success": result.get("bundleHash") is not None,
    ...
    async def submit_private_tx(self, signed_tx) -> dict:
        ...
        for _provider_name, handler in providers:
    ...
    async def _submit_flashbots(self, signed_tx) -> dict:
        try:
            ...
            result = await self.providers["flashbots"].send_bundle(
                bundle, target_block_number=target_block, opts={"simulation": False}
            )
            return {
                "success": result.get("bundleHash") is not None,
                "tx_hash": result.get("bundleHash"),
    ...
    async def _submit_eden(self, signed_tx) -> dict:
        try:
            result = await self.providers["eden"].send_transaction(
                signed_tx.rawTransaction
            )
            return {
                "success": result.get("tx_hash") is not None,
    ...
    async def _submit_bloxroute(self, signed_tx) -> dict:
        try:
            result = await self.providers["bloxroute"].submit_transaction(
...
class EnhancedMEVProtection:
    ...
    def _parse_revert_reason(self, error: Exception) -> str:
        ...
        try:
            ...
            decoded = self.mev_manager.providers[
                "flashbots"
            ].contract.decode_function_input("0x" + hex_data)
