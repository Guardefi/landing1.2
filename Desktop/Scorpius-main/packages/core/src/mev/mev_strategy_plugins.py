...
class MEVStrategyPlugin(ABC):
    ...
    async def execute_strategy(
            self,
            opportunity: MEVOpportunity) -> ExecutionResult:
    ...
    async def validate_opportunity(self, opportunity: MEVOpportunity) -> bool:
        ...
        return opportunity.estimated_profit > self.config.get(
            "min_profit", 0.01) and opportunity.confidence_score > self.config.get(
            "min_confidence", 0.7)
...
class SandwichAttackPlugin(MEVStrategyPlugin):
    ...
    async def _calculate_sandwich_profit(
            self, tx_data: dict[str, Any]) -> float:
        ...
        return [
            {
                ...
                # Use 10% of target volume
                "amount": tx_data.get("value", 0) * 0.1,
...
class ArbitragePlugin(MEVStrategyPlugin):
    ...
    async def _calculate_arb_profit(
            self,
            price_a: float,
            price_b: float) -> float:
...
class LiquidationPlugin(MEVStrategyPlugin):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.health_factor_threshold = config.get(
    ...
    async def execute_strategy(
        ...
        self.logger.info(
            f"Executing liquidation strategy for {
                opportunity.id}"
        )
