...
class TwoHopArbitrageStrategy(AbstractStrategy):
    ...
    def __init__(self, config: StrategyConfig, web3, explain: bool = False):
        ...
        self.logger.info(
            f"Two-hop arbitrage strategy initialized with {len(self._monitored_pairs)} pairs"
        )
    ...
    def _initialize_monitored_pairs(self) -> None:
        ...
        self._monitored_pairs = [
            (
                ...
                "0xA0b86a33E6c77218f9ceB6ffa4fd4Ee5Bd6a96be",
            ),  # WETH/USDC
            (
                "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2",
        ...
        try:
            ...
            if await self._is_large_swap(tx):
                ...
                opportunities = await self._scan_arbitrage_opportunities(
                    tx.block_number or 0, tx.timestamp
                )
            ...
            self.logger.error(
                f"Error analyzing transaction for arbitrage: {e}")
            return None

    async def on_block(
        self, block_number: int, timestamp: float
    ) -> list[MEVOpportunity]:
        """
            ...
            if opportunities and self.explain:
                self._log_explanation(
                    f"Block {block_number}: Found {
                        len(opportunities)} arbitrage opportunities"
                )
                ...
                for route in routes:
                    if route.net_profit > self.config.min_profit_wei:
                        ...
                        opportunity = MEVOpportunity(
                            ...
                            profit_estimate=Decimal(route.expected_profit)
                            / Decimal(10**18),
        ...
        try:
            ...
            for i in range(len(dex_names)):
                for j in range(i + 1, len(dex_names)):
                    ...
                    route1 = await self._calculate_arbitrage_route(
                        pool1, pool2, token_a
                    )
                    if route1 and route1.net_profit > 0:
                        routes.append(route1)

                    route2 = await self._calculate_arbitrage_route(
                        pool2, pool1, token_a
                    )
    ...
    async def _calculate_arbitrage_route(
        self, buy_pool: DEXPool, sell_pool: DEXPool, token_in: str
    ) -> ArbitrageRoute | None:
        """
        ...
        try:
            ...
            token_out = (buy_pool.token1 if token_in ==
                         buy_pool.token0 else buy_pool.token0)
    ...
    async def _find_optimal_amount(
        ...
        try:
            ...
            # Simple search (in production, use more sophisticated
            # optimization)
            for amount in range(
                int(min_amount), int(max_amount), int(
                    1 * 10**18)):
                try:
                    # Simulate the arbitrage
                    token_out = (
                        buy_pool.token1
                        if token_in == buy_pool.token0
                        else buy_pool.token0
                    )

                    tokens_bought = buy_pool.get_amount_out(amount, token_in)
                    final_amount = sell_pool.get_amount_out(
                        tokens_bought, token_out)
    ...
    async def _build_arbitrage_transaction(
        ...
        try:
            ...
            # that performs the arbitrage atomically, possibly using a custom
            # contract
            ...
            return BundleTransaction(
                ...
                value=(
                    route.optimal_amount
                    if route.token_a == "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"
                    else 0
                ),
