...
class EliteMEVDetector:
    ...
    async def _scan_block_for_mev(
        ...
        try:
            ...
            logger.error(
                f"Error scanning block {block_number} on chain {chain_id}: {e}"
            )
                ...
                if opportunity.net_profit > 0:
                    ...
                    logger.info(
                        f"MEV opportunity detected: {
                            opportunity.id} - ${
                            opportunity.net_profit:.4f} profit"
                    )
    ...
    def get_recent_opportunities(
            self, hours: int = 24) -> list[MEVOpportunity]:
        """Get MEV opportunities from the last N hours."""
        cutoff_time = datetime.utcnow() - timedelta(hours=hours)
        return [opp for opp in self.opportunities if opp.timestamp > cutoff_time]
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        total_profit = sum(
            opp.net_profit for opp in recent_opportunities if opp.net_profit > 0)
