...
class SandwichStrategy(AbstractStrategy):
    ...
    async def on_tx(self, tx: TransactionData) -> MEVOpportunity | None:
        """
        ...
        try:
            ...
            pool = await self._get_pool_info(
                swap_params["path"][0], swap_params["path"][1]
            )
            if not pool:
                return None

            # Calculate sandwich opportunity
            sandwich_opp = await self._calculate_sandwich_opportunity(
                tx, swap_params, pool
            )
            if not sandwich_opp:
                return None
            ...
            if sandwich_opp.expected_profit < self.config.min_profit_wei:
                if self.explain:
                    self._log_explanation(
                        f"Sandwich not profitable: expected profit {
                            sandwich_opp.expected_profit /
                            10**18:.6f} ETH "
                        f"< minimum {
            ...
            if self.explain:
                self._log_explanation(
                    f"Sandwich opportunity found! Victim slippage: {
                        sandwich_opp.victim_slippage:.2f}%, "
                    f"Expected profit: {
                        sandwich_opp.expected_profit / 10**18:.6f} ETH"
                )
    ...
    async def build_bundle(
            self,
            opportunity: MEVOpportunity) -> BundleRequest | None:
        """
        ...
        try:
            ...
            if self.explain:
                self._log_explanation(
                    f"Building sandwich bundle for {
                        opportunity.id}: "
                    f"frontrun {
            ...
            bundle = BundleRequest(
                transactions=[
                    frontrun_tx,
                    backrun_tx,
                ],  # Victim tx will be inserted by execution engine
    ...
    async def _decode_swap_transaction(
    ...
    async def _calculate_sandwich_opportunity(
        self, tx: TransactionData, swap_params: dict[str, Any], pool: UniswapV2Pool
    ) -> SandwichOpportunity | None:
        """
        ...
        try:
            ...
            victim_expected_out = pool.get_amount_out(
                victim_amount_in, pool.token0)
            ...
            # This is a simplified calculation - real implementation would be
            # more sophisticated
            optimal_frontrun = min(
                victim_amount_in // 4,
                10 * 10**18,  # Max 25% of victim amount  # Max 10 ETH
            )

            # Simulate the sandwich
            # 1. Frontrun execution
            pool_after_frontrun = self._simulate_swap(
                pool, optimal_frontrun, pool.token0
            )
            tokens_bought = pool.get_amount_out(optimal_frontrun, pool.token0)

            # 2. Victim execution on modified pool
            pool_after_frontrun.get_amount_out(victim_amount_in, pool.token0)
            pool_after_victim = self._simulate_swap(
                pool_after_frontrun, victim_amount_in, pool.token0
            )

            # 3. Backrun execution
            eth_received = pool_after_victim.get_amount_out(
                tokens_bought, pool.token1)
    ...
    def _simulate_swap(
        self, pool: UniswapV2Pool, amount_in: int, token_in: str
    ) -> UniswapV2Pool:
        """
    ...
    def _calculate_confidence(
            self,
            sandwich_opp: SandwichOpportunity) -> float:
        """
    ...
    async def _build_frontrun_transaction(
        self, sandwich_data: SandwichOpportunity, pool: UniswapV2Pool
    ) -> BundleTransaction | None:
        """
        Build frontrun transaction
    ...
    async def _build_backrun_transaction(
        ...
        try:
            ...
            tokens_to_sell = pool.get_amount_out(
                sandwich_data.frontrun_amount, pool.token0
            )
