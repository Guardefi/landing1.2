...
# ---- Additional Modules ----
try:
    from arbitrage_math import get_base_token_price, get_token
...
def load_abi(filename: str) -> list[dict] | None:
    if filename in _ABI_CACHE:
        return _ABI_CACHE[filename]

    script_dir = os.path.dirname(__file__)
    paths = [os.path.join(script_dir, "abis", filename), filename]
    for fpath in paths:
        ...
        if os.path.exists(abs_path):
            try:
                ...
                with open(abs_path) as f:
                    raw = json.load(f)
                abi = raw.get("abi") if isinstance(
                    raw, dict) and "abi" in raw else raw
...
class MEVBot:
    ...
    def __init__(self):
        ...
        self.flashbots: Flashbots | None = None
        ...
        try:
            ...
            if not artifact_path_rel:
                raise ValueError(
                    "Missing ARB_CONTRACT_ARTIFACT_PATH in config.")
            artifact_abs = os.path.abspath(
                os.path.join(os.path.dirname(__file__), artifact_path_rel)
            )
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.flashloan_contract_instance = self.w3.eth.contract(
                address=self.flashloan_contract_address, abi=self.flashloan_contract_abi)
            ...
            self.performance_tracker.base_token_price_usd_cache = (
                await get_base_token_price(self._base_token, self.w3)
            )
    ...
    async def _call_rust_scanner(self) -> list[dict]:
        ...
        cmd = [rust_exe, "--chain", chain, "--max-hops", str(max_hops)]
    ...
    async def run_main_loop(self):
        ...
        while self.running:
            try:
                ...
                # Possibly do further logic: ML filter, finalize execution,
                # etc.

                if self.liquidation_monitor and self.config.get(
                        "MONITOR_LIQUIDATIONS"):
                    liqs = await self.liquidation_monitor.find_liquidatable_positions()
                    logger.debug(
                        f"Liquidation monitor => {
                            len(liqs)} ops found."
                    )
