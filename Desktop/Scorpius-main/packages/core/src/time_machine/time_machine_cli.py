...
class TimeMachine:
    ...
    def _display_analysis_results(
            self, results: dict[str, Any], output: str) -> None:
    ...
    def _export_data(self, data: dict[str, Any],
                     format_type: str, output: str) -> None:
        """Export data"""
        console.print(
            f"[green]Exported {format_type} data to {output}[/green]")
...
class BlockchainSnapshot:
    ...
    def __init__(self, block_number: int,
                 timestamp: datetime, data: dict[str, Any]):
...
@click.option("--relative", "-r",
              help="Relative time (e.g., '1d ago', '1h ago', 'yesterday')")
...
def bookmark(name, description):
    ...
    time_machine.bookmarks[name] = {
        ...
        or f"Bookmark at block {
            time_machine.current_block}",
    }
...
def analyze(type, address, hash, depth, output):
    ...
    with console.status(f"[bold blue]Analyzing {type}..."):
        try:
            analysis_result = time_machine._perform_analysis(
                type, address, hash, depth)
...
@click.option("--include-state", "-s", is_flag=True,
              help="Include full state in snapshot")
def snapshot(name, include_state):
    ...
    with console.status("[bold blue]Creating snapshot..."):
        try:
            ...
            # Limit snapshots
            if len(
                    time_machine.snapshots) > time_machine.config["max_snapshots"]:
