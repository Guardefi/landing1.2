...
from models.replay_models import Exploit, ReplaySession, SessionStatus
...
async def convert_exploit_to_time_event(exploit: Exploit) -> TimeEvent:
    ...
    if exploit.affected_contracts:
        try:
            ...
            affected_contracts = (
                json.loads(exploit.affected_contracts)
                if isinstance(exploit.affected_contracts, str)
                else exploit.affected_contracts
            )
    ...
    return TimeEvent(
        ...
        severity=(
            ...
            if exploit.financial_impact and exploit.financial_impact > 100
            else (
                SeverityLevel.HIGH
                if exploit.financial_impact and exploit.financial_impact > 10
...
async def convert_replay_session_to_time_event(
        session: ReplaySession) -> TimeEvent:
    """Convert replay session to time event"""
    event_type = (
        EventType.VULNERABILITY
        if session.session_type == "exploit"
        else EventType.TRANSACTION
    )
    ...
    return TimeEvent(
        ...
        description=f"Replay session on {
            session.chain} at block {
            session.fork_block}",
...
async def get_timeline_events(
        time_range: str = Query(
            default="24h",
            description="Time range: 1h, 24h, 7d, 30d, all"),
    ...
    try:
        ...
        critical_count = len(
            [e for e in events if e.severity == SeverityLevel.CRITICAL]
        )
        ...
        stats = TimelineStats(
            ...
            blocks_analyzed=len(
                {e.metadata.get("block_number", 0) for e in events if e.metadata}
            ),
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch timeline events: {
                str(e)}",
        )
    ...
    try:
        ...
        exploit_count_result = await db.execute(
            select(func.count(Exploit.id)).where(
                and_(Exploit.created_at >= start_time, Exploit.created_at <= end_time)
            )
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch timeline stats: {str(e)}"
        )


@router.post("/replay/start")
async def start_time_machine_replay(
    request: TimelineRequest, db: AsyncSession = Depends(get_async_db)
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        for exploit in exploits:
            ...
            replay_session = ReplaySession(
                ...
                parameters=json.dumps(
                    {"time_range": request.time_range, "automated": True}
                ),
                results=json.dumps({}),
        ...
        return {
            "success": True,
            "message": f"Started {
                len(replay_sessions)} replay sessions",
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to start replay: {
                str(e)}",
        )
