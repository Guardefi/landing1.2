...
class CeleryConfig:
    ...
    accept_content: list[str] | None = field(default_factory=lambda: ["json"])
    result_serializer: str = "json"
    timezone: str = "UTC"
    enable_utc: bool = True

    # Task routing
    task_routes: dict[str, dict[str, str]] | None = field(
        default_factory=lambda: {
...
class BlockchainConfig:
    ...
    max_fork_duration_hours: int = int(
        os.getenv("MAX_FORK_DURATION_HOURS", "6"))
...
class LoggingConfig:
    ...
    worker_log_file: str = os.getenv(
        "WORKER_LOG_FILE", "time_machine_worker.log")
    startup_log_file: str = os.getenv(
        "STARTUP_LOG_FILE", "time_machine_startup.log")
...
class SecurityConfig:
    ...
    secret_key: str = os.getenv(
        "SECRET_KEY", "time-machine-secret-key-change-in-production"
    )
...
class CleanupConfig:
    ...
    temp_files_max_age_hours: int = int(
        os.getenv("TEMP_FILES_MAX_AGE_HOURS", "12"))
    snapshot_max_age_hours: int = int(
        os.getenv("SNAPSHOT_MAX_AGE_HOURS", "48"))

    # Cleanup schedules (cron format)
    session_cleanup_schedule: str = os.getenv(
        "SESSION_CLEANUP_SCHEDULE", "0 2 * * *"
    )  # Daily at 2 AM
    fork_cleanup_schedule: str = os.getenv(
        "FORK_CLEANUP_SCHEDULE", "0 */6 * * *"
    )  # Every 6 hours
    temp_cleanup_schedule: str = os.getenv(
        "TEMP_CLEANUP_SCHEDULE", "0 */4 * * *"
    )  # Every 4 hours
...
class TimeMachineConfig:
    ...
    # Feature flags
    enable_websockets: bool = os.getenv(
        "ENABLE_WEBSOCKETS", "false").lower() == "true"
    enable_metrics: bool = os.getenv(
        "ENABLE_METRICS", "false").lower() == "true"
    enable_auth: bool = os.getenv("ENABLE_AUTH", "false").lower() == "true"

    # System limits
    max_concurrent_replays: int = int(
        os.getenv("MAX_CONCURRENT_REPLAYS", "10"))
    max_replay_duration_minutes: int = int(
        os.getenv("MAX_REPLAY_DURATION_MINUTES", "60")
    )
...
def validate_config(config: TimeMachineConfig) -> dict[str, Any]:
    ...
    if config.api.host == "0.0.0.0" and os.getenv(
    ...
    if config.security.secret_key == "time-machine-secret-key-change-in-production":
        issues.append(
            "Secret key is using default value - change in production")
    ...
    if config.max_concurrent_replays > 50:
        warnings.append(
            "High max concurrent replays limit may impact performance")
