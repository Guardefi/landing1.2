...
class QueryRequest(BaseModel):
    ...
    gas_price_min: str | None = None
    gas_price_max: str | None = None
...
class Config:
    ...
    def __init__(self):
        ...
        self.postgres_url = os.getenv(
            "POSTGRES_URL",
            "postgresql://postgres:password@localhost:5432/scorpius")
        ...
        self.archive_interval_hours = int(
            os.getenv("ARCHIVE_INTERVAL_HOURS", "1"))
        self.batch_size = int(os.getenv("BATCH_SIZE", "10000"))
        self.compression_type = os.getenv(
            "COMPRESSION_TYPE", CompressionType.GZIP)
        self.archive_format = os.getenv(
        ...
        self.max_concurrent_archives = int(
            os.getenv("MAX_CONCURRENT_ARCHIVES", "3"))
    ...
    async def upload_file(self, file_path: str, file_data: bytes) -> bool:
        ...
        try:
            self.s3_client.put_object(
                Bucket=self.config.s3_bucket, Key=file_path, Body=file_data
            )
...
class ArchiveManager:
    ...
    async def create_archive(
        self, transactions: list[Transaction], metadata: ArchiveMetadata
    ) -> bool:
        ...
        try:
            ...
            # Compress data
            compressed_data = DataCompressor.compress_data(
                file_data, metadata.compression
            )

            # Upload to S3
            success = await self.s3_manager.upload_file(
                metadata.file_path, compressed_data
            )

            if success:
                logger.info(
                    f"Created archive {
                        metadata.file_path} with {
    ...
    async def load_archive(
            self,
            metadata: ArchiveMetadata) -> list[Transaction] | None:
        ...
        try:
            ...
            # Decompress data
            file_data = DataCompressor.decompress_data(
...
class QueryEngine:
    ...
    async def execute_query(
            self, query: QueryRequest) -> tuple[list[Transaction], int]:
        ...
        logger.info(
            f"Query returned {
                len(paginated_transactions)} transactions from {total_count} total matches")

        return paginated_transactions, total_count

    async def get_relevant_archives(
            self, query: QueryRequest) -> list[ArchiveMetadata]:
    ...
    def apply_filters(
        self, transactions: list[Transaction], query: QueryRequest
    ) -> list[Transaction]:
...
class TimeMachineService:
    ...
    async def initialize(self):
        ...
        self.db_pool = await asyncpg.create_pool(
            self.config.postgres_url, min_size=5, max_size=20
        )
    ...
    async def create_archive(
        ...
        # Generate file path
        file_path = f"chain_{chain_id}/{
            start_time.year}/{
            start_time.month:02d}/{
            start_time.day:02d}/{
                start_time.hour:02d}/transactions.{
        ...
        if success:
            ...
            logger.info(
                f"Archived {
                    len(transactions)} transactions for chain {chain_id}"
            )
