    ...
    def client(self):
        ...
        if self._client is None:
            try:
                ...
                raise RuntimeError(
                    "AWS credentials required for production deployment")
        return self._client
    ...
    def get_secret(
        ...
        try:
            ...
            if error_code == "ResourceNotFoundException":
                if required:
                    logger.error(
                        f"❌ Required secret '{secret_name}' not found in AWS Secrets Manager")
                    raise RuntimeError(
                        f"Required secret '{secret_name}' not found")
                else:
                    logger.warning(
                        f"⚠️ Optional secret '{secret_name}' not found")
                    return None

            elif error_code == "InvalidRequestException":
                logger.error(f"❌ Invalid request for secret '{secret_name}'")
                raise RuntimeError(
                    f"Invalid secret configuration: {secret_name}")
    ...
    def get_secrets_batch(
        ...
        for config_key, secret_config in secret_configs.items():
            secret_value = self.get_secret(
                secret_config.secret_name, secret_config.required
            )
...
class SecureSettings(BaseSettings):
    ...
    def _load_secrets(self):
        ...
        if self.environment == "development":
            logger.info(
                "🔧 Development mode: using environment variables for secrets")
            self._load_dev_secrets()
        else:
            logger.info(
                "🔒 Production mode: loading secrets from AWS Secrets Manager")
    ...
    def _load_aws_secrets(self):
        ...
        secret_configs = {
            "database": SecretConfig(
                secret_name=f"scorpius/{self.environment}/database", required=True
            ),
            "redis": SecretConfig(
                secret_name=f"scorpius/{self.environment}/redis", required=True
            ),
            "jwt": SecretConfig(
                secret_name=f"scorpius/{self.environment}/jwt-secret", required=True
            ),
            "web3": SecretConfig(
                secret_name=f"scorpius/{self.environment}/web3-providers", required=True
            ),
            "coinbase": SecretConfig(
                secret_name=f"scorpius/{self.environment}/coinbase-api", required=False
            ),
        }
