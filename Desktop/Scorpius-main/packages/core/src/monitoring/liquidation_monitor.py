...
from abis import (
    ...
    IPriceOracle_ABI,
    IUiPoolDataProviderV3_ABI,
)

# From arbitrage_math, we use get_token (not getToken) and
# simulate_swap_step, etc.
from arbitrage_math import get_eth_price
...
class LiquidationMonitor:
    ...
    def __init__(self, provider):
        ...
        self.flashloan_contract_address = CONFIG.get(
        ...
        self.aave_config = {
            ...
            "MONITORED_PAIRS": [
                ...
                for p in base.get(
                    "MONITORED_PAIRS", [{"collateral": "WETH", "debt": "USDC"}]
                )
            ],
            "MONITORED_RESERVES": [
    ...
    async def initialize(self):
        ...
        if not provider_addr_str or not data_provider_addr_str:
            logger.error(
                "Aave V3 Liq disabled: missing provider/data provider addresses."
            )
        ...
        try:
            ...
            data_provider_addr = Web3.to_checksum_address(
                data_provider_addr_str)
            addresses_provider = self.w3.eth.contract(
                provider_addr, abi=IPoolAddressesProvider_ABI
            )

            # fetch pool + oracle addresses with retry
            self.aaveV3["poolAddress"] = Web3.to_checksum_address(
                await with_retry_async(
                    lambda: addresses_provider.functions.getPool().call(),
                    "Aave getPool",
                )
            )
            self.aaveV3["oracleAddress"] = Web3.to_checksum_address(
                await with_retry_async(
            ...
            logger.info(
                f"Aave V3 contracts loaded: pool={
                    self.aaveV3['poolAddress']} oracle={
                    self.aaveV3['oracleAddress']}"
            )
    ...
    async def fetch_risky_aave_users(self, config: dict) -> list[str]:
        ...
        hf_threshold_str = Web3.to_hex(
            Web3.to_wei(config.get("HF_THRESHOLD", 1.0) + 0.03, "ether")
        )
        ...
        try:
            ...
            async with Client(
                ...
                logger.info(
                    f"Fetched {
                        len(user_addresses)} risky users from subgraph."
                )
    ...
    async def find_liquidatable_positions(self) -> list[dict]:
        ...
        logger.info(
            f"Checking {
                len(risky_users)} addresses for liquidation potential..."
        )
        ...
        try:
            ...
            for addr in reserves_list:
                ...
                price_tasks[addr_checksum] = with_retry_async(
                    lambda a=addr_checksum: self.aaveV3["oracle"]
                    .functions.getAssetPrice(a)
                    .call(),
            ...
            for k, v in zip(price_tasks.keys(), results, strict=False):
        ...
        if liquidatable_ops:
            logger.info(
                f"Found {
                    len(liquidatable_ops)} potentially profitable Aave liquidations.")
        else:
            logger.info("No profitable Aave V3 liquidations found.")

        logger.debug(
            f"Liquidation scan completed in {
                time.monotonic() -
                start_time:.2f}s"
        )
