...
from abis import IAaveOracle_ABI, IChainlinkAggregatorV3_ABI, IPoolAddressesProvider_ABI
from arbitrage_math import get_token  # Needs helpers
...
async def get_price_from_chainlink(
    asset_symbol: str, w3: Web3
) -> dict[str, Any] | None:
    """
    Helper function to get price from Chainlink oracle.
    Returns dict with 'price' and 'decimals' or None on failure.
    """
    # This is a placeholder implementation - implement actual Chainlink price
    # fetching
    try:
        # Mock implementation - replace with actual Chainlink oracle calls
        logger.warning(f"Mock Chainlink price fetch for {asset_symbol}")
        return {
            # Mock price in 8 decimals (like $1800 for ETH)
            "price": 1800000000,
            "decimals": 8,
        }
    except Exception as e:
        logger.error(f"Failed to get Chainlink price for {asset_symbol}: {e}")
        return None
...
class OracleMonitor:
    ...
    def __init__(self, provider):  # Needs the base async provider object
        ...
        # Stores { symbol: { contract: ContractInstance, decimals: int,
        # address: str } }
        ...
        if not self.enabled:
            logger.info("OracleMonitor disabled by configuration.")
            return
        self._validate_config()  # Validate sub-config

    def _validate_config(self):
        # ... (implement validation logic similar to JS version) ...
        if not self.config.get("monitoredAssets"):
            raise ValueError("OracleMon: monitoredAssets missing")
        if not self.config.get("cexToCheck"):
            raise ValueError("OracleMon: cexToCheck missing")
        # ... etc ...
        pass  # Add checks
    ...
    async def initialize(self):
        ...
        for exchange_id in self.config.get("cexToCheck", []):
            try:
                ...
                # Load API keys if provided for potential private endpoints /
                # higher rate limits
                ...
                # Add custom headers if needed: options['options'] =
                # {'headers': {'X-Custom': 'value'}}
                client = exchange_class(options)
                self.cex_clients[exchange_id] = client
                # Async load_markets with retry
                await with_retry_async(
                    lambda: client.load_markets(reload=True),
                    f"CCXT loadMarkets {exchange_id}",
                    {"retries": 2},
                )
                logger.info(
                    f"OracleMonitor: CCXT async client for {exchange_id} initialized.")
            except AttributeError:
                logger.error(
                    f"OracleMonitor: CCXT exchange '{exchange_id}' not found.")
            except Exception as e:
                logger.error(
                    f"OracleMonitor: Failed init CCXT {exchange_id}: {e}"
                )  # Error logged by retry

        if not self.cex_clients:
            logger.error("OracleMon: No CEX clients init. Disabling.")
            self.enabled = False
            return
        ...
        try:
            if oracle_type == "CHAINLINK":
                ...
                for asset_symbol in self.config.get("monitoredAssets", []):
                    ...
                    if (
                        feed_config
                        and feed_config.get("address")
                        and feed_config.get("quote") == "USD"
                    ):
                        try:
                            address = Web3.to_checksum_address(
                                feed_config["address"])
                            contract = self.w3.eth.contract(
                                address=address, abi=IChainlinkAggregatorV3_ABI
                            )
                            decimals = await with_retry_async(
                                lambda: contract.functions.decimals().call(),
                                f"CL decimals {asset_symbol}",
                            )
                            self.chainlink_feeds[asset_symbol] = {
                                "contract": contract,
                                "decimals": int(decimals),
                                "address": address,
                            }
                            logger.info(
                                f"OracleMonitor: Chainlink {asset_symbol} (USD/{decimals}dec) -> {address}")
                        except Exception as e:
                            logger.error(
                                f"Failed init Chainlink feed for {asset_symbol}: {e}")
                    # else: logger.debug(f"No Chainlink USD feed for
                    # {asset_symbol}")

            elif oracle_type == "AAVE":
                provider_addr = oracle_source.get(
                    "poolProviderAddress"
                )  # Required for Aave type
                if not provider_addr:
                    raise ValueError(
                        "Aave oracle source needs poolProviderAddress")
                provider_addr_cs = Web3.to_checksum_address(provider_addr)
                add_prov_contract = self.w3.eth.contract(
                    address=provider_addr_cs, abi=IPoolAddressesProvider_ABI
                )
                oracle_address = await with_retry_async(
                    lambda: add_prov_contract.functions.getPriceOracle().call(),
                    "Aave getPriceOracle",
                )
                self.aave_oracle_contract = self.w3.eth.contract(
                    address=Web3.to_checksum_address(oracle_address),
                    abi=IAaveOracle_ABI,
                )
                logger.info(
                    f"OracleMonitor: Aave oracle at {oracle_address} (Prices vs ETH)")
                # We also need an ETH/USD Chainlink feed for AAVE type to work
                if not self.chainlink_feeds.get(
                        "ETH") and not self.chainlink_feeds.get("WETH"):
                    raise ValueError(
                        "Aave oracle requires an ETH or WETH Chainlink USD feed configured for conversions."
                    )
            ...
            logger.error(f"OracleMon On-chain setup failed: {e}")
            self.enabled = False
            return
        ...
        logger.info(
            f"OracleMonitor initialization complete in {
                time.monotonic() -
    ...
    async def start_monitoring(self):
        ...
        if not self.enabled or not self.is_initialized or self.is_monitoring:
            return
        logger.info(
            f"OracleMonitor starting monitoring interval ({
                self.config.get(
        ...
        async def monitor_loop():
            ...
            while self.is_monitoring:
                try:
                    ...
                    logger.info("Oracle monitor loop cancelled.")
                    break
                except Exception:
                    logger.exception("Error in oracle monitor loop:")
    ...
    async def stop_monitoring(self):
        ...
        if self.monitor_task and not self.monitor_task.done():
            ...
            try:
                ...
                logger.info("Oracle monitor task cancelled.")
        ...
        for exchange_id, client in self.cex_clients.items():
            try:
                ...
                logger.error(f"Error closing CCXT client {exchange_id}: {e}")
    ...
    async def _fetch_on_chain_prices(self):
        ...
        try:
            ...
            if (
                oracle_type == "AAVE"
                or "ETH" in self.config.get("monitoredAssets", [])
                or "WETH" in self.config.get("monitoredAssets", [])
            ):
                eth_feed = self.chainlink_feeds.get(
                    "ETH") or self.chainlink_feeds.get("WETH")
                if eth_feed:
                    eth_price_res = await get_price_from_chainlink(
                        "ETH", self.w3
                    )  # Use shared helper
                    if eth_price_res:
                        eth_price_data = {
                            "price": eth_price_res["price"],
                            "decimals": eth_price_res["decimals"],
                        }  # price is BN(18dec)
                        # Store ETH price directly if monitored
                        for sym in ["ETH", "WETH"]:
                            if sym in self.latest_prices:
                                self.latest_prices[sym].update(
                                    {
                                        "onChainScaledUsd": eth_price_data["price"],
                                        "lastChainUpdate": time.time(),
                                    }
                                )
                elif oracle_type == "AAVE":
                    logger.error("Cannot fetch Aave prices without ETH feed.")
                    return

            # Fetch other assets
            for asset_symbol in self.config.get("monitoredAssets", []):
                if asset_symbol.upper() in ["ETH", "WETH"]:
                ...
                try:
                    ...
                    if oracle_type == "CHAINLINK":
                        price_data = await get_price_from_chainlink(
                            asset_symbol, self.w3
                        )  # Use helper
                        if price_data:
                        ...
                        oracle_type == "AAVE"
                        and self.aave_oracle_contract
                        and eth_price_data
                    ):
                        token = get_token(asset_symbol)
                        if token:
                            price_vs_eth = await with_retry_async(
                                lambda: self.aave_oracle_contract.functions.getAssetPrice(
                                    token.address
                                ).call(),
                                f"Aave getAssetPrice {asset_symbol}",
                            )
                            # Convert to USD (scaled 18 dec)
                            price_scaled_usd = (
                                price_vs_eth * eth_price_data["price"]
                            ) // (
                                10**18
                            )  # ETH price is already 18 dec
                            timestamp = time.time()

                    if price_scaled_usd is not None:
                        self.latest_prices[asset_symbol][
                            "onChainScaledUsd"
                        ] = price_scaled_usd
                    ...
                    logger.warning(
                        f"Failed fetching on-chain price for {asset_symbol}: {e}")

        except Exception as e:
            logger.error(f"Error in _fetchOnChainPrices: {e}", exc_info=True)
    ...
    async def _fetch_cex_prices(self):
        ...
        for asset_symbol in self.config.get("monitoredAssets", []):
            ...
            base_quote = "USD"
            alt_quote = "USDT"
            # Standard pairs to check
            market_symbol = f"{asset_symbol}/{base_quote}"
            alt_market = f"{asset_symbol}/{alt_quote}"

            for ex_id, client in self.cex_clients.items():
                symbol_to_fetch = None
                if market_symbol in client.markets:
                    symbol_to_fetch = market_symbol
                elif alt_market in client.markets:
                    symbol_to_fetch = alt_market

                if symbol_to_fetch:
                    tasks.append(
                        with_retry_async(
                            lambda: client.fetch_ticker(symbol_to_fetch),
                            f"CCXT fetchTicker {ex_id} {symbol_to_fetch}",
                            custom_options={"retries": 1},
                        )
                    )
                # else: logger.debug(f"Market {market_symbol} or {alt_market}
                # not found on {ex_id}")
            ...
            weighted_sum = 0
            sources = 0
            latest_update = self.latest_prices[asset_symbol]["lastCexUpdate"] or 0
            stale_threshold_ms = (
                self.config.get("stalePriceThresholdSeconds", 90) * 1000
            )
            ...
            for res in results:
                if not isinstance(res, Exception) and res:
                    try:
                        price = res.get("vwap") or res.get(
                            "last")  # Prefer VWAP
                        volume = res.get(
                            "quoteVolume"
                        )  # Use quote volume for weighting
                        ...
                        if (
                            ...
                            and volume
                            and timestamp
                            and (now_ms - timestamp < stale_threshold_ms)
                        ):
                        ...
                        # else: logger.debug(f"Stale/invalid ticker data
                        # ignored: {res}")
                    except Exception as proc_err:
                        logger.warning(
                            f"Error processing CEX ticker {res}: {proc_err}")
                # elif isinstance(res, Exception): logger.warning(f"CEX fetch
                # failed: {res}") # Error logged by retry

            if sources > 0 and total_volume > 0:
                final_price = weighted_sum / total_volume
                self.latest_prices[asset_symbol]["cexUsd"] = final_price
                self.latest_prices[asset_symbol]["lastCexUpdate"] = (
                    latest_update / 1000.0
                )  # Store as seconds
    ...
    def _compare_all_prices(self):
        ...
        for asset_symbol in self.config.get("monitoredAssets", []):
            ...
            on_chain_price_wei = prices.get(
                "onChainScaledUsd"
            )  # Already scaled to 18 dec
            ...
            if (
                is_chain_stale
                or is_cex_stale
                or on_chain_price_wei is None
                or cex_price_float is None
            ):
                ...
                try:
                    # Compare prices (both effectively USD, onChainScaledUsd is
                    # 18 dec wei)
                    on_chain_price_num = float(
                        Web3.from_wei(on_chain_price_wei, "ether")
                    )
                    cex_price_num = float(cex_price_float)
                    if (
                        on_chain_price_num > 1e-9
                    ):  # Avoid division by zero or tiny numbers
                        diff = abs(1.0 - (cex_price_num / on_chain_price_num))
                        should_be_deviating = diff > self.config.get(
            ...
            if should_be_deviating != self.deviation_flags.get(
                    flag_key, False):
                self.deviation_flags[flag_key] = should_be_deviating
                if should_be_deviating:
                    logger.warning(
                        f"ORACLE DEVIATION DETECTED for {asset_symbol}!")
                else:
                    logger.info(
                        f"Oracle deviation resolved/stale for {asset_symbol}.")
    ...
    def get_deviation_status(self) -> dict[str, bool]:
        ...
        def is_asset_deviating(self, asset_symbol: str) -> bool:
            ...
            return self.deviation_flags.get(
                asset_symbol.upper(), False
