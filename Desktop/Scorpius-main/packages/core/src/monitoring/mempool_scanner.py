...
from ..core.types import NetworkConfig, TransactionData
...
class MempoolScanner:
    ...
    def __init__(
        ...
        self._tx_queue: asyncio.Queue[TransactionData] = asyncio.Queue(
            maxsize=queue_size
        )
    ...
    async def stop(self) -> None:
        ...
        for network_name, ws in self._connections.items():
            try:
                ...
                self.logger.error(
                    f"Error closing WebSocket for {network_name}: {e}")
    ...
    async def _maintain_websocket_connection(
        self, network_name: str, network_config: NetworkConfig
    ) -> None:
        """
        ...
        while self._running:
            try:
                self.logger.info(
                    f"Connecting to {network_name} WebSocket: {
                ...
                async with websockets.connect(
                    ...
                    self.logger.info(
                        f"Subscribed to {network_name} pending transactions"
                    )
                    ...
                    async for message in websocket:
                        ...
                        await self._handle_websocket_message(
                            network_name, network_config, message
                        )

            except (ConnectionClosed, ConnectionClosedError) as e:
                self.logger.warning(
                    f"WebSocket connection lost for {network_name}: {e}"
                )
                ...
                if network_name in self._connections:
                    ...
                    self.metrics.connections_active = max(
                        0, self.metrics.connections_active - 1
                    )

            # Wait before reconnecting
            if self._running:
                self.logger.info(
                    f"Reconnecting to {network_name} in 5 seconds...")
                await asyncio.sleep(5)

    async def _handle_websocket_message(
        self, network_name: str, network_config: NetworkConfig, message: str
    ) -> None:
        """
    ...
    async def _fetch_transaction(
        self, network_name: str, network_config: NetworkConfig, tx_hash: str
    ) -> None:
        """
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    network_config.rpc_url, json=rpc_data
                ) as response:
                    ...
                    if "result" in result and result["result"]:
                        ...
                        transaction = self._parse_transaction(
                            network_config.chain_id, tx_data
                        )
                        ...
                        if transaction:
                            ...
                            if self.filter_engine.passes_filters(transaction):
                                ...
                                try:
                                    ...
                                    self.backpressure.update_queue_size(
                                        self._tx_queue.qsize()
                                    )
                                except asyncio.QueueFull:
                                    self.logger.warning(
                                        "Transaction queue full - dropping transaction"
                                    )
                                    self.metrics.backpressure_events += 1
    ...
    def _parse_transaction(
        self, chain_id: int, tx_data: dict[str, Any]
    ...
    async def _process_transactions(self) -> None:
        ...
        while self._running:
            try:
                ...
                self._tx_timestamps = [
                    ts for ts in self._tx_timestamps if ts > cutoff_time
                ]
    ...
    async def _process_single_transaction(
            self, transaction: TransactionData) -> None:
        """
    ...
    async def _update_metrics(self) -> None:
        ...
        while self._running:
            try:
                ...
                current_time = time.time()
                recent_timestamps = [
                    ts for ts in self._tx_timestamps if ts > current_time - 1.0
                ]
                ...
                if self._processing_times:
                    self.metrics.avg_processing_time_ms = sum(
        ...
        while self._running:
            try:
                ...
                if self.metrics.avg_processing_time_ms > 50:  # 50ms threshold
                    self.logger.warning(
                        f"High transaction processing time: {
                            self.metrics.avg_processing_time_ms:.2f}ms"
                    )
                ...
                if self.metrics.connections_active < len(self.networks):
                    missing = len(self.networks) - \
                        self.metrics.connections_active
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "queue_utilization": (
                self._tx_queue.qsize() / self.queue_size) * 100,
        }
