        ...
        if metric.metric_type == MetricType.COUNTER:
            if metric.name not in self.counters:
                self.counters[metric.name] = Counter(
                    metric.name,
                    metric.description or metric.name,
                    labels,
            ...
            self.histograms[metric.name].labels(
                *label_values).observe(metric.value)
...
class AlertManager:
    ...
    def __init__(self):
        ...
        self.alert_rules: list[Callable[..., Any]] = []
        self.notification_channels: list[Callable[..., Any]] = []

    def add_alert_rule(self, rule: Callable[..., Any]):
        """Add an alert rule."""
        self.alert_rules.append(rule)

    def add_notification_channel(self, channel: Callable[..., Any]):
...
class AdvancedMonitoringDashboard:
    ...
    def _setup_default_alert_rules(self):
        ...
        async def high_cpu_rule(
                metrics: dict[str, list[Metric]]) -> list[Alert]:
    ...
    async def _update_dashboard_data(self):
        ...
        for metric_name, metric_list in self.metrics_collector.metrics.items():
            if metric_list:
                # Last 10 values
                recent_metrics[metric_name] = metric_list[-10:]
    ...
    async def _calculate_health_score(self) -> float:
        ...
        if memory_metrics:
            avg_memory = sum(
                m.value for m in memory_metrics) / len(memory_metrics)
            if avg_memory > 85:
                score -= 25
            elif avg_memory > 70:
                score -= 15
    ...
    async def get_metrics_export(self, format_type: str = "prometheus") -> str:
        ...
        if format_type == "prometheus":
            ...
            raise ValueError(
                f"Unsupported export format: {format_type}") from None
...
_dashboard_instance: AdvancedMonitoringDashboard | None = None
...
def get_dashboard_instance() -> AdvancedMonitoringDashboard | None:
