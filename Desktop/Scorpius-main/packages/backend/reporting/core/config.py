...
class Settings(BaseSettings):
    ...
    # QLDB Configuration
    QLDB_LEDGER_NAME: str = Field(
        default="scorpius-audit",
        env="QLDB_LEDGER_NAME")
    QLDB_TABLE_NAME: str = Field(
        default="report_hashes",
        env="QLDB_TABLE_NAME")

    # Signature Configuration
    SIGNATURE_CERT_PATH: str = Field(
        default="./certs/signing.crt", env="SIGNATURE_CERT_PATH"
    )
    SIGNATURE_KEY_PATH: str = Field(
        default="./certs/signing.key", env="SIGNATURE_KEY_PATH"
    )
    SIGNATURE_ALGORITHM: str = Field(
        default="RS256", env="SIGNATURE_ALGORITHM")

    # Storage Configuration
    REPORTS_BASE_PATH: str = Field(
        default="./reports",
        env="REPORTS_BASE_PATH")
    ...
    RATE_LIMIT_WINDOW: int = Field(
        default=3600, env="RATE_LIMIT_WINDOW")  # 1 hour
    ...
    AUDIT_SERVICE_URL: str = Field(
        default="http://localhost:8003", env="AUDIT_SERVICE_URL"
    )
    AUTH_SERVICE_URL: str = Field(
        default="http://localhost:8001", env="AUTH_SERVICE_URL"
    )

    # PDF Generation
    PDF_TEMPLATE_PATH: str = Field(
        default="./templates",
        env="PDF_TEMPLATE_PATH")
    PDF_FONTS_PATH: str = Field(default="./fonts", env="PDF_FONTS_PATH")
    PDF_WATERMARK_ENABLED: bool = Field(
        default=True, env="PDF_WATERMARK_ENABLED")

    # SARIF Generation
    SARIF_SCHEMA_VERSION: str = Field(
        default="2.1.0", env="SARIF_SCHEMA_VERSION")
    SARIF_VALIDATION_ENABLED: bool = Field(
        default=True, env="SARIF_VALIDATION_ENABLED")
    ...
    def validate_signature_algorithm(cls, v):
        ...
        if v not in valid_algorithms:
            raise ValueError(
                f"SIGNATURE_ALGORITHM must be one of {valid_algorithms}")
        return v
