...
class PDFReportRequest(ReportRequest):
    ...
    template: Optional[str] = Field(
        default="default", description="PDF template to use"
    )
    include_signature: bool = Field(
        default=True, description="Include cryptographic signature"
    )
    watermark: Optional[str] = Field(
        None, description="Optional watermark text")
...
class ScanResult(BaseModel):
    ...
    locations: List[Dict[str, Any]] = Field(
        default_factory=list, description="Finding locations"
    )
...
class MetricsResponse(BaseModel):
    ...
    reports_generated_total: int = Field(
        default=0, description="Total reports generated"
    )
    signatures_created_total: int = Field(
        default=0, description="Total signatures created"
    )
    audit_entries_total: int = Field(
        default=0, description="Total audit entries")
    service_uptime_seconds: float = Field(
        default=0.0, description="Service uptime in seconds"
    )
    active_reports: int = Field(
        default=0, description="Currently processing reports")
...
class BatchReportRequest(BaseModel):
    ...
    reports: List[Dict[str, Any]
                  ] = Field(..., description="List of report requests")
...
class BatchReportResponse(BaseModel):
    ...
    report_ids: List[str] = Field(...,
                                  description="Individual report identifiers")
