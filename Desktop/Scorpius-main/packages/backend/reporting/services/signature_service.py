...
class SignatureService:
    ...
    async def initialize(self):
        ...
        try:
            if not CRYPTOGRAPHY_AVAILABLE:
                logger.warning(
                    "Cryptography not available, using mock signatures")
    ...
    async def _generate_key_pair(self):
        ...
        try:
            ...
            os.makedirs(
                os.path.dirname(
                    self.settings.SIGNATURE_KEY_PATH),
                exist_ok=True)
    ...
    async def _generate_self_signed_cert(self):
        ...
        try:
            ...
            os.makedirs(
                os.path.dirname(
                    self.settings.SIGNATURE_CERT_PATH),
                exist_ok=True)
    ...
    async def sign_pdf(
        ...
        try:
            ...
            signature_payload = json.dumps(
                signature_metadata, sort_keys=True).encode("utf-8")
    ...
    async def _mock_verify_signature(
            self, signed_content: Dict[str, Any]) -> bool:
