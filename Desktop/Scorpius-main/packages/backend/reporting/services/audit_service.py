...
class AuditService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if ASYNCPG_AVAILABLE and self.settings.DATABASE_URL.startswith(
                "postgresql"
            ):
    ...
    async def _check_security_alerts(self, audit_entry: Dict[str, Any]):
        ...
        try:
            ...
            if audit_entry["risk_score"] >= 80:
                logger.warning(
                    f"High-risk audit event: {audit_entry['event_type']} by {audit_entry['user_id']}"
                )

            # Check for suspicious patterns
            if audit_entry["event_type"] in [
                "report_download",
    ...
    async def _check_download_patterns(self, audit_entry: Dict[str, Any]):
        ...
        try:
            ...
            if download_count > 10:
                logger.warning(
                    f"Suspicious download pattern: {download_count} downloads by user {user_id}")
    ...
    async def get_audit_events(
        ...
        """
        if not self.db_pool:
            return await self._get_audit_events_from_file(
                user_id, event_type, start_time, end_time, limit
            )
    ...
    async def get_audit_summary(
        ...
        try:
            ...
            for event in events:
                ...
                daily_counts[str(event_date)] = daily_counts.get(
                    str(event_date), 0) + 1
