...
class SARIFGenerator:
    ...
    async def generate_report(
        ...
        try:
            ...
            sarif_report = {
                ...
                "runs": [await self._create_run(scan_results, tool_info, run_metadata)],
            }
            ...
            logger.info(
                f"Generated SARIF report with {
                    len(scan_results)} results"
            )
            return sarif_report
    ...
    async def _create_invocation(
        self, run_metadata: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        if run_metadata:
            ...
            if "environment_variables" in run_metadata:
                invocation["environmentVariables"] = run_metadata[
                ...
                rule = {
                    ...
                    "fullDescription": {
                        "text": result.get(
                            ...
                            result.get("rule_description", f"Rule {rule_id}"),
                        )
                    },
    ...
    async def _create_results(
        ...
        for result in scan_results:
            sarif_result = {
                ...
                "level": self._map_level(result.get("level", "warning")),
                "message": {"text": result.get("message", "No message provided")},
            ...
            if "code_flows" in result:
                sarif_result["codeFlows"] = await self._create_code_flows(
                    result["code_flows"]
                )
        ...
        for location in locations:
            ...
            if "context" in location:
                sarif_location["physicalLocation"]["contextRegion"] = {
                    ...
                    "snippet": {"text": location["context"].get("text", "")},
                }
    ...
    async def _create_fixes(
            self, fixes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
                ...
                for change in fix["changes"]:
                    ...
                    if "replacements" in change:
                        for replacement in change["replacements"]:
                            sarif_replacement = {
                                "deletedRegion": {
                                    "startLine": replacement.get(
                                        "start_line", 1), "endLine": replacement.get(
                                        "end_line", 1), "startColumn": replacement.get(
                                        "start_column", 1), "endColumn": replacement.get(
                                        "end_column", 1), }, "insertedContent": {
                                    "text": replacement.get(
                                        "new_text", "")}, }
                            artifact_change["replacements"].append(
                                sarif_replacement)
    ...
    async def _create_code_flows(
        self, code_flows: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
                ...
                for thread_flow in flow["thread_flows"]:
                    ...
                    if "locations" in thread_flow:
                        for location in thread_flow["locations"]:
                            ...
                            sarif_thread_flow["locations"].append(
                                sarif_location)
    ...
    async def _validate_sarif(self, sarif_report: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Validate runs
            if (
                not isinstance(sarif_report["runs"], list)
                or len(sarif_report["runs"]) == 0
            ):
                raise SARIFGenerationError(
                    "SARIF report must contain at least one run")

            for run in sarif_report["runs"]:
                if "tool" not in run:
                    raise SARIFGenerationError(
                        "Run must contain tool information")
