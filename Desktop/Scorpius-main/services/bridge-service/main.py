...
BRIDGE_TRANSACTIONS = Counter(
    ...
    ["chain_from", "chain_to", "status"],
)
TRANSACTION_DURATION = Histogram(
    "bridge_transaction_duration_seconds", "Transaction processing time"
)
LIQUIDITY_POOL_SIZE = Gauge(
    "bridge_liquidity_pool_size", "Liquidity pool size", ["token", "chain"]
)
...
class BridgeTransaction(BaseModel):
    ...
    deadline: Optional[int] = Field(
        default=None, description="Transaction deadline")
...
class BridgeStatus(BaseModel):
    ...
    confirmations: int = Field(
        default=0, description="Number of confirmations")
...
async def initiate_bridge_transaction(
    transaction: BridgeTransaction, background_tasks: BackgroundTasks
):
    ...
    try:
        ...
        if transaction.from_chain == transaction.to_chain:
            raise HTTPException(
                status_code=400,
                detail="Source and destination chains cannot be the same",
            )

        # Generate transaction ID if not provided
        if not transaction.transaction_id:
            transaction.transaction_id = f"bridge_{
                int(
                    time.time())}_{
                hash(
        ...
        background_tasks.add_task(
            process_bridge_transaction, transaction.transaction_id
        )
        ...
        logger.info(
            f"Bridge transaction initiated: {
...
async def get_bridge_status(transaction_id: str):
    ...
    try:
        ...
        transaction_data = await redis_client.hgetall(
            f"bridge:transaction:{transaction_id}"
        )
        ...
        return BridgeStatus(
            ...
            updated_at=datetime.fromisoformat(
                transaction_data.get("updated_at", transaction_data.get("created_at"))
            ),
...
async def cancel_bridge_transaction(transaction_id: str):
    ...
    try:
        ...
        if status not in ["pending", "processing"]:
            raise HTTPException(
                status_code=400,
                detail=f"Cannot cancel transaction with status: {status}",
            )

        # Update transaction status
        await redis_client.hset(
            f"bridge:transaction:{transaction_id}", "status", "cancelled"
        )
        await redis_client.hset(
            f"bridge:transaction:{transaction_id}",
            "updated_at",
        ...
        await publish_event(
            ...
            {"transaction_id": transaction_id, "reason": "user_requested"},
        )
...
async def process_bridge_transaction(transaction_id: str):
    ...
    try:
        ...
        await redis_client.hset(
            f"bridge:transaction:{transaction_id}", "status", "processing"
        )
        await redis_client.hset(
            f"bridge:transaction:{transaction_id}",
            "updated_at",
        ...
        # Publish event
        await publish_event(
            "transaction_processing", {"transaction_id": transaction_id}
        )
        ...
        if random.random() < 0.9:
            await redis_client.hset(
                f"bridge:transaction:{transaction_id}", "status", "completed"
            )
            await redis_client.hset(
                f"bridge:transaction:{transaction_id}", "confirmations", "12"
            )
            ...
            await publish_event(
                "transaction_completed", {"transaction_id": transaction_id}
            )
            logger.info(f"Bridge transaction completed: {transaction_id}")
        else:
            await redis_client.hset(
                f"bridge:transaction:{transaction_id}", "status", "failed"
            )
            await redis_client.hset(
                f"bridge:transaction:{transaction_id}",
                "error",
            ...
            await publish_event(
                "transaction_failed",
                {"transaction_id": transaction_id, "error": "Insufficient liquidity"},
            )
        ...
        logger.error(
            f"Error processing bridge transaction {transaction_id}: {e}")
