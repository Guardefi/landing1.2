...
export class WebSocketManager {
  ...
  subscribeToMEV(callbacks: {
    ...
    if (callbacks.onProfitUpdate) {
      const unsub = wsService.subscribe('mev_profit_updates', callbacks.onProfitUpdate);
      unsubscribers.push(unsub);
    }
  ...
  subscribeToScanner(callbacks: {
    ...
    if (callbacks.onScanProgress) {
      const unsub = wsService.subscribe('scan_progress', callbacks.onScanProgress);
      unsubscribers.push(unsub);
    }
  ...
  subscribeToThreats(callbacks: {
    ...
    if (callbacks.onZeroDayUpdates) {
      const unsub = wsService.subscribe('zero_day_updates', callbacks.onZeroDayUpdates);
      unsubscribers.push(unsub);
    }
  ...
  subscribeToSystemHealth(callbacks: {
    ...
    if (callbacks.onSystemAlerts) {
      const unsub = wsService.subscribe('system_alerts', callbacks.onSystemAlerts);
      unsubscribers.push(unsub);
    }
  ...
  subscribeToModule(moduleName: string, callbacks: Record<string, Function>) {
    ...
    if (subscribe) {
      ...
      console.warn(`No WebSocket subscription available for module: ${moduleName}`);
      return () => {};
    }
  }
