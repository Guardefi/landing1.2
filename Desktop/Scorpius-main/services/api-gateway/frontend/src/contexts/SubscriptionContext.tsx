...
interface SubscriptionContextType {
  ...
  getTierInfo: (tier: SubscriptionTier) => (typeof TIER_PRICING)[SubscriptionTier];
...
export const SubscriptionProvider: React.FC<SubscriptionProviderProps> = ({
      ...
      if (storedSubscription) {
        ...
        if (subData.expiresAt && !subData.isLifetime) {
          ...
          if (now > expires) {
            ...
            localStorage.setItem('scorpius_subscription', JSON.stringify(expiredSub));
            return;
          }
        }
  ...
  const canUseFeature = (feature: string): { allowed: boolean; reason?: string } => {
    ...
    if (
      ...
      if ((subscription.reportsRemaining || 0) <= 0) {
        return {
          allowed: false,
          reason: 'No reports remaining. Purchase additional reports or upgrade.',
        };
      }
    }
  ...
  const upgradeToTier = async (tier: SubscriptionTier): Promise<boolean> => {
    try {
      ...
      localStorage.setItem('scorpius_subscription', JSON.stringify(newSubscription));
