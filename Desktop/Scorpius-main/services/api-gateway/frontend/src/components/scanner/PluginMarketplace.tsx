...
interface MarketplacePlugin {
  ...
  category: 'security' | 'performance' | 'compliance' | 'defi' | 'nft' | 'governance';
  tier: UserTier;
  price: {
...
export function PluginMarketplace({
  ...
  const installPlugin = async (plugin: MarketplacePlugin) => {
    ...
    try {
      ...
      if (response.success) {
        setPlugins(prev =>
          prev.map(p =>
            p.id === plugin.id ? { ...p, installed: true, installing: false } : p,
          ),
        );
  ...
  const canAccessPlugin = (plugin: MarketplacePlugin): boolean => {
    ...
    const tierHierarchy: UserTier[] = ['community', 'starter', 'pro', 'enterprise'];
                          ...
                          <CardHeader className="pb-3">
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                ...
                                <div className="flex items-center gap-2 mb-2">
                                  ...
                                  {plugin.price.type === 'free' ? (
                                    ...
                                    <Badge variant="outline" className="text-xs">
                                      ${plugin.price.amount}/{plugin.price.period}
                                    </Badge>
                                  )}
                                </div>
                              </div>
                              <Avatar className="h-8 w-8">
                                ...
                                <AvatarFallback>
                                  {plugin.author.name.substring(0, 2).toUpperCase()}
                                </AvatarFallback>
                              </Avatar>
                            </div>
                          </CardHeader>
            ...
            <DialogHeader>
              <div className="flex items-start justify-between">
                ...
                {selectedPlugin.installed ? (
                  ...
                  <Button
                    disabled={
                      !canAccessPlugin(selectedPlugin) || selectedPlugin.installing
                    }
              ...
              <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-sm text-gray-600 ml-1">
                        {selectedPlugin.rating.average} ({selectedPlugin.rating.count}{' '}
                        reviews)
                      </span>
                    </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p>{new Date(selectedPlugin.lastUpdated).toLocaleDateString()}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                {selectedPlugin.website && (
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => window.open(selectedPlugin.website, '_blank')}
                  >
