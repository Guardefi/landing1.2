...
const MempoolMonitor = () => {
  ...
  const generateMockTransaction = useCallback((): Transaction => {
    ...
    return {
      ...
      method: ['transfer', 'swap', 'approve', 'withdraw', 'deposit', 'mint', 'burn'][
  ...
  const generateThreatDetection = useCallback(
    (contractAddress: string, txHash: string): ThreatDetection => {
      ...
      const descriptions = {
        reentrancy: 'Potential reentrancy attack detected in contract execution',
        front_run: 'Front-running attempt detected based on gas price and timing',
        sandwich: 'Sandwich attack pattern identified in transaction sequence',
        honeypot: 'Honeypot behavior detected - contract may prevent withdrawals',
        suspicious_volume: 'Unusually large transaction volume detected',
        gas_manipulation: 'Gas price manipulation detected for transaction ordering',
      };
    ...
    const interval = setInterval(
      () => {
        ...
        if (newTx.isTracked && Math.random() > 0.8) {
          const threat = generateThreatDetection(newTx.contractAddress!, newTx.hash);
      ...
      >
        ...
        >
          ...
          {trackedContracts.length > 0 ? (
            ...
            >
              No contracts being tracked. Add contract addresses above to monitor their
              transactions and detect threats.
            </div>
          )}
        </motion.div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {threatDetections.slice(0, 5).map((threat, index) => (
                <motion.div
                  ...
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.3, delay: 0.1 * index }}
                  className={threat.severity === 'critical' ? 'threat-alert' : ''}
                  style={{
                    padding: '16px',
                    backgroundColor: 'rgba(0, 0, 0, 0.4)',
                    borderRadius: '12px',
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      <span>Contract: {formatAddress(threat.contractAddress)}</span>
                  ...
                  >
                    ...
                    <td style={{ padding: '12px', fontSize: '11px' }}>
                      <div style={{ color: '#cccccc', fontFamily: 'monospace' }}>
                        ...
                        <div style={{ color: '#999999' }}>→ {formatAddress(tx.to)}</div>
                      </div>
                    </td>
                    <td
                      style={{
