...
REQUEST_COUNT = Counter(
    "gateway_requests_total", "Total requests", ["method", "service", "status"]
)
REQUEST_DURATION = Histogram(
    "gateway_request_duration_seconds", "Request duration", ["service"]
)
ACTIVE_WEBSOCKETS = Gauge(
    "gateway_active_websockets",
    "Active WebSocket connections")
CIRCUIT_BREAKER_STATE = Gauge(
    "gateway_circuit_breaker_state", "Circuit breaker state", ["service"]
)
RATE_LIMIT_HITS = Counter(
    "gateway_rate_limit_hits_total", "Rate limit hits", ["client_id"]
)
...
class GatewayConfig:
    def __init__(self):
        ...
        self.services = {
            "bytecode": os.getenv(
                "BYTECODE_SERVICE_URL",
                "http://bytecode-service:8000"),
            "honeypot": os.getenv(
                "HONEYPOT_SERVICE_URL",
                "http://honeypot-service:8000"),
            "mempool": os.getenv(
                "MEMPOOL_SERVICE_URL",
                "http://mempool-service:8000"),
            "bridge": os.getenv(
                "BRIDGE_SERVICE_URL",
                "http://bridge-service:8000"),
            "quantum": os.getenv(
                "QUANTUM_SERVICE_URL",
                "http://quantum-service:8000"),
            "time_machine": os.getenv(
                "TIME_MACHINE_SERVICE_URL",
                "http://time-machine-service:8000"),
            "scanner_slither": os.getenv(
                "SCANNER_SLITHER_URL",
                "http://scanner-slither:8000"),
            "scanner_mythril": os.getenv(
                "SCANNER_MYTHRIL_URL",
                "http://scanner-mythril:8000"),
            "scanner_mythx": os.getenv(
                "SCANNER_MYTHX_URL",
                "http://scanner-mythx:8000"),
            "scanner_manticore": os.getenv(
                "SCANNER_MANTICORE_URL",
                "http://scanner-manticore:8000"),
        }
...
async def require_roles(required_roles: List[str]):
    ...
    async def check_roles(user: User = Depends(require_auth)) -> User:
        if not any(role in user.roles for role in required_roles):
            raise HTTPException(
                status_code=403, detail=f"Requires one of: {required_roles}"
            )
        return user
...
async def require_permissions(required_permissions: List[str]):
    ...
    async def check_permissions(user: User = Depends(require_auth)) -> User:
        if not any(perm in user.permissions for perm in required_permissions):
            raise HTTPException(
                status_code=403,
                detail=f"Requires one of: {required_permissions}")
        return user

    return check_permissions


async def require_subscription(min_tier: str = "pro"):
    ...
    async def forward_request(
        ...
        try:
            if files:
                # Handle file uploads
                files_data = {
                    f"file_{i}": (file.filename, file.file, file.content_type)
...
class ConnectionManager:
    ...
    async def connect(
        self, websocket: WebSocket, connection_id: str, user_id: str = None
    ):
...
if config.environment == "production":
    app.add_middleware(
        TrustedHostMiddleware, allowed_hosts=["*.scorpius.io", "scorpius.io"]
    )
...
async def login(request: Request, credentials: LoginRequest):
    ...
    return TokenResponse(access_token=token, expires_in=86400)  # 24 hours
...
async def start_scan(
    ...
    response = await service_client.forward_request(
        service=service_name, method="POST", path="/scan", data=scan_request
    )
...
async def get_scan_results(scan_id: str, user: User = Depends(require_auth)):
    """Get scan results."""
    response = await service_client.forward_request(
        service="scanner_slither", method="GET", path=f"/scan/{scan_id}/results"
    )
...
async def upload_contract(
    ...
    response = await service_client.forward_request(
        service="scanner_slither", method="POST", path="/upload", files=[file]
    )
...
async def analyze_honeypot(
    ...
    response = await service_client.forward_request(
        service="honeypot", method="POST", path="/analyze", data=analysis_request
    )
...
async def get_honeypot_history(
    limit: int = 50, offset: int = 0, user: User = Depends(require_auth)
):
...
async def get_mempool_transactions(
    limit: int = 100, user: User = Depends(require_auth)
):
    """Get recent mempool transactions."""
    response = await service_client.forward_request(
        service="mempool", method="GET", path=f"/transactions?limit={limit}"
    )
...
async def get_mempool_stats(user: User = Depends(require_auth)):
    ...
    response = await service_client.forward_request(
        service="mempool", method="GET", path="/stats"
    )
...
async def initiate_bridge_transfer(
    ...
    response = await service_client.forward_request(
        service="bridge", method="POST", path="/transfer", data=transfer_request
    )
...
async def get_bridge_transactions(
    limit: int = 50, user: User = Depends(require_subscription("pro"))
):
...
async def quantum_analysis(
    ...
    response = await service_client.forward_request(
        service="quantum", method="POST", path="/analyze", data=analysis_request
    )
...
async def time_machine_simulation(
    simulation_request: dict = Body(...), user: User = Depends(require_auth)
):
    """Run time machine simulation."""
    simulation_request["user_id"] = user.id

    response = await service_client.forward_request(
        service="time_machine", method="POST", path="/simulate", data=simulation_request
    )
    ...
    try:
        while True:
            ...
            if message.get("type") == "subscribe":
                ...
                await manager.send_personal_message(
                    json.dumps({"type": "echo", "data": message}), connection_id
                )
