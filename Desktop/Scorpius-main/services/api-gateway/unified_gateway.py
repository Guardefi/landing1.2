...
class GatewayConfig:
    ...
    def __init__(self):
        ...
        self.jwt_secret = os.getenv(
            "JWT_SECRET", "your-secret-key")  # Load from env
...
class ServiceRequest(BaseModel):
    ...
    timeout: Optional[float] = Field(
        default=30.0, description="Request timeout")
...
class AuthManager:
    ...
    def create_token(
        self, user_data: Dict[str, Any], expires_delta: timedelta = None
    ) -> str:
        """Create JWT token."""
...
class ConnectionManager:
    ...
    async def broadcast_to_service(
            self, service: str, message: Dict[str, Any]):
...
class ServiceCircuitBreaker:
    ...
    def is_available(self, service: str) -> bool:
        ...
        if state == "closed":
            ...
            if (last_failure and (datetime.utcnow() -
                                  last_failure).seconds >= self.recovery_timeout):
...
async def proxy_service_request(
    ...
    if service_info.status != ServiceStatus.HEALTHY:
        raise HTTPException(
            status_code=503, detail=f"Service '{service_name}' is not healthy"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Service request failed: {
                str(e)}",
        )
...
async def gateway_health():
    ...
    return GatewayHealth(
        ...
        metrics={
            ...
            "uptime": orchestrator_health.get(
                "uptime",
                0),
            "total_services": orchestrator_health.get(
                "services",
                {}).get(
                "total",
                0),
            "healthy_services": orchestrator_health.get(
                "services",
                {}).get(
                "healthy",
                0),
...
async def create_access_token(user_id: str, permissions: List[str] = None):
    ...
    user_data = {"user_id": user_id, "permissions": permissions or ["read"]}

    token = auth_manager.create_token(user_data)

    return AuthToken(
        access_token=token, token_type="bearer", expires_in=86400  # 24 hours
    )


@app.post("/services/register")
async def register_service(
    service_info: ServiceInfo, current_user: Dict = Depends(get_current_user)
):
...
async def proxy_request(
    ...
    if rate_limiter and not await rate_limiter.is_allowed(
        client_id, config.rate_limit_per_minute
    ):
            ...
            try:
                ...
                if message_type == "heartbeat":
                    ...
                    if service:
                        ...
                        connection_manager.service_subscriptions[service].add(
                            connection_id
                        )

                        await connection_manager.send_message(
                            connection_id,
                            {"type": "subscription_ack", "service": service},
                        )

            except json.JSONDecodeError:
                await connection_manager.send_message(
                    connection_id, {"type": "error", "message": "Invalid JSON message"}
                )
...
async def honeypot_check(
    ...
    return await proxy_service_request(
        service_name="honeypot", method="POST", path="check", data=request, timeout=15.0
    )


@app.get("/api/mempool/monitor")
async def mempool_monitor(current_user: Dict = Depends(get_current_user)):
    """Convenience endpoint for mempool monitoring."""
    return await proxy_service_request(
        service_name="mempool", method="GET", path="monitor", timeout=10.0
    )
