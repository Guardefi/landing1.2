...
@app.api_route("/api/{service_prefix}/{path:path}",
               methods=["GET", "POST", "PUT", "DELETE"])
async def proxy_prefixed_route(
        service_prefix: str,
        path: str,
        request: Request):
...
class TokenApproval(BaseModel):
    ...
    is_unlimited: bool = Field(...,
                               description="Whether approval is unlimited")
...
class WalletCheckResponse(BaseModel):
    ...
    approvals: List[TokenApproval] = Field(...,
                                           description="List of token approvals")
...
async def wallet_check(request: WalletCheckRequest):
    ...
    high_risk_count = sum(
        1 for approval in mock_approvals if approval.risk_level in [
            "high", "critical"])
    ...
    if high_risk_count > 0:
        recommendations.append(
            "Revoke unlimited approvals for high-risk contracts")
