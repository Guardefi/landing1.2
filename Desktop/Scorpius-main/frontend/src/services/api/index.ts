...
const WS_BASE_URL = process.env.VITE_WS_BASE_URL || 'ws://localhost:8000';
...
export interface AlertConfig {
  id: string;
  name: string;
  type: 'GAS_PRICE' | 'MEV_OPPORTUNITY' | 'SECURITY_THREAT' | 'SYSTEM_HEALTH' | 'CUSTOM';
  conditions: {
    metric: string;
    operator: '>' | '<' | '=' | '!=' | '>=' | '<=';
...
export class ScannerService {
  static async startScan(contractAddress: string, options?: any): Promise<ApiResponse<ScanResult>> {
    return apiClient.post('/api/scanner/scan', { contractAddress, ...options });
  }

  static async getScanResults(scanId: string): Promise<ApiResponse<ScanResult>> {
  ...
  static async uploadContract(file: File): Promise<ApiResponse<{ scanId: string }>> {
...
export class MempoolService {
  static async getTransactions(limit = 50): Promise<ApiResponse<Transaction[]>> {
...
export class BridgeService {
  static async getTransactions(limit = 20): Promise<ApiResponse<BridgeTransaction[]>> {
  ...
  static async initiateBridge(params: any): Promise<ApiResponse<BridgeTransaction>> {
...
export class SystemService {
  ...
  static async getLogs(service?: string, limit = 100): Promise<ApiResponse<any[]>> {
...
export class AnalyticsService {
  ...
  static async getChartData(type: string, timeframe = '24h'): Promise<ApiResponse<any>> {
    return apiClient.get(`/api/analytics/charts/${type}?timeframe=${timeframe}`);
  }
