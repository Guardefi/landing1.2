        ...
        for service in self.services:
            ...
            try:
                ...
                self.docker_client.images.build(
                    path=build_path, tag=image_name, rm=True, pull=True
                )
        ...
        for service in self.services:
            try:
                ...
                deployment = self.k8s_apps_client.read_namespaced_deployment(
                    name=service, namespace=self.environment
                )

                # Check pods
                pods = self.k8s_client.list_namespaced_pod(
                    namespace=self.environment, label_selector=f"app={service}"
                )
            ...
            try:
                self.k8s_apps_client.rollback_namespaced_deployment(
                    ...
                    body=client.AppsV1DeploymentRollback(
                        name=service, rolled_back_to_revision=version
                    ),
                )
                logger.info(
                    f"Successfully rolled back {service} to version {version}")
            except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="Scorpius Platform Deployment Manager")
    parser.add_argument(
        "--environment",
        default="production",
