...
def check_import_paths():
                    ...
                    for pattern in patterns:
                        ...
                        for match in matches:
                            ...
                            if any(
                                ...
                                for keyword in ["test", "config", "docker", "docs"]
                            ):
    ...
    if potential_issues:
        ...
        for issue in potential_issues[:10]:  # Show first 10
            print(
                f"  {issue['file']}:{issue['line']} - {issue['line_content']}")
...
def check_configuration_files():
    ...
    if issues:
        print(
            f"⚠️  Configuration files that may need updates: {
                ', '.join(issues)}"
        )
    else:
...
def check_documentation_links():
                ...
                for link_text, link_url in links:
                    if link_url.startswith("./") or link_url.startswith("../"):
                        if not os.path.exists(link_url):
                            broken_links.append(
                                {"file": doc_file, "link": link_url, "text": link_text}
                            )
        except Exception as e:
    ...
    if broken_links:
        print(
            f"⚠️  Found {
                len(broken_links)} potentially broken documentation links:"
        )
