...
class PlatformManager:
    ...
    async def start_services(self):
        ...
        startup_order = sorted(
            services_config.items(), key=lambda x: x[1].get("startup_delay", 0)
        )
        ...
        for service_name, service_config in startup_order:
            if service_config.get("enabled", True):
                ...
                if delay > 0:
                    logger.info(
                        f"Waiting {delay}s before starting next service...")
    ...
    async def _start_service(self, service_name: str, config: dict):
        ...
        try:
            ...
            if service_dir and service_dir.exists():
                ...
                logger.warning(
                    f"Service directory not found for {service_name}")
    ...
    async def run(self):
        ...
        try:
            ...
            if not is_healthy:
                logger.warning(
                    "Some services are not healthy, but continuing...")
