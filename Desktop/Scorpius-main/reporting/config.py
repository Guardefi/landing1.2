...
class SecurityConfig(BaseSettings):
    ...
    pdf_cert_password: Optional[str] = Field(
        default=None, env="PDF_CERT_PASSWORD")
...
class ReportingConfig(BaseSettings):
    ...
    max_concurrent_reports: int = Field(
        default=5, env="MAX_CONCURRENT_REPORTS")
    report_timeout_minutes: int = Field(
        default=30, env="REPORT_TIMEOUT_MINUTES")
    cleanup_expired_reports: bool = Field(
        default=True, env="CLEANUP_EXPIRED_REPORTS")
    ...
    custom_themes_dir: Optional[Path] = Field(
        default=None, env="CUSTOM_THEMES_DIR")
...
class Settings(BaseSettings):
    ...
    app_name: str = Field(
        default="Scorpius Enterprise Reporting",
        env="APP_NAME")
    ...
    if not config.reporting.templates_dir.exists():
        issues.append(
            f"Templates directory does not exist: {
                config.reporting.templates_dir}")

    # Check PDF signing configuration
    if config.security.pdf_signing_enabled:
        if not config.security.pdf_cert_path or not os.path.exists(
                config.security.pdf_cert_path):
            issues.append("PDF signing enabled but certificate path not found")
        if not config.security.pdf_key_path or not os.path.exists(
                config.security.pdf_key_path):
