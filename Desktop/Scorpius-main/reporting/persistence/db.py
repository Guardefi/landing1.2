...
try:
    ...
    from ..models import AuditLog, Base, Report, ScanResult, User, VulnerabilityFinding
except ImportError:
...
class ScanResultRepository:
    ...
    async def get_scan_results(
        ...
        async with self.db_manager.get_session() as session:
            ...
            if target_address:
                query = query.where(
                    ScanResult.target_info["address"] == target_address)
            if status:
                query = query.where(ScanResult.status == status)

            # Apply pagination
            query = (
                query.limit(limit).offset(offset).order_by(
...
class VulnerabilityRepository:
    ...
    async def get_vulnerabilities_for_scan(
        ...
        async with self.db_manager.get_session() as session:
            query = select(VulnerabilityFinding).where(
    ...
    async def update_vulnerability(
        self, vulnerability_id: str, updates: Dict
...
class ReportRepository:
    ...
    async def get_reports(
        ...
        async with self.db_manager.get_session() as session:
            ...
            query = query.limit(limit).offset(
                offset).order_by(Report.created_at.desc())
    ...
    async def update_report(
            self,
            report_id: str,
            updates: Dict) -> Optional[Report]:
        """Update report"""
        async with self.db_manager.get_session() as session:
            query = update(Report).where(
                Report.id == report_id).values(
                **updates)
            await session.execute(query)
