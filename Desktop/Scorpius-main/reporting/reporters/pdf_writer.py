...
class PDFReporter(TemplatedReporter):
    ...
    def _register_html_filters(self) -> None:
        ...
        def severity_style(severity: str) -> str:
            ...
            return styles.get(
                severity.lower(), "background-color: #6b7280; color: white;"
            )
    ...
    async def _generate_with_reportlab(
        ...
        **kwargs,
        ...
        story.append(
            Paragraph(
                f"Generated: {
                    context.metadata.created_at.strftime('%Y-%m-%d %H:%M:%S')}",
                styles["Normal"],
            ))
        story.append(Spacer(1, 12))
    ...
    async def _generate_chart_images(
    ...
    def _add_html_watermark(self, html_content: str,
        ...
        watermark_div = f'<div class="watermark">{
            watermark.get(
        ...
        if "<head>" in html_content:
            html_content = html_content.replace(
                "<head>", f"<head>{watermark_css}")
        if "<body>" in html_content:
            html_content = html_content.replace(
                "<body>", f"<body>{watermark_div}")
