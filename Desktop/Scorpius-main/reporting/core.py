...
class ReportEngine:
        ...
        try:
            ...
            for format_name in context.output_formats:
                output_path = (
                    self.output_dir /
                    f"report_{
                        datetime.now().strftime('%Y%m%d_%H%M%S')}.{format_name}")

                if format_name == "html":
                    await self._save_html(html_content, output_path)
                elif format_name == "pdf":
                    await self._generate_pdf(
                        html_content, output_path, context.metadata
                    )
    ...
    async def _generate_csv(
        ...
        with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            # Write headers
            writer.writerow(["Finding ID", "Severity",
        ...
        for finding in findings:
            sarif_result = {
                "ruleId": finding.get(
                    ...
                    "text": finding.get(
                        "description", "")}, "level": self._map_severity_to_sarif(
                    finding.get(
                        "severity", "info")), "locations": [
                            {
                                "physicalLocation": {
                                    "artifactLocation": {
                                        "uri": finding.get(
                                            "file", "unknown")}, "region": {
                                                "startLine": finding.get(
                                                    "line", 1)}, }}], }
...
class ReportProcessor:
    ...
    async def process_scan_result(
            self, scan_data: Dict[str, Any]) -> Dict[str, Any]:
