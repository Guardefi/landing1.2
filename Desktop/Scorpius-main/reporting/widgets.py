...
class HeatmapWidget(BaseWidget):
    ...
    def render(self, data: dict[str, Any], **kwargs) -> str:
        ...
        functions = list({v.get("function_name", "unknown")
...
class CallGraphWidget(BaseWidget):
    ...
    def render(self, data: dict[str, Any], **kwargs) -> str:
        ...
        for node in G.nodes():
            vuln_count = len(
                [v for v in vulnerabilities if v.get("function_name") == node]
            )
            if vuln_count >= 3:
...
class MetricsWidget(BaseWidget):
    ...
    def render(self, data: dict[str, Any], **kwargs) -> str:
        ...
        critical_vulns = len([v for v in vulnerabilities if v.get(
            "severity", "").lower() == "critical"])
        high_vulns = len([v for v in vulnerabilities if v.get(
            "severity", "").lower() == "high"])

        unique_functions = len(set(v.get("function_name", "")
                               for v in vulnerabilities))
        avg_risk_score = sum(v.get("risk_score", 0)
                             for v in vulnerabilities) / max(total_vulns, 1)
...
class WidgetRegistry:
    ...
    def register_widget(
            self,
            widget_type: str,
            widget_class: type[BaseWidget]) -> None:
        """
    ...
    def create_widget(
        self, widget_type: str, widget_id: str, config: dict[str, Any]
    ) -> BaseWidget:
        """
    ...
    def render_widget(self, widget_type: str,
