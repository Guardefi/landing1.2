...
class ReportDiffEngine:
    ...
    def __init__(self):
        self.similarity_threshold = (
            0.8  # Threshold for considering vulnerabilities similar
        )
    ...
    async def compare_scans(
        ...
        base_vulns = {self._generate_vuln_key(
            v): v for v in base_scan.vulnerabilities}
        compare_vulns = {
            self._generate_vuln_key(v): v for v in compare_scan.vulnerabilities
        }
        ...
        for key in exact_matches:
            ...
            if detailed:
                vuln_diff = self._compare_vulnerabilities(
                    base_vuln, compare_vuln)
        ...
        diff.severity_changes = self._calculate_severity_changes(
            base_scan, compare_scan
        )
    ...
    def _vulnerabilities_equal(
        self, vuln1: VulnerabilityFinding, vuln2: VulnerabilityFinding
    ) -> bool:
        """Check if two vulnerabilities are exactly equal"""

        # Compare key fields
        return (
            vuln1.title == vuln2.title
            and vuln1.description == vuln2.description
            and vuln1.severity == vuln2.severity
            and vuln1.category == vuln2.category
            and vuln1.function_name == vuln2.function_name
            and vuln1.contract_name == vuln2.contract_name
            and vuln1.line_number == vuln2.line_number
            and abs(vuln1.risk_score - vuln2.risk_score) < 0.1
        )

    def _compare_vulnerabilities(
            self,
            base_vuln: VulnerabilityFinding,
            compare_vuln: VulnerabilityFinding) -> VulnerabilityDiff:
        ...
        if base_vuln.title != compare_vuln.title:
            changes["title"] = {
                "from": base_vuln.title,
                "to": compare_vuln.title}

        if base_vuln.description != compare_vuln.description:
            changes["description"] = {
                "from": base_vuln.description,
                "to": compare_vuln.description,
            }

        if base_vuln.severity != compare_vuln.severity:
            changes["severity"] = {
                "from": base_vuln.severity.value,
                "to": compare_vuln.severity.value,
            }

        if abs(base_vuln.risk_score - compare_vuln.risk_score) >= 0.1:
            changes["risk_score"] = {
                "from": base_vuln.risk_score,
                "to": compare_vuln.risk_score,
            }

        if abs(base_vuln.confidence - compare_vuln.confidence) >= 0.05:
            changes["confidence"] = {
                "from": base_vuln.confidence,
                "to": compare_vuln.confidence,
            }

        if base_vuln.line_number != compare_vuln.line_number:
            changes["line_number"] = {
                "from": base_vuln.line_number,
                "to": compare_vuln.line_number,
            }

        if base_vuln.recommendation != compare_vuln.recommendation:
            changes["recommendation"] = {
                "from": base_vuln.recommendation,
                "to": compare_vuln.recommendation,
            }
    ...
    def _calculate_similarity(
        ...
        func_sim = self._string_similarity(
            vuln1.function_name or "", vuln2.function_name or ""
        )
        scores.append(("function", func_sim, 0.2))

        # Description similarity (low weight)
        desc_sim = self._string_similarity(
            vuln1.description, vuln2.description)
    ...
    def _calculate_severity_changes(
        self, base_scan: ScanResult, compare_scan: ScanResult
    ...
    def generate_diff_summary(self, diff: ScanDiff) -> Dict[str, Any]:
        ...
        critical_delta = diff.severity_changes.get(
            "critical", {}).get("delta", 0)
    ...
    def _generate_recommendations(self, diff: ScanDiff) -> List[str]:
        ...
        if diff.vulnerabilities_added > 0:
            recommendations.append(
                f"Review and address {
                    diff.vulnerabilities_added} newly identified vulnerabilities")

        if diff.vulnerabilities_removed > 0:
            recommendations.append(
                f"Verify that {
                    diff.vulnerabilities_removed} resolved vulnerabilities are truly fixed")
        ...
        if not recommendations:
            recommendations.append(
                "No significant changes detected - maintain current security practices"
            )
...
class DiffReportGenerator:
    ...
    async def generate_diff_report(
        ...
        # Generate diff
        diff = await self.diff_engine.compare_scans(
            base_scan, compare_scan, detailed=True
        )
        ...
        report = {
            ...
            "detailed_changes": {
                ...
                "removed_vulnerabilities": [
                    v.dict() for v in diff.removed_vulnerabilities
                ],
