...
try:
    from ..models import Report, ScanResult, VulnerabilityFinding
except ImportError:
...
class WebhookNotifier:
    ...
    def __init__(
        self, webhook_urls: List[str], timeout: int = 30, max_retries: int = 3
    ):
        ...
        if not AIOHTTP_AVAILABLE:
            logger.warning(
                "aiohttp not available, webhook notifications disabled")
    ...
    async def send_webhook(self, url: str, event: WebhookEvent) -> bool:
        ...
        logger.error(
            f"Webhook failed after {
                self.max_retries} attempts: {url}"
        )
        return False
    ...
    async def send_webhooks(self, event: WebhookEvent) -> List[bool]:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(
                    f"Webhook task failed for {
                        self.webhook_urls[i]}: {result}"
                )
    ...
    async def notify_critical_vulnerability(
            self, vulnerability: VulnerabilityFinding):
        ...
        event = WebhookEvent(
            ...
            data={
                ...
                "description": (
                    ...
                    if len(vulnerability.description) > 500
            ...
            metadata={
                ...
                # Limit references
                "references": (
                    vulnerability.references[:3] if vulnerability.references else [
                    ]
                ),
            },
        )
