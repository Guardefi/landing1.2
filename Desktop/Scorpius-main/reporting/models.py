...
class ScanResult(BaseModel):
    ...
    def high_issues(self) -> int:
        return len(
            [v for v in self.vulnerabilities if v.severity == SeverityLevel.HIGH]
        )
    ...
    def low_issues(self) -> int:
        return len(
            [v for v in self.vulnerabilities if v.severity == SeverityLevel.LOW])

    @property
    def info_issues(self) -> int:
        return len(
            [v for v in self.vulnerabilities if v.severity == SeverityLevel.INFO]
        )
...
class ReportJobCreate(BaseModel):
    ...
    include_signature: bool = Field(
        default=False, description="Include digital signature"
    )
...
class ReportExportRequest(BaseModel):
    ...
    report_ids: List[str]
    export_format: str = Field(default="zip",
                               description="Export format (zip, tar)")
...
class ReportDiffRequest(BaseModel):
    ...
    current_scan_id: str
    diff_type: str = Field(
        default="full", description="full, summary, or vulnerabilities_only"
    )
...
class ScanRequest(BaseModel):
    ...
    project_name: str = Field(..., min_length=1, max_length=255)
    contract_files: List[str] = Field(...,
                                      description="List of contract file paths")
