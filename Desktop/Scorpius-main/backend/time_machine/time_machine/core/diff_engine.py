...
class DiffEngine:
    ...
    async def generate_diff(
        ...
        """
        logger.info(
            f"Generating diff between {from_branch_id} and {to_branch_id}")
    ...
    def _analyze_state_changes(
        ...
        for key in all_keys:
            change = self._compare_values(
                key, from_state.get(key), to_state.get(key))
            if change:
        ...
        for change in changes:
            if "storage" in change.path.lower() or "slot" in change.path.lower():
                ...
                for i, part in enumerate(path_parts):
                    if part.startswith("0x") and len(
                            part) == 42:  # Ethereum address
                        address = part
            ...
            log_changes.append(
                {
                    ...
                    "description": f"Log count changed from {
                        len(from_logs)} to {
                        len(to_logs)}",
                }
            )
    ...
    def _extract_gas_changes(
        ...
        if from_gas != to_gas:
            gas_changes = {
                ...
                "percentage_change": (
                    ((to_gas - from_gas) / from_gas * 100) if from_gas > 0 else 0),
            }

        return gas_changes

    def _calculate_diff_statistics(
            self, changes: List[StateChange]) -> DiffStatistics:
        """Calculate statistics about the diff."""
        total_changes = len(changes)
        additions = sum(
            1 for c in changes if c.change_type == ChangeType.ADDED)
        deletions = sum(
            1 for c in changes if c.change_type == ChangeType.REMOVED)
        modifications = sum(
        ...
        if diff.storage_changes:
            ...
            for addr, changes in diff.storage_changes.items():
                ...
                for slot, change in changes.items():
                    html += f'<div class="modified">Slot {slot}: {
                        change["old_value"]} → {
                        change["new_value"]}</div>'
            html += "</div>"
