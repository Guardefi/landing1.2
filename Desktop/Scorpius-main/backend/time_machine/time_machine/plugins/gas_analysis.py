...
class GasAnalysisPlugin(AnalysisPlugin):
    ...
    async def _analyze_gas_patterns(
                ...
                if isinstance(tx, dict):
                    ...
                    if receipt:
                        ...
                        if tx.get("input") and len(tx["input"]) >= 10:
                            ...
                            patterns["function_signatures"][func_sig].append(
                                gas_used)
        ...
        for func_sig, gas_usages in patterns["function_signatures"].items():
            if len(gas_usages) > 1:
                ...
                variance = sum(
                    (x - avg_gas) ** 2 for x in gas_usages) / len(gas_usages)
    ...
    async def _generate_optimizations(
            ...
            optimizations.append(
                {
                    ...
                    "description": f"Transaction {
                        gas_metrics.max_gas_tx} uses excessive gas",
        ...
        gas_prices = [trend["gas_price"]
                      for trend in patterns.get("gas_price_trends", [])]
        if gas_prices:
                ...
                optimizations.append(
                    {
                        ...
                        "description": f"Contract {contract} consumes {
                            total_gas:,        } gas ({
                            total_gas /
                            gas_metrics.total_gas_used *
                            100:.1f}% of total)",
    ...
    def _calculate_efficiency_score(
        ...
        variance = sum((x - avg_gas) ** 2 for x in gas_per_tx) / \
            len(gas_per_tx)
