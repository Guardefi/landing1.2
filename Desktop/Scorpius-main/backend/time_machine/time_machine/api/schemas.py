...
from ..core.models import FrameType, SessionType, VMBackend
...
class ReplayRequest(BaseModel):
    ...
    name: Optional[str] = Field(
        None, description="Human-readable name for the replay")
    ...
    from_block: Optional[int] = Field(
        None, description="Start block for range replay")
    to_block: Optional[int] = Field(
        None, description="End block for range replay")
    vm_backend: VMBackend = Field(
        VMBackend.ANVIL,
        description="VM backend to use")
    patches: List[Dict[str, Any]] = Field(
        default_factory=list, description="State patches to apply"
    )
    fork_url: Optional[str] = Field(
        None, description="Blockchain RPC URL to fork from")
...
class PatchRequest(BaseModel):
    ...
    conflict_resolution: str = Field(
        "fail", description="Conflict resolution strategy")
...
class DiffRequest(BaseModel):
    ...
    include_unchanged: bool = Field(
        False, description="Include unchanged items")
...
class ForensicSessionRequest(BaseModel):
    ...
    session_type: SessionType = Field(...,
                                      description="Type of forensic session")
...
class BookmarkRequest(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="Bookmark description")
...
class AnalysisRequest(BaseModel):
    ...
    plugins: Optional[List[str]] = Field(
        None, description="Specific plugins to run")
...
class BulkPatchRequest(BaseModel):
    ...
    parallel_execution: bool = Field(
        False, description="Execute patches in parallel")
