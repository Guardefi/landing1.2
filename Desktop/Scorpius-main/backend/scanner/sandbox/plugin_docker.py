...
class DockerPluginManager:
    ...
    async def build_plugin_images(
            self, plugins: List[str] = None) -> Dict[str, bool]:
        """
        ...
        for plugin in plugins:
            ...
            if not build_context.exists():
                logger.error(
                    f"Build context not found for {plugin}: {build_context}")
                results[plugin] = False
                continue
    ...
    async def _build_image(
        ...
        try:
            logger.info(
                f"Building Docker image for {plugin_name}: {image_tag}")
            ...
            if proc.returncode == 0:
                ...
                logger.error(
                    f"Failed to build image for {plugin_name}: {
                        stderr.decode()}"
                )
                return False
    ...
    async def create_plugin_container(
        ...
        try:
            ...
            if container_id:
                ...
                logger.info(
                    f"Created container for {plugin_name}: {container_id}")
                return container_id
            else:
    ...
    def _get_plugin_command(
        ...
        return commands.get(
            plugin_name, [
                "echo", f"Unknown plugin: {plugin_name}"])
