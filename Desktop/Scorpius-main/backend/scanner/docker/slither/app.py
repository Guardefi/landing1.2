...
async def scan_upload(file: UploadFile = File(...), options: str = Form("{}")):
    ...
    try:
        ...
        return ScanResponse(
            scan_id=scan_id,
            status="started",
            message=f"Slither scan {scan_id} started for uploaded file {
                file.filename}",
        )
    ...
    try:
        ...
        if process.returncode == 0 or stdout:
            ...
            try:
                ...
                logger.info(
                    f"Slither scan {scan_id} completed with {
            ...
            logger.error(
                f"Slither scan {scan_id} failed with return code {
                    process.returncode}"
            )
...
def extract_location(elements: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "line": (
            source_mapping.get("lines", [0])[0] if source_mapping.get("lines") else 0
        ),
