...
async def scan_upload(file: UploadFile = File(...), options: str = Form("{}")):
    ...
    try:
        ...
        return ScanResponse(
            scan_id=scan_id,
            status="started",
            message=f"Mythril scan {scan_id} started for uploaded file {
                file.filename}",
        )
    ...
    try:
        ...
        if process.returncode == 0 or stdout:
            ...
            try:
                ...
                logger.info(
                    f"Mythril scan {scan_id} completed with {
            ...
            logger.error(
                f"Mythril scan {scan_id} failed with return code {
                    process.returncode}"
            )
