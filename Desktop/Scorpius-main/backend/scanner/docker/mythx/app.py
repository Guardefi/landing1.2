...
async def start_scan(request: ScanRequest):
    ...
    try:
        ...
        # Check for API key
        api_key = request.options.get(
            "api_key") or os.environ.get("MYTHX_API_KEY")
        if not api_key:
            raise HTTPException(
                status_code=400,
                detail="MythX API key is required")
...
async def scan_upload(file: UploadFile = File(...), options: str = Form("{}")):
    ...
    try:
        ...
        return ScanResponse(
            scan_id=scan_id,
            status="started",
            message=f"MythX scan {scan_id} started for uploaded file {
                file.filename}",
        )
...
async def run_mythx_scan(scan_id: str, target_path: str,
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"MythX scan {scan_id} completed with {
...
async def run_mythx_cli(scan_id: str, target_path: str,
                        options: Dict[str, Any]):
