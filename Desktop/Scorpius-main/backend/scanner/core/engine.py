...
class ScanEngine:
        ...
        try:
            ...
            for plugin_name, plugin in plugins_to_run.items():
                try:
                    ...
                    # Remove plugin from execution
                    plugins_to_run.pop(plugin_name)

            # Create sandboxes for dynamic plugins if enabled
            sandboxes = {}
            if self.config.get("sandbox_enabled", True):
                for plugin_name, plugin in plugins_to_run.items():
                    # Check if plugin needs a sandbox (currently all dynamic
                    # plugins use sandbox)
                    if (
            ...
            # Sequential scanning for now - could be made parallel if plugins
            # support it
            for plugin_name, plugin in plugins_to_run.items():
                try:
                    logger.info(
                        f"Running plugin {plugin_name} for scan {scan_id}")
                    ...
                    complete_plugins += 1
                    scan_result.progress = int(
                        (complete_plugins / total_plugins) * 100)

                    logger.info(
                        f"Plugin {plugin_name} completed with {
                            len(plugin_findings)} findings"
                    )
                except Exception as e:
            ...
            for plugin_name, plugin in plugins_to_run.items():
                try:
                    ...
                    logger.error(
                        f"Failed to cleanup plugin {plugin_name}: {e}")
            ...
            logger.info(
                f"Scan {scan_id} completed successfully with {
                    len(
                        scan_result.findings)} findings in {
                    scan_result.scan_duration:.2f} seconds"
            )
    ...
    def _process_findings(
        ...
        # Use a simple set to track seen findings (in a real implementation,
    ...
    async def schedule_scan(
        ...
        logger.info(
            f"Scan scheduled for {
                target.identifier} with ID {schedule_id}"
        )
        return schedule_id
    ...
    async def cleanup_sandboxes(self, sandboxes: Dict[str, str]) -> None:
        ...
        for plugin_name, sandbox_id in sandboxes.items():
            try:
                ...
                logger.info(
                    f"Cleaned up sandbox {sandbox_id} for plugin {plugin_name}")
            except Exception as e:
    ...
    async def generate_scan_reports(
        ...
        report_data = {
            ...
            "plugins_used": (
                [scan_result.current_plugin] if scan_result.current_plugin else []
            ),
        ...
        logger.info(
            f"Generated {
                len(report_files)} reports for scan {scan_id}"
        )
        return report_files
