...
class ArithmeticOverflowStrategy(BaseStrategy):
    ...
    def __init__(self):
        ...
        self.unsafe_arithmetic_patterns = [
            # Addition without bounds check
            r"(\w+)\s*\+\s*(\w+)(?!\s*<=|\s*<|\s*>=|\s*>)",
            # Multiplication without bounds check
            r"(\w+)\s*\*\s*(\w+)(?!\s*<=|\s*<|\s*>=|\s*>)",
    ...
    async def analyze(
        ...
        if not context.source_code:
            self.logger.warning(
                "No source code available for arithmetic analysis")
            return findings
        ...
        has_automatic_protection = self._has_automatic_overflow_protection(
            solidity_version
        )
    ...
    def _has_automatic_overflow_protection(
            self, version: Optional[str]) -> bool:
    ...
    async def _analyze_unchecked_blocks(
    ...
    async def _analyze_unsafe_arithmetic(
        self, context: StrategyContext
    ) -> List[VulnerabilityFinding]:
    ...
    async def _analyze_vulnerable_patterns(
        self, context: StrategyContext
    ) -> List[VulnerabilityFinding]:
