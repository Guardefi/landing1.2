...
class FlashLoanAttackStrategy(BaseStrategy):
    ...
    async def analyze(
        ...
        if not context.source_code:
            self.logger.warning(
                "No source code available for flash loan analysis")
            return findings
    ...
    async def _analyze_oracle_function(
        ...
        if re.search(
            r"\.getPrice\s*\(|\.latestAnswer\s*\(",
            func_body,
                re.IGNORECASE):
            if not re.search(
                r"TWAP|timeWeighted|average",
                func_body,
                    re.IGNORECASE):
        ...
        if re.search(r"balanceOf.*balanceOf", func_body, re.IGNORECASE):
            vulnerability_score += 2
            vulnerability_details.append(
                "Calculates prices based on token balances")

        # Check for lack of price validation
        if not re.search(
            r"require.*price|assert.*price|price.*require",
            func_body,
                re.IGNORECASE):
            vulnerability_score += 1
