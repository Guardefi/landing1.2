...
class ReentrancyStrategy(BaseStrategy):
    ...
    async def analyze(
            self,
            context: StrategyContext) -> List[VulnerabilityFinding]:
        """Analyze for reentrancy vulnerabilities"""
        findings = []

        if not context.source_code:
            self.logger.warning(
                "No source code available for reentrancy analysis")
            return findings
        ...
        if context.simulation_engine and pattern_findings:
            simulation_findings = await self._simulation_analysis(
    ...
    async def _pattern_analysis(
        ...
        functions_with_calls = self._find_functions_with_external_calls(
            source_code)

        for func_info in functions_with_calls:
            # Check for reentrancy vulnerability patterns
            vulnerability = await self._analyze_function_for_reentrancy(
    ...
    def _find_functions_with_external_calls(
    ...
    async def _analyze_function_for_reentrancy(
        ...
        if has_protection:
            self.logger.debug(
                f"Function {func_name} has reentrancy protection")
            return None
        ...
        for call in external_calls:
            ...
            state_changes_after = [
                sc for sc in state_changes if sc["position"] > call_pos
            ]
        ...
        if vulnerability_score > 0:
            ...
            return self.create_finding(
                ...
                exploit_scenario=self._generate_exploit_scenario(
                    func_name,
                    external_calls),
    ...
    def _find_external_calls_in_function(
            self, func_body: str) -> List[Dict[str, Any]]:
    ...
    def _find_state_changes_in_function(
            self, func_body: str) -> List[Dict[str, Any]]:
    ...
    def _generate_exploit_scenario(
        self, func_name: str, external_calls: List[Dict[str, Any]]
    ) -> str:
        ...
        scenario += (
            "3. In the fallback function, recursively calling the vulnerable function\n"
        )
        scenario += (
            "4. Draining funds or manipulating state before the original call completes"
        )
    ...
    async def _simulation_analysis(
        self, context: StrategyContext, pattern_findings: List[VulnerabilityFinding]
    ) -> List[VulnerabilityFinding]:
    ...
    async def _test_reentrancy_with_simulation(
        self, context: StrategyContext, function_name: str
        ...
        try:
            ...
            if (
                ...
                and "EXPLOIT_SUCCESSFUL" in simulation_result.stdout
            ):
                return {
                    "exploitable": True,
            ...
            self.logger.error(
                f"Simulation test failed for {function_name}: {e}")
            return None

    def _generate_exploit_contract(
        self, target_address: str, function_name: str
    ) -> str:
