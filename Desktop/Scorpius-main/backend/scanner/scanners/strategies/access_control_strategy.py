...
class AccessControlStrategy(BaseStrategy):
    ...
    async def analyze(
        ...
        if not context.source_code:
            self.logger.warning(
                "No source code available for access control analysis")
            return findings
    ...
    async def _find_unprotected_functions(
        ...
        for pattern in self.sensitive_patterns:
            matches = re.finditer(
                pattern, source_code, re.IGNORECASE | re.DOTALL)
    ...
    async def _check_initialization_vulnerabilities(
        ...
        for match in re.finditer(
            ...
            if not re.search(
                r"initialized|_initialized|initializer",
                func_body,
                    re.IGNORECASE):
