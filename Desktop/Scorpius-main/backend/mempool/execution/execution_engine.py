...
class ExecutionEngine:
    ...
    def __init__(
        ...
        try:
            self.account: LocalAccount = Account.from_key(
                self.bot_wallet_private_key)
            if self.account.address.lower() != self.bot_wallet_address.lower():
                raise ValueError(
                    "Private key does not match wallet address") from None
            logger.info(
                f"Initialized execution engine with wallet: {
        ...
        self.gas_buffer_multiplier = config.get(
            "default_gas_buffer_multiplier", 1.25)
    ...
    async def execute_opportunity(
            ...
            method: ExecutionMethod | None = None) -> ExecutionResult:
        """
        ...
        try:
            logger.info(
                f"Executing opportunity {
                    opportunity.opportunity_id} via {
    ...
    async def _build_transactions_for_opportunity(
        ...
        try:
            if opportunity.strategy_type == MEVStrategyType.SANDWICH:
                ...
                logger.warning(
                    f"Unknown strategy type: {
                        opportunity.strategy_type}"
                )

        except Exception as e:
            logger.error(
                f"Error building transactions for {
                    opportunity.opportunity_id}: {e}"
            )
    ...
    async def _build_sandwich_transactions(
        ...
        try:
            ...
            # 10% higher for front-running
            front_run_gas_price = int(gas_price * 1.1)
            # 10% lower for back-running
            back_run_gas_price = int(gas_price * 0.9)
            ...
            logger.info(
                f"Built {
                    len(transactions)} transactions for sandwich attack"
            )
    ...
    async def _build_arbitrage_transactions(
        ...
        try:
            ...
            if not all([token_pair, dex_a, dex_b, amount]):
                raise ValueError(
                    "Missing required parameters for arbitrage") from None
            ...
            logger.info(
                f"Built {
                    len(transactions)} transactions for arbitrage"
            )
    ...
    async def _build_liquidation_transactions(
        ...
        try:
            ...
            logger.info(
                f"Built {
                    len(transactions)} transactions for liquidation"
            )
    ...
    async def _execute_via_flashbots(
            self,
            opportunity: MEVOpportunity,
            transactions: list[TransactionParams]) -> ExecutionResult:
        ...
        try:
            ...
            if bundle_result.get("success"):
                ...
                return ExecutionResult(
                    success=False, error_message=bundle_result.get(
                        "error", "Unknown Flashbots error"), )
    ...
    async def _execute_via_public_mempool(
        ...
        try:
            ...
            logger.info(
                f"Transaction submitted to public mempool: {
                    tx_hash.hex()}"
            )
    ...
    async def _refresh_wallet_state(self) -> None:
        ...
        try:
            ...
            self.stats["wallet_balance_eth"] = wei_to_ether(
                self.wallet_balance)

            logger.debug(
                f"Wallet state: balance={
                    self.stats['wallet_balance_eth']:.4f} ETH, nonce={
                    self.wallet_nonce}"
            )
