...
def load_config(config_dir: Path | None = None) -> dict[str, Any]:
    ...
    config_api["viewer_api_key"] = os.getenv(
        "ELITE_VIEWER_API_KEY", config_api.get(
            "viewer_api_key", "dev_viewer_key"))
    ...
    # Bot wallet - critical, should ideally come from secure source, env vars
    # are common for non-prod
    config.setdefault("execution_engine", {})[
        "bot_wallet_address"] = os.getenv("BOT_WALLET_ADDRESS")
    # PRIVATE KEY SHOULD NOT BE IN YAML. Only ENV or secrets manager.
    # The application code will fetch BOT_WALLET_PRIVATE_KEY directly from
    # os.getenv.
    ...
    check_placeholder(eth_mainnet_config.get(
        "http_rpc_for_sync"), infura_placeholder)
    check_placeholder(
