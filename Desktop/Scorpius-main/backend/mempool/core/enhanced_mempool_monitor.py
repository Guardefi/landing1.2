...
class EnhancedMempoolMonitor:
    ...
    def __init__(
        ...
        logger.info(
            f"EnhancedMempoolMonitor initialized for chain {chain_id} with {
                len(
                    self._web3_instances)} RPC endpoints."
        )
    ...
    def _make_request_with_session_manager(self, url: str, session_key: str):
        ...
        async def request_func(method: str, params: list[Any]) -> Any:
            ...
            try:
                ...
                logger.error(
                    f"RPC request error to {url} with method {method}: {e}")
                raise e from e
        ...
        for url in self.rpc_urls:
            try:
                if url.startswith("ws"):
                    ...
                    session_key = f"mempool_monitor_{
                        self.chain_id}_{
                        url[
                            :30].replace(
                            '/',
                ...
                # Manually set endpoint_uri on provider for logging/reference
                # if not auto-set by Web3.py
                ...
                logger.error(
                    f"Failed to initialize Web3 for {url}: {e}",
                    exc_info=True)

        if not self._web3_instances:
            raise ConnectionError(
                "No valid Web3 providers could be initialized.")
            ...
            try:
                ...
                if not self._pending_filter_id:
                    try:
                        ...
                        logger.info(
                            f"New filter: {
                                self._pending_filter_id} on {
                                getattr(
                                    self._active_web3.provider,
                                    'endpoint_uri',
                                    'Unknown')}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Filter creation failed: {e}. Resetting.")
                ...
                pending_tx_hashes = [tx_hash.hex()
                                     for tx_hash in pending_tx_hashes_raw]

                if pending_tx_hashes:
                    self._stats["txs_fetched_from_filter"] += len(
                        pending_tx_hashes)
        ...
        for w3 in self._web3_instances:
            ...
            try:
                if not await w3.is_connected():
                    ...
                    if isinstance(w3.provider, WebSocketProvider):
                        try:
                            ...
                            logger.warning(
                                f"Reconnect failed for {uri}: {connect_e}")
                            continue
                    if not await w3.is_connected():
                ...
                if fetched_chain_id == self.chain_id:
                    ...
                    logger.warning(
                        f"Chain ID mismatch for {uri}: exp {
                            self.chain_id}, got {fetched_chain_id}"
                    )
            except Exception as e:
    ...
    async def _process_pending_transaction_hashes(
            self, tx_hashes: list[str]) -> None:
        """Process a batch of transaction hashes."""
        new_unique_hashes = [
            h for h in tx_hashes if h not in self._seen_tx_hashes]
        tasks = []
    ...
    async def _fetch_and_process_tx_event(self, tx_hash: str) -> None:
        ...
        try:
            ...
            raw_tx_obj = RawMempoolTransaction(
                tx_hash, tx_data_dict, self.chain_id)
            ...
            for callback in self._callbacks:
                try:
                    ...
                    logger.error(
                        f"Sync callback error for {tx_hash}: {e}",
                        exc_info=True)
    ...
    def _passes_filters(self, tx_data_dict: dict[str, Any]) -> bool:
        ...
        if (
            ...
            return False
        to_lower = (str(tx_data_dict.get("to", "")).lower()
                    if tx_data_dict.get("to") else "")
        from_lower = (
    ...
    async def _cleanup_old_data(self) -> None:
        ...
        if len(self._seen_tx_hashes) > self.max_stored_txs * 10:
            logger.info(
                f"Pruning _seen_tx_hashes from {len(self._seen_tx_hashes)}.")
        ...
        logger.debug(
            f"Cleanup done. Pending: {
                len(
                    self._pending_txs)}, Confirmed: {
                len(
                    self._confirmed_txs)}, Seen: {
                        len(
                            self._seen_tx_hashes)}"
        )
    ...
    def add_async_callback(self,
                           callback: Callable[[MempoolEvent],
                                              Coroutine]) -> None:
    ...
    def set_filter_addresses(self, addresses: list[str]) -> None:
        ...
        self._filter_addresses = {a.lower()
                                  for a in addresses if Web3.is_address(a)}
    ...
    def set_filter_method_signatures(self, method_sigs: list[str]) -> None:
        ...
        logger.info(
            f"Filters: Method sigs set ({len(self._filter_method_sigs)}).")
