            ...
            for i in range(retries):
                try:
                    ...
                    if i == retries - 1:
                        logger.error(
                            f"Function {
                                func.__name__} failed after {retries} retries: {e}")
                        raise e from e
                    logger.warning(
                        f"Attempt {
                            i +
                            1} for {
                            func.__name__} failed: {e}. Retrying in {
                            _delay:.2f}s..."
                    )
...
def _is_new_address(address: str) -> bool:
    ...
    # Very basic heuristic - addresses with repeating patterns might be
    # new/generated
