...
async def get_mev_opportunities(
    ...
    try:
        ...
        if min_profit:
            param_count += 1
            query_parts.append(
                f"AND mo.estimated_profit::numeric >= ${param_count}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve MEV opportunities: {
                str(e)}",
        )
...
async def analyze_transactions_for_mev(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to analyze transactions for MEV: {
                str(e)}",
        )
...
async def get_mev_protection_recommendations(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to generate MEV protection recommendations: {
                str(e)}",
        )
