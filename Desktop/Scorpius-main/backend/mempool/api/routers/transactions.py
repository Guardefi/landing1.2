...
from ..models import PaginationParams, TransactionResponse
...
async def get_transactions(
    ...
    try:
        ...
        if filters.min_value:
            param_count += 1
            query_parts.append(
                f"AND value::numeric >= ${param_count}::numeric")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve transactions: {
                str(e)}",
        )
...
async def get_transaction_details(
    ...
    try:
        ...
        async with db.acquire() as conn:
            ...
            if tx_data.get("gas_used") and tx_data.get("gas"):
                gas_efficiency = float(
                    tx_data["gas_used"]) / float(tx_data["gas"])
            ...
            tx_data["similar_transactions"] = [row["hash"]
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve transaction details: {
                str(e)}",
        )
...
async def get_transaction_trace(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve transaction trace: {
                str(e)}",
        )
...
async def get_address_transactions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve address transactions: {
                str(e)}",
        )
...
async def get_transaction_stats(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve transaction stats: {
                str(e)}",
        )
