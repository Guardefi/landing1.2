...
from ..models import PaginationParams, RuleCondition, RuleRequest, RuleResponse
...
async def create_rule(
    ...
    try:
        ...
        conditions_json = json.dumps([cond.dict()
                                     for cond in rule_data.conditions])
        actions_json = json.dumps([action.dict()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to create rule: {
                str(e)}",
        )
...
async def update_rule(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to update rule: {
                str(e)}",
        )
...
async def delete_rule(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to delete rule: {
                str(e)}",
        )
...
async def toggle_rule(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to toggle rule: {
                str(e)}",
        )
...
async def test_rule(
    ...
    try:
        ...
        confidence = (len(matched_conditions) /
...
async def _validate_rule(rule_data: RuleRequest):
    ...
    for condition in rule_data.conditions:
        if condition.type not in valid_condition_types:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid condition type: {
                    condition.type}",
            )
