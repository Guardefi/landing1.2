...
class ConnectionManager:
    ...
    async def broadcast_to_filtered(
            self,
            message: dict,
            event_type: str = "all"):
        ...
        for client_id, websocket in self.active_connections.items():
            try:
                ...
                subscribed_events = subscription.get(
                    "subscribed_events", ["all"])
    ...
    def _message_matches_filters(self, message: dict, filters: dict) -> bool:
        ...
        if "chain_id" in filters and message.get(
        ...
        if "min_severity" in filters and message.get("type") == "alert":
            ...
            message_severity = severity_levels.get(
                message.get("severity", "low"), 1)
    ...
    async def update_client_subscription(
            self, client_id: str, subscription_data: dict):
    ...
    if message_type == "subscribe":
        ...
        if client_id in connection_manager.connection_subscriptions:
            current_events = connection_manager.connection_subscriptions[client_id].get(
                "subscribed_events", [])
            updated_events = [
                e for e in current_events if e not in events_to_remove]
            await connection_manager.update_client_subscription(
