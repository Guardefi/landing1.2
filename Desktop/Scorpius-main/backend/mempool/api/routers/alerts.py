...
from ..models import AlertResponse, AlertSeverity, PaginationParams
...
class AlertFilters(BaseModel):
    ...
    status: Optional[str] = Field(
        None, description="pending, acknowledged, dismissed")
...
async def get_alert_details(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve alert details: {
                str(e)}",
        )
...
async def update_alert(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to update alert: {
                str(e)}",
        )
    ...
    try:
        ...
        async with db.acquire() as conn:
            ...
            return {
                ...
                "status": update_data.status,
                "message": f"{updated_count} alerts {
                    update_data.status} successfully",
            }
        ...
        async with db.acquire() as conn:
            ...
            resolution_rate = (
                (resolution_stats["resolved"] /
                 ...
                 100) if resolution_stats["total"] > 0 else 0)
...
async def get_live_alert_feed(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve live alert feed: {
                str(e)}",
        )
...
async def delete_alert(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to delete alert: {
                str(e)}",
        )
