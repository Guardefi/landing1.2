...
class EliteMempoolSystem:
    ...
    async def _initialize_web3_instances(self) -> None:
        ...
        for network_name, network_config in networks_config.items():
            ...
            try:
                ...
                if not rpc_urls:
                    logger.warning(
                        f"No RPC URLs configured for {network_name}")
                    continue
                ...
                if not http_rpc and rpc_urls:
                    http_rpc = next(
                        (url for url in rpc_urls if url.startswith("http")),
                        rpc_urls[0])
                ...
                logger.error(
                    f"Failed to initialize Web3 for {network_name}: {e}")
        ...
        for network_name, network_config in networks_config.items():
            ...
            try:
                ...
                monitor = EnhancedMempoolMonitor(
                    ...
                    max_stored_txs=mempool_config.get(
                        "max_stored_txs",
                        10000),
                    poll_interval=mempool_config.get(
                        "poll_interval_seconds",
                        0.1),
                    cleanup_interval=mempool_config.get(
                        "cleanup_interval_seconds",
                        60.0),
                    reconnect_delay=mempool_config.get(
                        "reconnect_delay_seconds",
                        5.0),
                    request_timeout=mempool_config.get(
                ...
                min_tx_value = mempool_config.get(
    ...
    async def _handle_mempool_event(self, event: MempoolEvent) -> None:
        ...
        try:
            ...
            if not network_name or network_name not in self.mev_detectors:
                logger.debug(
                    f"No MEV detector found for network {
                        event.network_id}"
                )
                return
            ...
            if opportunities:
                self.system_stats["opportunities_detected"] += len(
                    opportunities)
                logger.info(
                    f"Detected {
                        len(opportunities)} MEV opportunities from tx {
                        event.tx_hash}"
                )
    ...
    async def _handle_mev_opportunities(
        ...
        try:
            ...
            min_profit_usd = opportunity_config.get(
                "min_execution_profit_usd", 5.0)
            ...
            for opportunity in opportunities:
                ...
                if opportunity.estimated_profit_usd < min_profit_usd:
                    logger.debug(
                        f"Opportunity {
                            opportunity.opportunity_id} below minimum profit threshold")
                    continue
                ...
                if result.success:
                    ...
                    if hasattr(result,
                               "profit_realized") and result.profit_realized:
                    ...
                    logger.warning(
                        f"Opportunity execution failed: {
                            opportunity.opportunity_id}, error: {error_msg}"
                    )
    ...
    async def _get_system_stats(self) -> Dict[str, Any]:
        ...
        for detector in self.mev_detectors.values():
            if hasattr(detector, "get_stats"):
                detector_stats = detector.get_stats()
                total_opportunities += detector_stats.get(
                    "opportunities_detected", 0)
