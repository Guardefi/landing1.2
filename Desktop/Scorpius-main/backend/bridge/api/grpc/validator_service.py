...
class ValidatorServicer:
    ...
    async def ListValidators(
        ...
        logger.info(
            f"gRPC list validators request: active_only={
                request.active_only}"
        )
        ...
        try:
            ...
            if request.active_only:
                mock_validators = [
                    v for v in mock_validators if v.status == "active"]
    ...
    async def GetValidatorStatus(
            self,
            request: ValidatorStatusRequest,
            context: grpc.aio.ServicerContext) -> ValidatorStatusResponse:
    ...
    async def StreamValidatorEvents(
        ...
        logger.info(
            f"gRPC validator event stream: {
                request.validator_id or 'all'}"
        )
...
async def add_validator_service_to_server(server: aio.Server):
    ...
    ValidatorServicer()
    # server.add_ValidatorServiceServicer_to_server(validator_servicer,
    # server)  # This would be generated from proto
