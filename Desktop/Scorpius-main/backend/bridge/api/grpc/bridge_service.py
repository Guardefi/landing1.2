...
class BridgeServicer:
    ...
    async def InitiateBridge(
        ...
        try:
            ...
            transaction_id = f"tx_{
                request.source_chain}_{
                request.target_chain}_mock"
    ...
    async def GetBridgeStatus(
        ...
        logger.info(
            f"gRPC status request for transaction: {
    ...
    async def StreamBridgeEvents(
        ...
        logger.info(
            f"gRPC event stream request: {
                request.transaction_id or 'all'}"
        )
...
async def create_grpc_server(port: int = 50051) -> aio.Server:
    ...
    BridgeServicer()
    # server.add_BridgeServiceServicer_to_server(bridge_servicer, server)  #
    # This would be generated from proto
