...
class QuantumEncryptRequest(BaseModel):
    ...
    algorithm: QuantumAlgorithmEnum = Field(
        default=QuantumAlgorithmEnum.LATTICE_BASED)
    security_level: SecurityLevelEnum = Field(
        default=SecurityLevelEnum.LEVEL_3)
...
class SecurityScanRequest(BaseModel):
    ...
    target: str = Field(...,
                        description="Target to scan (address, contract, etc.)")
