...
class GuardianConfig:
    ...
    surveillance_chains: List[int] = field(
        default_factory=lambda: [1, 137, 42161, 56])
    ...
    def from_env(cls) -> "MevGuardianConfig":
        ...
        guardian = GuardianConfig(
            threat_confidence_threshold=float(
                os.getenv(
                    "THREAT_DETECTION_THRESHOLD", "0.8")), simulation_fork_provider=os.getenv(
                "SIMULATION_FORK_PROVIDER", "tenderly"), honeypot_scan_interval=int(
                    os.getenv(
                        "HONEYPOT_SCAN_INTERVAL", "30")), )

        # Attack configuration
        attack = AttackConfig(
            profit_threshold_eth=float(
                os.getenv(
                    "MEV_PROFIT_THRESHOLD", "0.01")), max_gas_price_gwei=int(
                os.getenv(
                    "MAX_GAS_PRICE_GWEI", "150")), mev_protection_enabled=os.getenv(
                        "MEV_PROTECTION_ENABLED", "true").lower() == "true", )
