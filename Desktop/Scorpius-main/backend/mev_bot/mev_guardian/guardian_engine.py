...
class GuardianEngine:
    ...
    def _initialize_detectors(self):
        ...
        self.threat_detectors[ThreatType.ORACLE_MANIPULATION] = (
        ...
        self.threat_detectors[ThreatType.LIQUIDATION_ATTACK] = (
            LiquidationAttackDetector(self.config)
        )

        self.logger.info(
            f"Initialized {len(self.threat_detectors)} threat detectors")
    ...
    async def _honeypot_scanning_loop(self):
        ...
        self.logger.info(
            f"Starting honeypot scanner (interval: {scan_interval}s)...")
    ...
    async def analyze_transaction(
            self, tx: TransactionData) -> List[ThreatDetection]:
        """
    ...
    async def simulate_attack(
            self,
            simulation_config: Dict) -> AttackSimulation:
        """
        ...
        if tx.value_eth > 10:  # Large ETH swaps
            return ThreatDetection(
                ...
                potential_loss_usd=float(
                    tx.value_eth) * 1800,  # Approximate ETH price
                metadata={
        ...
        if (
            ...
            return ThreatDetection(
                ...
                transaction_hashes=[
                    tx.hash],
                contract_addresses=[
                    tx.to_address] if tx.to_address else [],
            )
