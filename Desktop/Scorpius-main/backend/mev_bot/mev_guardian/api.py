...
class SimulationRequest(BaseModel):
    ...
    fork_block: Optional[int] = Field(
        None, description="Block number to fork from")
...
async def get_guardian_status(authenticated: bool = Depends(verify_token)):
    ...
    if not guardian_engine:
        raise HTTPException(status_code=503,
                            detail="Guardian engine not available")
...
@app.get("/api/v1/guardian/simulations",
...
@app.get("/api/v1/guardian/simulations/{simulation_id}",
