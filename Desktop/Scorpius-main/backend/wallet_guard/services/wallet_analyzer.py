...
class WalletAnalyzer:
    ...
    async def analyze_wallets(
        ...
        # Parse results
        risky_approvals = results[0] if include_approvals and len(
            results) > 0 else []
        drainer_signatures = (
            results[1] if include_signatures and len(results) > 1 else []
        )
        spoofed_approvals = results[2] if include_spoofed and len(
            results) > 2 else []
    ...
    async def _analyze_approvals(
            self,
            adapter,
            address: str) -> List[ApprovalRisk]:
    ...
    async def _analyze_spoofed(
            self,
            adapter,
            address: str) -> List[SpoofedApproval]:
    ...
    async def generate_revoke_transactions(
        ...
        tx_data_str = json.dumps([tx.dict()
                                 for tx in transactions], sort_keys=True)
