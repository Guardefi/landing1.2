    ...
    async def _cleanup_expired_records(self):
        ...
        while True:
            try:
                ...
                for client_id, timestamps in list(
                        self.request_records.items()):
                ...
                for client_id, block_time in list(
                        self.blocked_clients.items()):
    ...
    async def check_rate_limit(self, request: Request):
        ...
        if client_id in self.blocked_clients:
            ...
            if block_remaining > 0:
                logger.warning(
                    f"Blocked request from {client_id}: {
                        request.url.path}"
                )
                raise HTTPException(
        ...
        if len(recent_requests) >= self.requests_limit:
            ...
            logger.warning(
                f"Rate limit exceeded for {client_id}, blocked for {
                    self.block_duration_seconds}s"
            )
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail=f"Rate limit exceeded: {
                    self.requests_limit} requests per {
                    self.window_seconds} seconds. Try again later.",
            )
