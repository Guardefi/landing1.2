...
async def regenerate_api_key(
    ...
    logger.info(
        f"API key regenerated for user {user_id} by admin {
            admin_user['username']}"
    )
...
@router.post("/users/{user_id}/rate-limit",
             status_code=status.HTTP_204_NO_CONTENT)
async def update_rate_limit(
    ...
    logger.info(
        f"Rate limit updated for user {user_id} to {rate_limit} by admin {
            admin_user['username']}"
    )
    return None
...
async def get_system_stats(_: Dict = Depends(admin_required)):
    ...
    try:
        ...
        # System uptime (placeholder, in production would use actual service
        # uptime)
        import psutil
