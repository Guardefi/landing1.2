...
async def get_dashboard_stats(
    ...
    try:
        ...
        detection_rate = (
            (honeypots_detected /
             total_analyses *
             100) if total_analyses > 0 else 0)
        ...
        logger.error(
            f"Error retrieving dashboard stats: {
                str(e)}",
            exc_info=True,
        )
...
async def get_contract_details(
    ...
    try:
        ...
        avg_confidence = sum(a["confidence"]
                             for a in analyses) / total_analyses
        ...
        logger.error(
            f"Error retrieving contract details: {
                str(e)}",
            exc_info=True,
        )
        raise HTTPException(status_code=500, detail=str(e))
