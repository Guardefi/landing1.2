...
class MLEngine:
    ...
    async def load_models(self):
        ...
        try:
            ...
            if os.path.exists(
                    self.model_path) and os.path.exists(
                    self.scaler_path):
            ...
            if hasattr(self.classifier, "feature_importances_"):
                for i, importance in enumerate(
                    ...
                    if i < len(self.feature_names):
                        feature_importance[self.feature_names[i]] = float(
                            importance)
    ...
    async def _extract_features(self, contract_data: dict) -> List[float]:
        ...
        suspicious_count = sum(
            1 for func in abi if func.get(
                "name", "").lower() in suspicious_functions)
    ...
    def _generate_synthetic_data(self, n_samples=1000):
        ...
        # High bytecode length + high control flow complexity + high
        # selfdestruct presence
        honeypot_idx = np.where(
            (X[:, 0] > 0.7)
            & (X[:, 1] > 0.7)  # High bytecode length
            # High control flow complexity  # Selfdestruct present
            & (X[:, 4] > 0.5)
        )[0]
        ...
        all_honeypot_idx = np.unique(
            np.concatenate([honeypot_idx, honeypot_idx2]))
