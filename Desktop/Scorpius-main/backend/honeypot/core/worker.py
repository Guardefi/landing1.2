...
def analyze_contract(
    ...
    """
    logger.info(
        f"Starting analysis of contract {contract_address} on chain {chain_id}")
...
async def _analyze_contract_async(
    ...
    try:
        ...
        logger.error(
            f"Error analyzing contract {contract_address}: {e}",
            exc_info=True)
        error_result = {
            "address": contract_address,
            "chain_id": chain_id,
...
def _calculate_risk_level(results: Dict[str, Any]) -> str:
    ...
    if results.get("symbolic_analysis"):
        symbolic_confidence = results.get(
            "symbolic_analysis", {}).get(
            "confidence", 0)

    tx_confidence = 0
    if results.get("transaction_analysis"):
        tx_confidence = results.get(
