...
class HoneypotDetector:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.error(
                f"Error initializing honeypot detector: {e}",
                exc_info=True)
            raise
    ...
    async def analyze_contract(
        ...
        logger.info(
            f"Starting analysis for contract {address} on chain {chain_id}")
        ...
        try:
            ...
            if not contract_data or not contract_data.get("bytecode"):
                raise ValueError(
                    f"Contract not found or has no bytecode: {address}")
        ...
        try:
            ...
            if deep_analysis:
                analysis_tasks.append(
                    self.symbolic_engine.analyze(contract_data))
            ...
            result = AnalysisResponse(
                ...
                engine_results={
                    ...
                    "symbolic": (
                        symbolic_result if deep_analysis else {"performed": False}
                    ),
                },
                contract_metadata={
            ...
            logger.info(
                f"Analysis completed for {address}: honeypot={is_honeypot}, confidence={
                    confidence:.2f}")
            return result

        except Exception as e:
            logger.error(
                f"Error analyzing contract {address}: {e}",
                exc_info=True)
            raise
