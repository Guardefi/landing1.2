...
class BytecodeComparisonRequest(BaseModel):
    bytecode1: str = Field(...,
                           description="First bytecode to compare (hex string)")
    bytecode2: str = Field(...,
                           description="Second bytecode to compare (hex string)")
...
class BytecodeComparisonResponse(BaseModel):
    ...
    confidence: float = Field(...,
                              description="Confidence in the similarity score")
...
class SimilaritySearchResponse(BaseModel):
    results: List[Dict[str, Any]
                  ] = Field(..., description="List of similar bytecodes")
...
class BatchIndexingResponse(BaseModel):
    ...
    bytecode_hashes: List[str] = Field(...,
                                       description="List of bytecode hashes")
...
async def compare_bytecodes(
    ...
    if not engine:
        raise HTTPException(status_code=503,
                            detail="Similarity engine not available")
...
async def index_bytecode(
    ...
    try:
        ...
        if not request.bytecode:
            raise HTTPException(
                status_code=400,
                detail="Bytecode must be provided")
