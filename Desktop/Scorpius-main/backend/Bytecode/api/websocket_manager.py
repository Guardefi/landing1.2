...
class WebSocketManager:
    ...
    async def subscribe(self, connection_id: str, event_types: List[str]):
        ...
        for event_type in event_types:
            ...
            if connection_id in self.connection_info:
                self.connection_info[connection_id].subscription_types.add(
                    event_type)
    ...
    async def broadcast_event(self, event: AnalysisEvent):
        ...
        logger.debug(
            f"Broadcasted {
                event.event_type} to {
                len(subscriber_ids)} connections"
        )
    ...
    async def send_analysis_result(
        ...
        event = AnalysisEvent(
            ...
            data=result,
            severity="warning" if result.get(
                "similarity_score",
                0) > 0.9 else "info",
        )
    ...
    async def cleanup_stale_connections(self, max_idle_minutes: int = 30):
        ...
        if stale_connections:
            logger.info(
                f"Cleaned up {
                    len(stale_connections)} stale connections"
            )
...
class RealtimeAnalysisQueue:
    ...
    async def _process_request(
            self, request: Dict[str, Any], similarity_engine):
