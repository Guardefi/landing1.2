...
class TestSimilarityEngine:
    ...
    def test_traditional_similarity_comparison(
        self, engine, sample_bytecode_pairs):
        """Test traditional similarity comparison methods."""
        bytecode1, bytecode2 = sample_bytecode_pairs[0]

        # Test Jaccard similarity
        similarity = engine.comparison_engine.jaccard_similarity(
            bytecode1, bytecode2)
        assert 0.0 <= similarity <= 1.0

        # Test n-gram similarity
        ngram_sim = engine.comparison_engine.ngram_similarity(
            bytecode1, bytecode2)
    ...
    async def test_async_similarity_computation(
        self, engine, sample_bytecode_pairs):
    ...
    def test_error_handling_invalid_bytecode(self, engine):
        ...
        for invalid in invalid_bytecodes:
            try:
                ...
                # Should either handle gracefully or raise appropriate
                # exception
                assert isinstance(result, dict)
