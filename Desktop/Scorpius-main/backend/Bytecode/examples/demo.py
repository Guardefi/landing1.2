...
def example_basic_comparison():
    ...
    if "metrics" in result:
        print(
            f"Computation Time: {
                result['metrics']['computation_time']:.4f}s"
        )
...
def example_similarity_search():
    ...
    for name, bytecode in contracts.items():
        if name != "simple_storage":
            similarity_result = engine.compute_similarity(
                query_bytecode, bytecode)
            results.append(
                {
                    "name": name,
...
def example_advanced_comparison():
    ...
    print(
        f"Jaccard Similarity: {
            comparison_engine.jaccard_similarity(
                bytecode1,
                bytecode2):.4f}"
    )
    print(
    ...
    print(
        f"Structural Similarity: {
            comparison_engine.structural_similarity(
                bytecode1,
                bytecode2):.4f}"
    )
    print(
        f"Opcode Frequency Similarity: {
            comparison_engine.opcode_frequency_similarity(
                bytecode1,
                bytecode2):.4f}"
    )
...
def example_performance_analysis():
    ...
    if "metrics" in result:
        print(
            f"Engine reported time: {
                result['metrics']['computation_time']:.4f} seconds"
        )
        print(
            f"Memory usage: {
                result['metrics'].get(
