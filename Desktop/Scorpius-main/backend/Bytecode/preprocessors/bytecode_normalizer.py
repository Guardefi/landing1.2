...
class BytecodeNormalizer:
    ...
    async def _normalize_jump_destinations(self, bytecode: str) -> str:
        ...
        # Replace jump destinations in PUSH instructions that precede
        # JUMP/JUMPI
        normalized = bytecode
        for pos, dest_name in dest_mapping.items():
            # This is a simplified approach - in practice, need to track jumps
            # properly
            hex_pos = hex(pos // 2)[2:].upper().zfill(4)
            if f"61{hex_pos}" in normalized:  # PUSH2 with destination
                normalized = normalized.replace(
                    f"61{hex_pos}", f"61{dest_name}")
...
class FeatureExtractor:
    ...
    def _extract_structural_features(self, opcodes: List[Dict]) -> Dict:
        ...
        return {
            ...
            "complexity_ratio": (
                len(set(op["mnemonic"] for op in opcodes)) / len(opcodes)
                if opcodes
                else 0
            ),
        }
