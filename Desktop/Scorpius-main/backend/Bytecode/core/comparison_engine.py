...
class MultiDimensionalComparison:
    ...
    async def compute_similarity(
        ...
        weighted_score = sum(
            similarities[dim] * weight for dim,
            weight in self.dimension_weights.items())
    ...
    def _extract_control_flow_dimension(self, opcodes: List[Dict]) -> Set[str]:
        ...
        for i, opcode in enumerate(opcodes):
            if opcode["mnemonic"] in ["JUMP", "JUMPI"]:
                ...
                context_before = [op["mnemonic"]
                                  for op in opcodes[max(0, i - 2): i]]
                context_after = [op["mnemonic"]
                                 for op in opcodes[i + 1: min(len(opcodes), i + 3)]]

                pattern = f"JUMP_PATTERN_{
                    '-'.join(context_before)}_JUMP_{
                    '-'.join(context_after)}"
