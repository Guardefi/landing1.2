...
class DockerPluginManager:
    ...
    def _parse_plugin_results(
        ...
        try:
            if "error" in results:
                logger.error(f"{plugin_name} analysis failed: {results['error']}")
                return []
    ...
    def _parse_slither_results(
        ...
        for detector in detectors:
            ...
            finding = VulnerabilityFinding(
                vulnerability_type=f"slither_{detector.get('check', 'unknown')}",
    ...
    def _parse_mythx_results(
        ...
        for issue in issues:
            ...
            finding = VulnerabilityFinding(
                ...
                location=f"Line {issue.get('sourceLocation', {}).get('line', 'unknown')}",
                source="mythx",
                metadata={
    ...
    def _parse_securify_results(
        ...
        for violation in violations:
            finding = VulnerabilityFinding(
                vulnerability_type=f"securify_{violation.get('type', 'unknown')}",
    ...
    async def scan_with_all_plugins(
        ...
        for plugin_name, task in tasks:
            try:
                ...
                logger.info(f"{plugin_name}: Found {len(findings)} vulnerabilities")
            except Exception as e:
