...
function NotificationTypes() {
  const { preferences, updateTypeChannels, availableChannels, notificationTypes } =
    useNotifications();
              ...
              <div className="flex flex-wrap gap-2">
                {availableChannels.map(channel => (
                  <Button
                    key={channel}
                    variant={
                      preferences.types[type]?.includes(channel) ? 'default' : 'outline'
                    }
                    size="sm"
                    onClick={() => {
...
export function NotificationCenter() {
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `scorpius-notifications-${
            ...
            {Object.entries(preferences.channels)
              ...
              .map(([channel, config]) => (
                <ChannelConfig
                  ...
                  channel={channel}
                  config={config}
                  onUpdate={(newConfig: any) => updateChannelConfig(channel, newConfig)}
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <span className="ml-2 font-medium">
                        {
                          Object.values(preferences.channels).filter(c => c.enabled)
                            .length
                        }
                      </span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Configured Types:</span>
                      <span className="ml-2 font-medium">
