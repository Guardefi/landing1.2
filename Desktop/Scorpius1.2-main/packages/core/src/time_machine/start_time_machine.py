...
def start_celery_worker(
    worker_name: str, queues: list[str], redis_url: str, log_level: str = "INFO"
) -> subprocess.Popen:
...
def main():
    ...
    parser.add_argument(
        "--host", default=DEFAULT_CONFIG["api_host"], help="API server host"
    )
    parser.add_argument(
        "--port", type=int, default=DEFAULT_CONFIG["api_port"], help="API server port"
    )
    parser.add_argument(
        "--redis-url", default=DEFAULT_CONFIG["redis_url"], help="Redis connection URL"
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=DEFAULT_CONFIG["workers"],
        help="Number of Celery workers",
    )
    parser.add_argument(
        "--log-level", default=DEFAULT_CONFIG["log_level"], help="Log level"
    )
    parser.add_argument("--dev", action="store_true", help="Enable development mode")
    parser.add_argument("--api-only", action="store_true", help="Start only API server")
    parser.add_argument(
        "--workers-only", action="store_true", help="Start only Celery workers"
    )
    ...
    logger.info(
        f"Configuration: host={args.host}, port={args.port}, workers={args.workers}"
    )
    ...
    try:
        ...
        if not args.api_only:
            ...
            for i in range(min(args.workers, len(worker_configs))):
                ...
                logger.info(
                    f"Starting worker: {config['name']} (queues: {config['queues']})"
                )
        ...
        if not args.workers_only:
            ...
            api_monitor_thread = threading.Thread(
                target=monitor_process, args=(api_process, "API", logger), daemon=True
            )
        ...
        logger.info(
            f"  - Health Check: http://{args.host}:{args.port}/api/system/health"
        )
        logger.info(
            f"  - Time Machine API: http://{args.host}:{args.port}/api/time-machine/"
        )
        ...
        while not shutdown_event.is_set():
            ...
            for i, process in enumerate(processes):
                if process.poll() is not None:
                    logger.error(
                        f"Process {i} has died with return code {process.returncode}"
                    )
                    shutdown_event.set()
                    break
            time.sleep(1)
