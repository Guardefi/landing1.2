        ...
        async with session.get(f"{BASE_URL}/opportunities?limit=10") as resp:
            if resp.status == 200:
                ...
                for opp in opportunities[:3]:  # Show first 3
                    print(
                        f"   {opp['strategy_type']}: {opp['estimated_profit']:.4f} ETH"
                    )
            else:
        ...
        async with session.get(f"{BASE_URL}/executions?limit=10") as resp:
            if resp.status == 200:
                ...
                for exec in executions[:3]:  # Show first 3
                    ...
                    print(
                        f"   {status_icon} {exec['strategy_type']}: {exec['estimated_profit']:.4f} ETH"
                    )
            else:
            ...
            async with session.get(f"{BASE_URL}/opportunities?limit=5") as resp:
                if resp.status == 200:
                    ...
                    if opportunities:
                        ...
                        print(
                            f"   📊 New opportunity: {latest['strategy_type']} - "
            ...
            while time.time() - start_time < 10:
                try:
                    ...
                    if data.get("type") == "status_update":
                        print(
                            f"📡 WebSocket update: {data['recent_opportunities']} opportunities, "
