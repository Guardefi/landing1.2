...
class FlashLoanArbitrageBot:
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            if not self.w3.isConnected():
                self.logger.error(
                    f"Failed to connect to primary RPC: {self.config['RPC_URL']}"
                )
            ...
            self.logger.critical(
                f"Configuration key missing: {e}. Bot cannot start.", exc_info=True
            )
            ...
            try:
                ...
                if sim_results.get("success") and sim_results.get(
                    ...
                    if execution_success:
                        ...
                        self.logger.info(
                            f"SUCCESS! Tx: {execution_details.get('tx_hash')}, Profit: {profit:.6f} ETH, Gas: {gas_cost:.6f} ETH, Net: {(profit - gas_cost):.6f} ETH"
                        )
    ...
    def _prepare_simulation_strategy(self, opportunity):
        ...
        if sim_res and "swap_route" in sim_res:
            ...
            self.logger.warning(
                f"Unknown opportunity type for {opportunity.get('tx_hash')}. Using default strategy."
            )
    ...
    def _determine_vulnerability_type(self, results):
        ...
        if any(
            "oracle" in str(v).lower() for v in results.get("price_oracle_risks", [])
        ):
    ...
    def _extract_features(self, opportunity, analysis_results=None, sim_results=None):
        ...
        expected_feature_keys = self.config.get(
            "ML_FEATURE_KEYS", ["sim_profit", "sim_gas_used_normalized", "risk_score"]
        )
    ...
    def get_stats(self):
        ...
        net_profit = (
            self.stats["total_profit_gross_eth"] - self.stats["total_gas_spent_eth"]
        )
        return {
...
async def main():
    ...
    while bot.running:
        ...
        if current_time - last_stat_time >= 60:
            ...
            print(
                f"Uptime: {stats['uptime_seconds']:.2f}s | Queue: {stats['current_queue_size']} | Detected: {stats['opportunities_detected']} | Processed: {stats['opportunities_processed']}"
            )
            print(
                f"Executed: {stats['transactions_executed']} | Success: {stats['successful_transactions']} ({stats['tx_success_rate_percent']:.2f}%) | Failed: {stats['failed_transactions']}"
            )
            print(
                f"Gross Profit: {stats['total_profit_gross_eth']:.4f} ETH | Gas Spent: {stats['total_gas_spent_eth']:.4f} ETH | Net Profit: {stats['total_profit_net_eth']:.4f} ETH"
            )
