...
class SmartGasPredictor:
    ...
    def __init__(self, w3: Web3):
        ...
        self.logger.info(
            f"SmartGasPredictor initialized. History length: {self.HISTORY_LENGTH}"
        )
    ...
    async def _update_history(self, max_blocks=5):
        async with self._history_lock:
            try:
                ...
                start_block = max(
                    self._last_processed_block + 1, latest_block_num - max_blocks + 1
                )
                if start_block > latest_block_num:
                    return
                self.logger.debug(
                    f"Updating gas history from block {start_block} to {latest_block_num}"
                )
                fetch_tasks = []
                ...
                for block in blocks:
                    if (
                        not block
                        or not block.get("number")
                        or block.number <= self._last_processed_block
                    ):
                        continue
                    base_fee = block.get("baseFeePerGas")
                    if base_fee is None:
                        self.logger.warning(
                            f"Block {block.number} missing baseFeePerGas. Skipping."
                        )
                        continue
                    priority_fees = []
                ...
                self.logger.debug(
                    f"Gas history updated. Current length: {len(self.history)}. Last block: {self._last_processed_block}"
                )
            except w3_exceptions.ProviderConnectionError as e:
                self.logger.error(
                    f"Error connecting to provider during history update: {e}"
                )
    ...
    def _calculate_priority_fee_target(self) -> int:
        ...
        if not recent_priority_fees:
            self.logger.warning(
                "No recent priority fee data available. Using minimum priority fee."
            )
            return self.MIN_PRIORITY_FEE_WEI
        try:
            calculated_fee = int(
                np.percentile(recent_priority_fees, self.PRIORITY_FEE_PERCENTILE)
            )
            target_priority_fee = max(
                self.MIN_PRIORITY_FEE_WEI,
                min(calculated_fee, self.MAX_PRIORITY_FEE_WEI),
            )
            self.logger.debug(
                f"Priority fees considered: {len(recent_priority_fees)}. Calculated P{self.PRIORITY_FEE_PERCENTILE}: {calculated_fee/1e9:.2f} Gwei. Target Priority Fee: {target_priority_fee/1e9:.2f} Gwei"
            )
            return target_priority_fee
        except Exception as e:
            self.logger.error(
                f"Error calculating priority fee percentile: {e}", exc_info=True
            )
            return self.MIN_PRIORITY_FEE_WEI
    ...
    async def estimate_optimal_gas_eip1559(self) -> dict:
        try:
            ...
            async with self._history_lock:
                if not self.history:
                    self.logger.warning(
                        "Gas history is empty. Fetching latest block base fee directly."
                    )
                    latest_block = await asyncio.to_thread(
                        self.w3.eth.get_block, "latest"
                    )
                    last_base_fee = (
                        latest_block.get("baseFeePerGas") if latest_block else None
                    )
                    if last_base_fee is None:
                        self.logger.error(
                            "Could not fetch latest base fee. Cannot estimate gas."
                        )
                        return {
                            "maxFeePerGas": int(50e9),
                            "maxPriorityFeePerGas": self.MIN_PRIORITY_FEE_WEI,
                        }
                else:
                ...
                self.logger.debug(
                    f"Last Base Fee: {last_base_fee/1e9:.2f} Gwei, Trend: {trend:.3f}, Predicted Base: {predicted_base_fee/1e9:.2f} Gwei"
                )
                target_priority_fee = self._calculate_priority_fee_target()
                buffer_multiplier = 2.0
                max_fee_per_gas = (
                    int(last_base_fee * buffer_multiplier) + target_priority_fee
                )
                self.logger.debug(
                    f"Calculated maxFeePerGas: {max_fee_per_gas/1e9:.2f} Gwei (using {buffer_multiplier}x buffer)"
                )
                max_allowed_gas = self.config.get("MAX_GAS_PRICE_GWEI", 200) * 1e9
                if max_fee_per_gas > max_allowed_gas:
                    self.logger.warning(
                        f"Calculated maxFeePerGas ({max_fee_per_gas/1e9:.2f} Gwei) exceeds limit ({max_allowed_gas/1e9:.2f} Gwei). Capping."
                    )
                    max_fee_per_gas = int(max_allowed_gas)
                    target_priority_fee = min(
                        target_priority_fee,
                        max(
                            self.MIN_PRIORITY_FEE_WEI,
                            max_fee_per_gas - predicted_base_fee,
                        ),
                    )
            result = {
            ...
            self.logger.info(
                f"Optimal Gas Estimate: Max Fee={result['maxFeePerGas']/1e9:.2f} Gwei, Priority Fee={result['maxPriorityFeePerGas']/1e9:.2f} Gwei"
            )
            return result
        except asyncio.CancelledError:
            ...
            self.logger.error(
                f"Failed to estimate optimal EIP-1559 gas: {e}", exc_info=True
            )
            return {
                "maxFeePerGas": int(50e9),
                "maxPriorityFeePerGas": self.MIN_PRIORITY_FEE_WEI,
