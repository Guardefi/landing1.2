...
class PerformanceTracker:
    ...
    def __init__(self):
        ...
        logger.info(
            f"PerformanceTracker Initialized. Base: {self.base_token_symbol} (dec={self.base_token_decimals}), Hist size={self.max_trade_history}"
        )
    ...
    def get_report(self) -> dict:
        ...
        sorted_failures = dict(
            sorted(
                m["failures_by_reason"].items(), key=lambda item: item[1], reverse=True
            )
        )
