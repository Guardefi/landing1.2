...
from .types import (
    ThreatDetection, ThreatType, ThreatSeverity,
    AttackSimulation, SimulationStatus,
    HoneypotDetection, TransactionData,
    ForensicAnalysis, SystemMetrics
)
...
class GuardianEngine:
    ...
    def _initialize_detectors(self):
        ...
        self.threat_detectors[ThreatType.ORACLE_MANIPULATION] = OracleManipulationDetector(
            self.config
        )
        ...
        # Honeypot Detector
        self.threat_detectors[ThreatType.HONEYPOT] = HoneypotDetector(
            self.config
        )
        
        # Liquidation Attack Detector
        self.threat_detectors[ThreatType.LIQUIDATION_ATTACK] = LiquidationAttackDetector(
            self.config
        )
            ...
            try:
                ...
                for sim_id, simulation in self.active_simulations.items():
                    if simulation.completed_at:
                        hours_since_completion = (current_time - simulation.completed_at).total_seconds() / 3600
    ...
    async def analyze_transaction(self, tx: TransactionData) -> List[ThreatDetection]:
        ...
        for threat_type, detector in self.threat_detectors.items():
            try:
                ...
                if detected_threat and detected_threat.confidence >= self.config.guardian.threat_confidence_threshold:
    ...
    async def simulate_attack(self, simulation_config: Dict) -> AttackSimulation:
        ...
        simulation = AttackSimulation(
            simulation_type=ThreatType(simulation_config.get('attack_type', 'sandwich_attack')),
...
class ThreatDetector:
    ...
    async def analyze_transaction(self, tx: TransactionData) -> Optional[ThreatDetection]:
...
class FlashLoanExploitDetector(ThreatDetector):
    ...
    async def analyze_transaction(self, tx: TransactionData) -> Optional[ThreatDetection]:
        ...
        if (tx.to_address in flashloan_providers and 
            tx.function_selector in flashloan_selectors):
...
class LiquidationAttackDetector(ThreatDetector):
    ...
    async def analyze_transaction(self, tx: TransactionData) -> Optional[ThreatDetection]:
        ...
        if (tx.to_address in lending_protocols and 
            tx.function_selector in liquidation_selectors):
