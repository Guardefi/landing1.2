...
class FlashLoanArbitrageExecutor:
    ...
    def __init__(self, config):
        ...
        try:
            ...
            raise ValueError(
                f"Invalid ARBITRAGE_CONTRACT address format: {self.aggregator_contract_address}"
            )
        ...
        self.logger.info(
            f"Executor configured for aggregator contract: {self.aggregator_contract_address}"
        )
    ...
    def _encode_aggregator_call(
        ...
        for action in strategy_sequence:
            if action.get("type") == "swap":
                ...
                self.logger.warning(
                    f"Unsupported action type in strategy: {action.get('type')}"
                )
                continue
    ...
    async def execute_arbitrage(self, opportunity: dict):
        ...
        self.logger.info(
            f"Executing arbitrage. Estimated Profit: {opportunity.get('estimated_profit', 0):.6f} ETH"
        )
        try:
            ...
            self.logger.info(
                f"Gas Settings: MaxFeePerGas={gas_settings['maxFeePerGas']/1e9:.2f} Gwei, "
            ...
            if not protection_result or not protection_result.get("success"):
                self.logger.error(
                    f"MEV protection submission failed: {protection_result.get('error', 'Unknown')}"
                )
                self.execution_stats["failures"] += 1
                return (
                    False,
                    f"MEV submission failed: {protection_result.get('error', 'Unknown')}",
                )
            ...
            if receipt and receipt.status == 1:
                ...
                self.logger.info(
                    f"Tx Confirmed! Gas Used: {gas_used}, Gas Cost: {gas_cost_eth:.6f} ETH, Profit: {actual_profit_eth:.6f} ETH, Net: {net_profit_eth:.6f} ETH"
                )
            ...
            self.logger.critical(
                f"Critical error during arbitrage execution: {e}", exc_info=True
            )
