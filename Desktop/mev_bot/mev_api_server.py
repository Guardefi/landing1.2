...
from mev_strategies import (
    ...
    GovernanceAttackStrategy,
    LiquidationBotStrategy,
    MEVOpportunity,
...
class UltimateMEVBot:
    ...
    def __init__(self):
        ...
        self.strategy_tasks: dict[StrategyType, asyncio.Task | None] = dict.fromkeys(
    ...
    async def start_strategy(self, strategy_type: StrategyType) -> bool:
        ...
        try:
            ...
            if self.strategy_tasks[strategy_type] is not None:
                self.logger.warning(
                    f"Strategy {strategy_type.value} is already running"
                )
                return True
    ...
    async def _execute_opportunity(self, opportunity: MEVOpportunity) -> None:
        ...
        try:
            ...
            if success:
                self.logger.info(
                    f"✅ Successfully executed {opportunity.strategy_type.value}"
                )
            else:
                self.logger.warning(
                    f"❌ Failed to execute {opportunity.strategy_type.value}"
                )
...
async def toggle_strategy(request: StrategyToggleRequest):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Failed to toggle strategy {request.strategy_type}",
            )

    except ValueError:
        raise HTTPException(
            status_code=400, detail=f"Invalid strategy type: {request.strategy_type}"
        ) from e
...
async def get_performance_metrics():
    ...
    total_profit = sum(
        strategy.stats.total_profit for strategy in mev_bot.strategies.values()
    )
    total_opportunities = sum(
        strategy.stats.total_opportunities for strategy in mev_bot.strategies.values()
    )
    ...
    return {
        ...
        "uptime_hours": (
            (time.time() - mev_bot.start_time) / 3600 if mev_bot.start_time else 0
        ),
        "strategies_active": len(
            [s for s in mev_bot.strategies.values() if s.is_active]
        ),
    }
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "mev_api_server:app", host="0.0.0.0", port=port, log_level="info", reload=False
    )
