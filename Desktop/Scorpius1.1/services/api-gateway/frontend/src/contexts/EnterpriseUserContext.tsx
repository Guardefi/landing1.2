  ...
  switch (action.type) {
    ...
    case 'SET_CURRENT_USER':
      return {
        ...
        isManager:
          action.payload?.role === 'admin' || action.payload?.role === 'manager',
      };
...
interface EnterpriseContextType extends EnterpriseState {
  inviteMember: (email: string, role: UserRole, message?: string) => Promise<boolean>;
  ...
  updateMemberFeatures: (memberId: string, features: string[]) => Promise<boolean>;
  ...
  const loadEnterpriseData = async () => {
    try {
      ...
      const mockCurrentUser: TeamMember = {
        ...
        status: 'active',
        tier: tier,
        permissions: USER_ROLES[(user?.role as UserRole) || 'admin'].permissions,
