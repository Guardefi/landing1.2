...
export const useScrollReveal = (
  ...
  const scale = useTransform(scrollYProgress, [0, 0.3, 0.8, 1], [0.8, 1, 1, 1.1]);
...
export const useScrollCounter = (
  ...
  const { startValue = 0, duration = 2, triggerOnce = true, threshold = 0.1 } = options;
...
export const useScrollColor = (colorStops: string[], positions: number[] = [0, 1]) => {
  const ref = useRef<HTMLElement>(null);
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: ['start end', 'end start'],
