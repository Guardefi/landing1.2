...
export const handlers = [
  ...
  http.post(`${config.api.mevUrl}/mev/strategies/:id/pause`, async ({ params }) => {
    await delay(300);
    return HttpResponse.json(createResponse({ id: params.id, status: 'paused' }));
  }),

  http.post(`${config.api.mevUrl}/mev/strategies/:id/stop`, async ({ params }) => {
    await delay(300);
    return HttpResponse.json(createResponse({ id: params.id, status: 'stopped' }));
  }),
  ...
  http.get(`${config.api.mempoolUrl}/mempool/transactions`, async ({ request }) => {
    await delay(400);
    const url = new URL(request.url);
    const page = parseInt(url.searchParams.get('page') || '1');
  ...
  http.post(`${config.api.baseUrl}/scheduler/jobs/:id/pause`, async ({ params }) => {
    await delay(200);
    return HttpResponse.json(createResponse({ id: params.id, status: 'paused' }));
  }),

  http.post(`${config.api.baseUrl}/scheduler/jobs/:id/resume`, async ({ params }) => {
    await delay(200);
    return HttpResponse.json(createResponse({ id: params.id, status: 'active' }));
  }),
  ...
  http.get(`${config.api.baseUrl}/test/timeout`, async () => {
    ...
    return HttpResponse.json(createResponse({ message: 'This should timeout' }));
  }),
];
