...
async def get_simulation_result(simulation_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get simulation result: {str(e)}"
        )
...
async def generate_report(request: ReportRequest):
    ...
    try:
        if request.scan_id not in scan_results_storage:
            raise HTTPException(
                status_code=404, detail=f"Scan result {request.scan_id} not found"
            )
        ...
        if request.format == "json":
            ...
            raise HTTPException(
                status_code=400, detail=f"Unsupported report format: {request.format}"
            )
        ...
        simulation_results_storage[simulation_id].update(
            {
                ...
                "success": True,
                "exploit_scenario": f"Simulated exploit for vulnerability {request.vulnerability_id}",
                "impact_assessment": {
                    "severity": "high",
