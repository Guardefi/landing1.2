...
async def run_comprehensive_demo():
    ...
    console.print(
        Panel.fit("🛡️ Enhanced Scorpius Vulnerability Scanner Demo", style="bold blue")
    )
    ...
    try:
        ...
        syntax = Syntax(DEMO_CONTRACT, "solidity", theme="monokai", line_numbers=True)
        console.print(
            Panel(syntax, title="Demo Contract: VulnerableDeFiVault", expand=False)
        )
        ...
        console.print(
            f"Execution time: {result.analysis_metadata.get('duration', 0):.2f} seconds"
        )
        console.print(
        ...
        for i, finding in enumerate(result.vulnerabilities, 1):
            ...
            affected_funcs = ", ".join(affected_functions[:2])  # Show max 2 functions
        ...
        for i, finding in enumerate(result.vulnerabilities[:3], 1):  # Show top 3
            console.print(
                f"\n[bold cyan]🔍 Vulnerability #{i}: {finding.title}[/bold cyan]"
            )
            console.print(f"[red]Severity:[/red] {finding.severity}")
            console.print(f"[yellow]Confidence:[/yellow] {finding.confidence:.1%}")
            console.print(f"[blue]Description:[/blue] {finding.description[:200]}...")
        ...
        console.print(f"Minimum Loss: [red]${risk.estimated_loss_min:,.0f}[/red]")
        console.print(f"Maximum Loss: [red]${risk.estimated_loss_max:,.0f}[/red]")
        console.print(f"Average Loss: [red]${risk.estimated_loss_avg:,.0f}[/red]")

        # Exploit prediction
        exploit_pred = result.exploit_prediction
        console.print(f"\n[bold]Exploit Development Prediction:[/bold]")
        console.print(
            f"Exploit Probability: [red]{exploit_pred.exploit_probability:.1%}[/red]"
        )
        console.print(
            f"Time to Exploit: [yellow]{exploit_pred.time_to_exploit} days[/yellow]"
        )
        console.print(
            f"Weaponization Likelihood: [magenta]{exploit_pred.weaponization_likelihood:.1%}[/magenta]"
        )
        ...
        for finding in result.vulnerabilities:
            ...
            findings_by_strategy[strategy] = findings_by_strategy.get(strategy, 0) + 1

        for strategy_result in result.analysis_metadata.get("strategy_results", []):
            if hasattr(strategy_result, "strategy_name"):
                ...
                strategy_table.add_row(strategy_name, str(findings_count), status)

        console.print(strategy_table)

        # Recommendations
        console.print(f"\n[bold red]🚨 Immediate Actions Required:[/bold red]")
        console.print("1. [red]Fix reentrancy vulnerability in withdraw function[/red]")
        console.print("2. [red]Add access control to emergencyWithdraw function[/red]")
        console.print(
            "3. [yellow]Implement SafeMath for arithmetic operations[/yellow]"
        )
        console.print("4. [yellow]Add TWAP oracle for price calculations[/yellow]")
        console.print("5. [blue]Add comprehensive access control system[/blue]")
