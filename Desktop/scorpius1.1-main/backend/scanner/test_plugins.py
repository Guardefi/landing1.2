...
async def test_mythril_plugin(contract_code: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Create scan config
        config = ScanConfig(timeout=120, max_depth=5)  # Mythril can take longer
...
async def run_plugin_tests() -> Dict[str, Any]:
    ...
    with Progress(
        ...
        try:
            ...
            results["manticore"] = {
                "success": False,
                "error": "Manticore test timed out",
            }
        except Exception as e:
        ...
        for plugin_name, result in plugin_results.items():
            if not result["success"]:
                if "import failed" in result.get("error", ""):
                    ...
                    console.print(
                        f"• Check {plugin_name} configuration: {result.get('error', '')[:100]}"
                    )


async def main():
    """Main test execution function"""
    console.print(Panel.fit("🛡️ Scorpius Plugin Test Suite", style="bold cyan"))
