...
class ReportingManager:
    ...
    def generate_report(
        ...
        output_path: str,
        format: Union[ReportFormat, str] = ReportFormat.JSON,
    ) -> str:
        """Generate a report
        ...
        if format not in self.generators:
            logger.warning(
                f"Report format {format.value} not supported, falling back to JSON"
            )
            format = ReportFormat.JSON
    ...
    def register_generator(
        self, format: ReportFormat, generator: ReportGenerator
    ) -> None:
