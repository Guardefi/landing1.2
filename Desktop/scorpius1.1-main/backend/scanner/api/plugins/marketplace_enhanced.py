...
class PluginMetadata(BaseModel):
    ...
    repository: Optional[str] = Field(None, description="Source code repository")
...
class PluginCapabilities(BaseModel):
    ...
    supported_blockchains: List[str] = Field(
        default_factory=lambda: ["ethereum"], description="Supported blockchains"
    )
    ...
    requires_internet: bool = Field(False, description="Requires internet access")
...
class PluginConfiguration(BaseModel):
    ...
    port: Optional[int] = Field(None, description="API port (if plugin has REST API)")
    ...
    networks: List[str] = Field(default_factory=list, description="Required networks")
...
async def search_plugins(
    ...
    try:
        ...
        if tags:
            ...
            plugins = [p for p in plugins if any(tag in p.tags for tag in tag_list)]
...
async def upload_plugin(
    ...
    try:
        ...
        plugin_id = f"{upload_request.metadata.name.lower().replace(' ', '-')}-{str(uuid.uuid4())[:8]}"
...
async def install_plugin(
    ...
    try:
        ...
        if plugin.status != "approved":
            raise HTTPException(
                status_code=400, detail="Plugin is not approved for installation"
            )
...
async def list_installed_plugins(token: str = Depends(security)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to list installed plugins: {str(e)}"
        )
...
async def get_marketplace_stats():
    ...
    try:
        ...
        return {
            ...
            "average_rating": sum(p.rating for p in approved_plugins)
            / len(approved_plugins)
            if approved_plugins
            else 0,
