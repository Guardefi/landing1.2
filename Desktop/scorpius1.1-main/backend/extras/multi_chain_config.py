...
class MultiChainConfig:
    ...
    def get_chains_with_flashloans(self) -> List[SupportedChain]:
        ...
        return [chain for chain, config in self.chains.items() if config.flashloan_providers]
    ...
    def get_low_fee_chains(self) -> List[SupportedChain]:
        ...
        return [chain for chain, config in self.chains.items() if config.max_gas_price_gwei <= 100]
