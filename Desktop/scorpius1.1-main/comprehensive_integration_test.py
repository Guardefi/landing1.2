...
class IntegrationTester:
    ...
    def wait_for_service(self, url: str, timeout: int = 60, interval: int = 2) -> bool:
        ...
        while time.time() < end_time:
            try:
                ...
                pass
            time.sleep(interval)
        self.log(f"Service at {url} did not become ready in time", "ERROR")
        return False
    ...
    def test_api_endpoints(self) -> Dict[str, Any]:
        ...
        # Wait for API gateway health endpoint
        if not self.wait_for_service(f"{base_url}/health"):
            return {
                "category": "API Gateway",
                "tests": [],
                "passed": 0,
                "total": 0,
                "error": "API Gateway not ready"
            }
    ...
    def test_scanner_services(self) -> Dict[str, Any]:
        ...
        for name, url in self.scanners.items():
            if not self.wait_for_service(f"{url}/health"):
                scanner_tests.append({
    ...
    def test_real_time_features(self) -> Dict[str, Any]:
        ...
        if not self.wait_for_service(f"{self.endpoints['api_gateway']}/health"):
