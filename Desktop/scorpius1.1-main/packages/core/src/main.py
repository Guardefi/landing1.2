...
class ArchiveManager:
    ...
    async def create_archive(
        ...
        try:
            ...
            if metadata.format == ArchiveFormat.PARQUET:
                ...
                raise ValueError(f"Unsupported format: {metadata.format}") from None
            ...
            if success:
                logger.info(
                    f"Created archive {metadata.file_path} with {len(transactions)} transactions"
                )
    ...
    async def load_archive(self, metadata: ArchiveMetadata) -> list[Transaction] | None:
...
class QueryEngine:
    ...
    async def execute_query(self, query: QueryRequest) -> tuple[list[Transaction], int]:
        ...
        logger.info(
            f"Query returned {len(paginated_transactions)} transactions from {total_count} total matches"
        )
...
class TimeMachineService:
    ...
    async def create_archive(self, transactions: list[Transaction], chain_id: int):
        ...
        if success:
            ...
            logger.info(
                f"Archived {len(transactions)} transactions for chain {chain_id}"
            )
