    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                logger.warning(
                    f"[SyncRetry] {operation_name} failed attempt {attempt+1}/{max_retries}: {e}"
                )
    ...
    **kwargs: Any,
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                logger.warning(
                    f"[AsyncRetry] {operation_name} failed attempt {attempt+1}/{max_retries}: {e}"
                )
        except Exception as e:
    ...
    logger.error(
        f"[AsyncRetry] {operation_name} failed after {max_retries+1} attempts."
    )
    raise (
        last_exception
        if last_exception
