...
import { SettingsService, ConfigVariable, ConfigCategory } from "@/services/api/settings";

const Settings = () => {
  const [config, setConfig] = useState<SystemConfig>(
    StorageManager.getSystemConfig(),
  );
  const [envVariables, setEnvVariables] = useState<ConfigVariable[]>([]);
  ...
  useEffect(() => {
    ...
    const syncWithBackend = async () => {
      setIsLoading(true);
      try {
        // Try the new settings service first
        const [variablesResponse, categoriesResponse] = await Promise.all([
          settingsService.getVariables(),
          settingsService.getCategories(),
        ]);

        if (variablesResponse.success && variablesResponse.data) {
          setEnvVariables(variablesResponse.data.variables);
        }
  ...
  const handleEnvVariableChange = (key: string, value: any) => {
    setEnvVariables((prev) =>
      prev.map((variable) =>
        variable.key === key ? { ...variable, value: value } : variable
      )
    );
    setHasChanges(true);
  };
  ...
  const saveConfiguration = async () => {
    setIsSaving(true);
    try {
      // Save local config to localStorage
      ...
      envVariables.forEach((variable) => {
        if (variable.value !== undefined) {
          updates[variable.key] = variable.value;
        }
      });

      if (Object.keys(updates).length > 0) {
        const response = await settingsService.updateMultipleVariables(updates);
        if (response.success) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          toast.success("Configuration saved locally (legacy backend sync failed)");
        }
      } catch (error) {
        toast.success("Configuration saved locally (legacy backend unavailable)");
      }
  ...
  const importConfiguration = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        StorageManager.importData(parsedData);
        if (parsedData) {
  ...
  const downloadBlueprint = async () => {
    try {
      const response = await settingsService.createBlueprint();
      if (response.success && response.data) {
        ...
        a.click();
        URL.revokeObjectURL(url);
        toast.success("Environment blueprint downloaded");
      }
    } catch (error) {
  ...
  const validateConfiguration = async () => {
    try {
      const response = await settingsService.validateConfiguration();
      if (response.success && response.data) {
        const { summary, errors, warnings } = response.data;
        if (errors.length > 0) {
          toast.error(`Configuration has ${errors.length} errors`);
        } else if (warnings.length > 0) {
          toast.warning(`Configuration has ${warnings.length} warnings`);
        } else {
  ...
  const renderEnvVariableInput = (variable: ConfigVariable) => {
    const value = variable.value || "";
    const onChange = (newValue: string) => handleEnvVariableChange(variable.key, newValue);
      ...
      return (
        <div key={variable.key} className="space-y-2">
          ...
          <div className="flex items-center space-x-2">
            <Switch
              checked={Boolean(value === "true" || value === true || value === "True")}
      ...
      return (
        <div key={variable.key} className="space-y-2">
          ...
          <Select value={value} onValueChange={onChange}>
            <SelectTrigger>
              <SelectValue placeholder={variable.default_value || "Select value"} />
            </SelectTrigger>
            <SelectContent>
  ...
  return (
    <PageLayout>
      ...
      <div className="space-y-6">
        ...
        <Tabs defaultValue="env-vars" className="space-y-6">
          ...
          <TabsContent value="env-vars">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Configure environment variables for all Scorpius services
                </CardDescription>
              </CardHeader>
                  ...
                  <>
                    <Alert>
                      ...
                      <AlertDescription>
                        ...
                        Secret values are masked and never exposed in logs.
                      </AlertDescription>
                    </Alert>
                    ...
                    {Object.entries(groupedVariables).map(([category, variables]) => (
                      <div key={category} className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {variables.map(renderEnvVariableInput)}
                        </div>
                        <Separator />
                      </div>
                    ))}
