...
export interface SimulationResult {
  ...
  simulation_results: {
    ...
    network_forked: string;
    block_number: number;
    simulation_successful: boolean;
  };
  exploit_simulations: Array<{
...
export class SimulationService {
  static async runSimulation(request: SimulationRequest): Promise<ApiResponse<SimulationResponse>> {
    return apiClient.post("/api/simulation/run", request);
  }

  static async getSimulationStatus(scanId: string): Promise<ApiResponse<SimulationStatusResponse>> {
    return apiClient.get(`/api/simulation/status/${scanId}`);
  }

  static async getSimulationResults(scanId: string): Promise<ApiResponse<SimulationResult>> {
    return apiClient.get(`/api/simulation/results/${scanId}`);
  }

  static async getCapabilities(): Promise<ApiResponse<SimulationCapabilities>> {
    return apiClient.get("/api/simulation/capabilities");
  }

  static async getSimulationHistory(limit = 10): Promise<ApiResponse<any>> {
