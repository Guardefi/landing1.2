...
from api.plugin_management import router as plugin_management_router
from api.results import router as results_router
from core.engine import ScanEngine
from core.enhanced_plugin_manager import EnhancedPluginManager
from core.models import ScanConfig, ScanType, Target
...
app.include_router(plugin_management_router, tags=["plugin-management"])
...
class ScanRequest(BaseModel):
    target_type: str = Field(
        ..., description="Type of target (file, source_code, contract_address)"
    )
    target_identifier: str = Field(
        ..., description="Target identifier (path, code, address)"
    )
    scan_type: str = Field(
        default="full", description="Type of scan (static, dynamic, full, quick)"
    )
    plugins: Optional[List[str]] = Field(
        default=None, description="Specific plugins to use"
    )
    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "medium_severity": len(
                    [f for f in scan_result.findings if f.severity == "medium"]
                ),
                "low_severity": len(
                    [f for f in scan_result.findings if f.severity == "low"]
                ),
            },
        }
...
async def start_simulation(
    ...
    try:
        ...
        # In practice, you'd need to look up the vulnerability from the
        # database
        ...
        # Create target
        target = Target(type="source_code", identifier=request.target_identifier)
        ...
        simulation_type = simulation_type_mapping.get(
            request.simulation_type, SimulationType.PROOF_OF_CONCEPT
        )
...
async def get_metrics(token: str = Depends(verify_token)):
    ...
    try:
        ...
        return {
            "scans": {"total": 0, "active": 0, "completed": 0, "failed": 0},
            "simulations": {"total": 0, "active": 0, "completed": 0},
            "plugins": {
                "available": (
                    len(plugin_manager.list_plugins()) if plugin_manager else 0
                ),
                "initialized": len(plugin_manager.plugins) if plugin_manager else 0,
            },
            "system": {"uptime": "0h 0m", "memory_usage": "0%", "cpu_usage": "0%"},
        }
