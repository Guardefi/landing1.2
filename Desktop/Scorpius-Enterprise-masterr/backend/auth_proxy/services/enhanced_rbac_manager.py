...
class EnhancedRBACManager:
    ...
    async def check_permission(
        ...
        try:
            ...
            if permission_id not in user_permissions:
                ...
                granted = all(conditions_met.values()) if conditions_met else True
    ...
    def _calculate_risk_score(
        ...
        if context.resource_sensitivity:
            sensitivity_scores = {"low": 0, "medium": 10, "high": 25, "critical": 50}
            risk_score += sensitivity_scores.get(context.resource_sensitivity, 0)

        # Permission risk
        if permission:
            if permission.action in [Action.DELETE, Action.CONFIGURE]:
                risk_score += 15
            if permission.resource_type in [ResourceType.USERS, ResourceType.SETTINGS]:
                risk_score += 10
