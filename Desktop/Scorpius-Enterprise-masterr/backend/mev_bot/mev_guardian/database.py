...
class DatabaseManager:
    ...
    async def get_simulations(
        self, limit: int = 100, offset: int = 0, simulation_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def get_honeypot_scans(
        self, limit: int = 100, offset: int = 0, contract_address: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def get_forensic_analyses(
        self, limit: int = 100, offset: int = 0, incident_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def get_metrics(
        self, metric_type: str, limit: int = 100, hours: int = 24
    ) -> List[Dict[str, Any]]:
    ...
    async def get_profit_statistics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        async with self.get_connection() as conn:
            ...
            rows = await conn.fetch(query)
            return {"period_hours": hours, "statistics": [dict(row) for row in rows]}
