...
class SecurityScanManager:
        ...
        try:
            ...
            for image in self.config["containers"]["images"]:
                ...
                results[image] = self.scanner.scan_image(
                    image_name=image, config=self.config["trivy"]
                )
    ...
    def generate_report(self, scan_results: dict) -> str:
        ...
        if "code" in scan_results:
            ...
            report += f"- Secrets found: {
                len(
                    scan_results['code'].get(
                        'secrets',
                        []))}\n"
        ...
        if "infrastructure" in scan_results:
            ...
            if "k8s" in scan_results["infrastructure"]:
                report += f"- Kubernetes: {
                    scan_results['infrastructure']['k8s'].get(
                        'status', 'N/A')}\n"
            if "network" in scan_results["infrastructure"]:
                report += f"- Network: {
                    scan_results['infrastructure']['network'].get(
                        'status', 'N/A')}\n"
        ...
        try:
            if self.config["alerts"]["methods"]["slack"]["enabled"]:
                ...
                response = requests.post(
                    webhook, json=message, headers={"Content-Type": "application/json"}
                )
...
def main():
    ...
    try:
        ...
        config_path = os.getenv(
            "SECURITY_CONFIG", "config/security/scanner_config.yaml"
        )
        ...
        print(
            f"Code Scan: {
                'PASS' if not scan_results['code'].get('secrets') else 'FAIL'}"
        )
        print(
            f"Container Scan: {
                'PASS' if all(
                    len(
                        r.get(
                            'critical',
                            [])) == 0 for r in scan_results['containers'].values()) else 'FAIL'}"
        )
        print(
            f"Infrastructure Scan: {
                'PASS' if not scan_results['infrastructure'].get('violations') else 'FAIL'}"
        )
        print("-" * 50)
