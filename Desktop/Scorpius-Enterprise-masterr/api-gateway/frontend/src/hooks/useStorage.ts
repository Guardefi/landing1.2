...
export function useStorage() {
  ...
  const updateSection = useCallback(
    <K extends keyof StorageData>(section: K, updates: Partial<StorageData[K]>) => {
      setIsLoading(true);
      try {
  ...
  const removeItem = useCallback(
    <K extends keyof StorageData>(section: K, arrayKey: string, itemId: string) => {
      setIsLoading(true);
      try {
...
export function useScannerData() {
  const { data, add, remove, updateItem, isLoading } = useStorageSection('scanner');
