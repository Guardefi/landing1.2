...
async def health_check():
    ...
    return HealthResponse(status="healthy", version="1.0.0", plugin="manticore")
...
async def get_capabilities():
    ...
    return {
        ...
        "supported_languages": ["solidity", "bytecode", "binary"],
        "scan_types": ["symbolic", "dynamic", "concolic"],
        "output_formats": ["json"],
        "vulnerabilities": [
            "integer-overflow",
            "integer-underflow",
...
async def scan_upload(file: UploadFile = File(...), options: str = Form("{}")):
    ...
    try:
        ...
        # Run scan in background
        asyncio.create_task(run_manticore_scan(scan_id, tmp_file_path, options_dict))

        return ScanResponse(
            scan_id=scan_id,
            status="started",
            message=f"Manticore scan {scan_id} started for uploaded file {file.filename}",
        )
...
async def run_manticore_scan(scan_id: str, target_path: str, options: Dict[str, Any]):
    ...
    try:
        ...
        class FindingCollector(Plugin):
            ...
            def did_evm_execute_instruction(
                ...
                # Check for specific vulnerabilities after instruction execution
        ...
        try:
            ...
            logger.info(
                f"Manticore scan {scan_id} completed with {len(findings)} findings"
            )
        ...
        logger.warning("Manticore Python API not available, falling back to CLI")
...
async def run_manticore_cli(scan_id: str, target_path: str, options: Dict[str, Any]):
