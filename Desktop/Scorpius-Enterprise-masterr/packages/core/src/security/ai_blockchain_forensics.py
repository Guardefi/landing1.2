...
class AIForensicsEngine:
        ...
        for node in graph.nodes():
            ...
            if is_mixer or self._has_mixer_characteristics(graph, node):
                ...
                if len(in_edges) >= 3 and len(out_edges) >= 3:
                    ...
                    all_transactions = [
                        edge[2].get("hash", "") for edge in in_edges + out_edges
                    ]
                    ...
                    patterns.append(
                        TransactionPattern(
                            ...
                            total_value=sum(
                                edge[2].get("amount", 0) for edge in in_edges
                            ),
                            risk_indicators=[
                                "mixer_service",
                                "privacy_tool",
    ...
    def _is_layering_pattern(self, path_data: dict[str, Any]) -> bool:
        ...
        value_preservation = (
            0.7 <= (total_output / total_input) <= 1.0 if total_input > 0 else False
        )
    ...
    def _has_mixer_characteristics(self, graph: nx.DiGraph, node: str) -> bool:
        ...
        out_std = (
            statistics.stdev(out_amounts) if len(out_amounts) > 1 else float("inf")
        )
...
class BlockchainForensicsEngine:
    ...
    async def generate_investigation_report(self, case_id: str) -> dict[str, Any]:
        ...
        case_alerts = [
            self.alerts[alert_id] for alert_id in case.alerts if alert_id in self.alerts
        ]
