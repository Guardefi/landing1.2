# logging_setup.py (the file is named logger.py, but code references
# "logging_setup.py")
import logging
...
try:
    ...
    print(
        f"CRITICAL: Failed to load config for logging: {config_err}. Using basic defaults.",
        file=sys.stderr,
    )
    LOG_DIR = "./logs"
    CONSOLE_LEVEL = "INFO"
...
try:
    ...
    print(
        f"WARNING: Invalid CONSOLE_LOG_LEVEL '{CONSOLE_LEVEL}', defaulting to INFO.",
        file=sys.stderr,
    )
...
try:
    ...
    try:
        ...
        print(
            f"WARNING: Invalid FILE_LOG_LEVEL '{FILE_LEVEL}', defaulting to DEBUG.",
            file=sys.stderr,
        )
    ...
    print(
        f"WARNING: Failed to create rotating file handler for combined log: {e}",
        file=sys.stderr,
    )
...
try:
    ...
    print(
        f"WARNING: Failed to create rotating file handler for error log: {e}",
        file=sys.stderr,
    )
