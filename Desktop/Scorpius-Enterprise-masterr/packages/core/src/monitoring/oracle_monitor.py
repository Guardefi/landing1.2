    ...
    async def initialize(self):
        ...
        for exchange_id in self.config.get("cexToCheck", []):
            try:
                ...
                logger.error(f"OracleMonitor: CCXT exchange '{exchange_id}' not found.")
            except Exception as e:
                logger.error(
        ...
        try:
            if oracle_type == "CHAINLINK":
                ...
                if not provider_addr:
                    raise ValueError("Aave oracle source needs poolProviderAddress")
                ...
                if not self.chainlink_feeds.get("ETH") and not self.chainlink_feeds.get(
                    "WETH"
                ):
                    raise ValueError(
    ...
    async def _fetch_on_chain_prices(self):
        ...
        try:
            ...
            if (
                ...
                eth_feed = self.chainlink_feeds.get("ETH") or self.chainlink_feeds.get(
                    "WETH"
                )
                if eth_feed:
            ...
            for res in results:
                if not isinstance(res, Exception) and res:
                    try:
                        ...
                        logger.warning(f"Error processing CEX ticker {res}: {proc_err}")
        ...
        for asset_symbol in self.config.get("monitoredAssets", []):
            ...
            if should_be_deviating != self.deviation_flags.get(flag_key, False):
                ...
                if should_be_deviating:
                    logger.warning(f"ORACLE DEVIATION DETECTED for {asset_symbol}!")
                else:
                    logger.info(f"Oracle deviation resolved/stale for {asset_symbol}.")
