...
class CompiledFilter:
    ...
    def __init__(
        self, expression: str, hot_pairs: set[str] = None, blacklist: set[str] = None
    ):
        """
...
class FilterTransformer(ast.NodeTransformer):
    ...
    def visit_Compare(self, node: ast.Compare) -> ast.AST:
        ...
        if len(node.ops) == 1 and isinstance(node.ops[0], ast.In):
            ...
            if isinstance(node.comparators[0], ast.Name) and node.comparators[0].id in [
                "hotPairs",
                "blacklist",
            ]:
...
class FilterEngine:
    ...
    def update_hot_pairs(self, hot_pairs: list[str]) -> None:
        ...
        for filter_obj in self._filters:
            ...
            filter_obj._compiled_func = filter_obj._compile_expression(
                filter_obj.expression
            )
    ...
    def benchmark_filter(
        self, expression: str, test_transactions: list[TransactionData]
    ) -> dict[str, Any]:
        """
        ...
        return {
            ...
            "matches": matches,
            "match_rate": (
                (matches / len(test_transactions)) * 100 if test_transactions else 0
            ),
            "total_time_microseconds": total_time_us,
            "avg_time_per_tx_microseconds": (
                total_time_us / len(test_transactions) if test_transactions else 0
            ),
            "transactions_per_second": (
                len(test_transactions) / (total_time_us / 1_000_000)
                if total_time_us > 0
                else 0
            ),
        }
...
COMMON_FILTERS = {
    ...
    # transfer, approve, swapExactETHForTokens
    "dex_swaps": 'selector in ["0xa9059cbb", "0x095ea7b3", "0x7ff36ab5"]',
    ...
    # swapExactETHForTokens with min value
    "sandwich_targets": '(selector == "0x7ff36ab5") and (valueEth > 0.1)',
    "arbitrage_opportunities": "(valueEth > 0.5) and (to in hotPairs)",
}


def create_default_filter_engine(
    hot_pairs: list[str] = None, blacklist: list[str] = None
) -> FilterEngine:
    """
