...
from models import PDFReportRequest, ReportResponse, ReportStatus, SARIFReportRequest
...
async def generate_pdf_report(
    ...
    background_tasks.add_task(process_pdf_report, report_id, request, api_key_data)
...
async def generate_sarif_report(
    ...
    # Start background processing
    background_tasks.add_task(process_sarif_report, report_id, request, api_key_data)
...
async def get_report_status(
    report_id: str, credentials: HTTPAuthorizationCredentials = Depends(security)
):
...
async def download_report(
    ...
    await audit_service.log_event(
        ...
        user_id=api_key_data.get("user_id"),
        details={"report_id": report_id, "report_type": report_status["report_type"]},
    )
...
async def process_pdf_report(
    report_id: str, request: PDFReportRequest, api_key_data: Dict
):
...
async def process_sarif_report(
    report_id: str, request: SARIFReportRequest, api_key_data: Dict
):
...
if __name__ == "__main__":
    uvicorn.run("app:app", host="0.0.0.0", port=8007, reload=True, log_level="info")
