...
def test_models():
    ...
    try:
        from models import PDFReportRequest, SARIFReportRequest, ScanResult, ToolInfo

        # Test PDF request
        pdf_request = PDFReportRequest(
            title="Test Report", data={"test": "data"}, template="default"
        )
        print("✓ PDF Request model works")

        # Test SARIF request
        scan_result = ScanResult(
            rule_id="TEST-001", level="error", message="Test finding"
        )

        tool_info = ToolInfo(name="Test Tool", version="1.0.0")

        sarif_request = SARIFReportRequest(
            title="Test SARIF", scan_results=[scan_result], tool_info=tool_info
        )
