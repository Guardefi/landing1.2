...
class SARIFGenerator:
    ...
    async def _create_locations(
        ...
        for location in locations:
            sarif_location = {
                "physicalLocation": {
                    "artifactLocation": {"uri": location.get("file_path", "unknown")}
                }
            }

            # Add region information if available
            if any(
                key in location for key in ["line", "column", "start_line", "end_line"]
            ):
                region = {}
    ...
    async def _create_fixes(self, fixes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for fix in fixes:
            sarif_fix = {
                "description": {"text": fix.get("description", "Suggested fix")},
                "artifactChanges": [],
            }
            ...
            if "changes" in fix:
                for change in fix["changes"]:
                    artifact_change = {
                        "artifactLocation": {"uri": change.get("file_path", "unknown")},
                        "replacements": [],
                    }
