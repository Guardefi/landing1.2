...
class ServiceCircuitBreaker:
    ...
    def is_available(self, service: str) -> bool:
        ...
        if state == "closed":
            ...
            if (
                last_failure
                and (datetime.utcnow() - last_failure).seconds >= self.recovery_timeout
            ):
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout) as client:
            ...
            REQUEST_COUNT.labels(
                method=method, service=service_name, status=response.status_code
            ).inc()
...
async def gateway_health():
    ...
    return GatewayHealth(
        ...
        metrics={
            ...
            "healthy_services": orchestrator_health.get("services", {}).get(
                "healthy", 0
            ),
        },
        uptime=orchestrator_health.get("uptime", 0),
