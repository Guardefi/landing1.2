...
class GatewayConfig:
    def __init__(self):
        ...
        self.services = {
            "bytecode": os.getenv(
                "BYTECODE_SERVICE_URL", "http://bytecode-service:8000"
            ),
            "honeypot": os.getenv(
                "HONEYPOT_SERVICE_URL", "http://honeypot-service:8000"
            ),
            ...
            "time_machine": os.getenv(
                "TIME_MACHINE_SERVICE_URL", "http://time-machine-service:8000"
            ),
...
async def require_permissions(required_permissions: List[str]):
    ...
    async def check_permissions(user: User = Depends(require_auth)) -> User:
        if not any(perm in user.permissions for perm in required_permissions):
            raise HTTPException(
                status_code=403, detail=f"Requires one of: {required_permissions}"
            )
        return user
...
async def require_subscription(min_tier: str = "pro"):
    ...
    async def check_subscription(user: User = Depends(require_auth)) -> User:
        ...
        if user_tier_level < required_level:
            raise HTTPException(
                status_code=402, detail=f"Requires {min_tier} subscription or higher"
            )
        return user
...
async def start_scan(
    request: Request, scan_request: dict = Body(...), user: User = Depends(require_auth)
):
