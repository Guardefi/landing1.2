...
export const useScanner = (): ScannerState & ScannerActions => {
  ...
  const analyzeContract = useCallback(
    async (
      ...
      try {
        const result = await scorpiusAPI.analyzeContract(address, options || {});
        setState(prev => ({
          ...prev,
          isAnalyzing: false,
  ...
  const pollScanStatus = useCallback(async (scanId: string) => {
    try {
      ...
      const errorMessage =
        error instanceof ScorpiusAPIError ? error.message : 'Failed to get scan status';
  ...
  const getScanResult = useCallback(async (scanId: string) => {
    try {
      ...
      const errorMessage =
        error instanceof ScorpiusAPIError ? error.message : 'Failed to get scan result';
  ...
  const cancelScan = useCallback(async (scanId: string) => {
    try {
      ...
      const errorMessage =
        error instanceof ScorpiusAPIError ? error.message : 'Failed to cancel scan';
