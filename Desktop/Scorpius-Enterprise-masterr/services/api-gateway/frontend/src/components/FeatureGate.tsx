...
const FeatureUpgradePrompt: React.FC<FeatureUpgradePromptProps> = ({
  ...
  return (
    ...
    >
      <Card className={`max-w-md w-full ${getTierColor(requiredTier)} border-2`}>
        <CardHeader className="text-center">
          ...
          <Badge variant="outline" className="mx-auto border-current text-current">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="text-center space-y-3">
            <p className="text-gray-300 text-sm">
              Upgrade to{' '}
              <strong className="text-white">{tierInfo[requiredTier].name}</strong> to
              unlock:
            </p>
...
const getFeatureDescriptions = (tier: SubscriptionTier): string[] => {
  switch (tier) {
    ...
    default:
      return ['Enhanced security features', 'Priority support', 'Advanced analytics'];
  }
};
