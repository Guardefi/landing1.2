...
import { ScannerConnectionTest } from '@/components/ScannerConnectionTest';
import { CompleteScannerTest } from '@/components/CompleteScannerTest';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setThreatData(prev => [...prev.slice(1), Math.floor(Math.random() * 50) + 10]);
      setNetworkData(prev => [...prev.slice(1), Math.floor(Math.random() * 100) + 20]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setTimeLabels(prev => [...prev.slice(1), newTimeLabel]);
      setThreatData(prev => [...prev.slice(1), Math.floor(Math.random() * 80) + 20]);
      setNetworkData(prev => [...prev.slice(1), Math.floor(Math.random() * 150) + 30]);
      setPerformanceData(prev => [
        ...prev.slice(1),
      ...
      setNetworkActivity(prev =>
        prev.map(network => ({
          ...network,
          value: Math.max(1, Math.min(50, network.value + (Math.random() - 0.5) * 8)),
        })),
      );
      ...
      <div className="max-w-7xl mx-auto p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6 mobile-scroll">
        ...
        >
          <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
            ...
            >
              <LayoutDashboard size={20} className="text-red-500 sm:w-6 sm:h-6" />
            </motion.div>
            <h1
          ...
          <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
            ...
            >
              {isLive ? <Pause className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2" /> : null}
                  ...
                  <svg width="100%" height="100%" className="overflow-visible">
                    ...
                    <motion.path
                      d={`M 0,${240 - (threatData[0] || 0) * 3} ${threatData
                        .map(
                          (value, index) =>
                            `L ${(index / (threatData.length - 1)) * 100}%,${
                              240 - value * 3
                            }`,
                        )
                        .join(' ')}`}
            ...
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  {Object.entries(securityMetrics).map(([key, value], index) => (
                    <motion.div
                      key={key}
                      initial={{ opacity: 0, x: 20 }}
                      animate={{ opacity: 1, x: 0 }}
              ...
              <div className="h-48 relative">
                ...
                <div className="absolute top-2 right-2 bg-purple-500/20 px-3 py-1 rounded-lg border border-purple-500/30">
                  <span className="text-purple-400 font-mono text-sm">
                    {performanceData[performanceData.length - 1]?.toFixed(1) || '0'}%
                    efficiency
                  </span>
                </div>
              </div>
                ...
                <div className="space-y-3">
                  {dashboardData.recentActivity.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm">
                        Activity will appear here when you start using the platform
                      </p>
                    </div>
                  ) : (
                  ...
                  {dashboardData.alerts.length === 0 ? (
                    ...
                    dashboardData.alerts.slice(0, 5).map(alert => (
                      <motion.div
                        ...
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        className={`p-3 bg-gray-900/50 rounded border-l-2 ${getAlertColor(
                          alert.type,
        ...
        <ScrollReveal>
          <Card className="bg-black/50 border-red-500/30">
            <CardHeader>
              <CardTitle className="text-red-400 font-mono">QUICK ACTIONS</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <ScrollReveal>
          <Card className="bg-black/50 border-red-500/30">
            <CardHeader>
              <CardTitle className="text-red-400 font-mono">SYSTEM STATUS</CardTitle>
            </CardHeader>
            <CardContent>
