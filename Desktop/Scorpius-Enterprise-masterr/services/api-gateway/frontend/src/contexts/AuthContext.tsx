  ...
  const initializeAuth = async () => {
    try {
      ...
      if (userSession) {
        ...
        const isLicenseVerified = licenseStatus === 'true' || licenseStatus === 'demo';
  ...
  const login = async (username: string, password: string): Promise<boolean> => {
    try {
      ...
      if (isValidCredentials) {
        ...
        const authToken = `scorpius_${Date.now()}_${Math.random()
          .toString(36)
          .substr(2, 9)}`;
  ...
  const updateUserProfile = (profile: Partial<User>) => {
    ...
    if (currentSession) {
      try {
        ...
        localStorage.setItem('scorpius_user_session', JSON.stringify(updatedSession));
      } catch (error) {
