...
class ReportGenerator:
    ...
    async def generate_full_audit_report(
        ...
        # Prepare report context - note that scan_result is a single result,
        # but ReportContext expects a list
        ...
        for format_name in formats:
            ...
            filename = f"security_audit_{
                scan_result.project_name}_{timestamp}.{format_name}"
            output_path = self.output_dir / filename

            try:
                # Generate report - reporters create their own files and return
                # paths
    ...
    async def generate_diff_report(
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"diff_report_{
            current_scan.project_name}_{timestamp}.{output_format}"
        output_path = self.output_dir / filename
    ...
    async def create_audit_bundle(
        ...
        # Create bundle
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        bundle_name = f"audit_bundle_{
            scan_result.project_name}_{timestamp}.zip"
        ...
        with zipfile.ZipFile(bundle_path, "w", zipfile.ZIP_DEFLATED) as bundle:
            ...
            scan_data = (
                ...
                else scan_result.__dict__
            )
            bundle.writestr(
                "scan_result.json", json.dumps(scan_data, indent=2, default=str)
            )
