...
async def submit_scan(
    ...
    try:
        ...
        background_tasks.add_task(
            process_scan_background, job_id, scan_request, current_user["user_id"]
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to submit scan: {
                str(e)}",
        )
...
async def create_report_job(
    ...
    try:
        ...
        background_tasks.add_task(
            generate_report_background, job_id, job_request, current_user["user_id"]
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create report job: {str(e)}"
        )
...
async def list_themes(current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to list themes: {
                str(e)}",
        )
...
async def list_widgets(current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to list widgets: {
                str(e)}",
        )
...
async def export_reports(
    ...
    try:
        ...
        bundle_data = await exporter.export_reports(
            report_ids=export_request.report_ids, format=export_request.format
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to export reports: {str(e)}"
        )
...
async def generate_report_background(
    job_id: str, job_request: ReportJobCreate, user_id: str
):
