...
class MarkdownReporter(BaseReporter):
    ...
    async def generate_report(self, context: ReportContext) -> Dict[str, Any]:
        ...
        output_path = (
            self.output_dir / f"{context.metadata.title.lower().replace(' ', '_')}.md"
        )
    ...
    def _generate_markdown_content(self, context: ReportContext) -> str:
        ...
        lines.append(
            f"**Generated:** {context.metadata.generated_at.strftime('%Y-%m-%d %H:%M:%S UTC')}\n"
        )
        ...
        if context.scan_result:
            ...
            lines.append(
                f"- **Total Vulnerabilities:** {len(context.scan_result.vulnerabilities)}"
            )
