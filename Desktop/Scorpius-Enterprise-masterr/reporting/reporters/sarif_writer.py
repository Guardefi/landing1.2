...
class SARIFReporter(BaseReporter):
    ...
    def _build_sarif_structure(
        self, context: ReportContext, tool_name: str, tool_version: str
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "version": "2.1.0",
            "runs": [self._build_run(context, tool_name, tool_version)],
        }
    ...
    def _build_tool(self, tool_name: str, tool_version: str) -> Dict[str, Any]:
        ...
        return {
            "driver": {
                ...
                "shortDescription": {"text": "Smart contract security analysis tool"},
                "fullDescription": {
        ...
        rules = [
            {
                ...
                "properties": {"category": "security", "cwe": "CWE-362"},
            },
            {
                "id": "access_control",
                "name": "Access Control",
                "shortDescription": {"text": "Access control vulnerability"},
                "fullDescription": {
                ...
                "properties": {"category": "security", "cwe": "CWE-284"},
            },
            {
                "id": "arithmetic",
                "name": "Arithmetic Issues",
                "shortDescription": {"text": "Integer overflow/underflow"},
                "fullDescription": {
                ...
                "properties": {"category": "security", "cwe": "CWE-190"},
            },
            {
                "id": "unchecked_calls",
                "name": "Unchecked External Calls",
                "shortDescription": {"text": "Unchecked external call"},
                "fullDescription": {
                ...
                "properties": {"category": "security", "cwe": "CWE-252"},
            },
            {
                "id": "denial_of_service",
                "name": "Denial of Service",
                "shortDescription": {"text": "Denial of service vulnerability"},
                "fullDescription": {
                    "text": "Code patterns that may lead to denial of service attacks"
                },
                "defaultConfiguration": {"level": "warning"},
                "helpUri": "https://docs.scorpius.security/rules/dos",
                "properties": {"category": "security", "cwe": "CWE-400"},
            },
        ]
        ...
        for scan in context.scan_results:
            for contract in scan.contracts:
                if contract.name not in artifact_map:
                    artifact = {
                        "location": {"uri": f"contracts/{contract.name}.sol"},
                        "description": {"text": f"Smart contract: {contract.name}"},
                        "properties": {
                    ...
                    if contract.source_code:
                        artifact["contents"] = {"text": contract.source_code}
                    ...
                    result["relatedLocations"] = [
                        {
                            ...
                            "message": {"text": f"Function: {vuln.function_name}"},
                        }
                    ]
...
class EnhancedSARIFReporter(SARIFReporter):
    ...
    def _build_tool(self, tool_name: str, tool_version: str) -> Dict[str, Any]:
        ...
        base_tool["driver"]["supportedTaxonomies"] = [
            {"name": "CWE", "index": 0, "guid": str(uuid.uuid4())},
            {"name": "OWASP-Top-10", "index": 1, "guid": str(uuid.uuid4())},
        ]
        ...
        base_tool["driver"]["notifications"] = [
            {
                "id": "scan-timeout",
                "shortDescription": {"text": "Scan timeout"},
                "fullDescription": {"text": "Analysis was terminated due to timeout"},
            },
            {
                "id": "parse-error",
                "shortDescription": {"text": "Parse error"},
                "fullDescription": {"text": "Unable to parse contract source code"},
            },
        ]
    ...
    def _build_results(self, context: ReportContext) -> List[Dict[str, Any]]:
        ...
        for result in results:
            if "properties" in result:
                ...
                if cwe_id:
                    result["taxa"] = [
                        {"toolComponent": {"name": "CWE", "index": 0}, "id": cwe_id}
                    ]
