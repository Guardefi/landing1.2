...
class JSONReporter(BaseReporter):
    ...
    def _build_json_structure(
        self, context: ReportContext, include_metadata: bool, include_raw_data: bool
    ) -> Dict[str, Any]:
        ...
        if include_raw_data:
            json_data["raw_scan_data"] = [scan.dict() for scan in context.scan_results]

        # Add sections if any
        if context.sections:
            json_data["sections"] = [section.to_dict() for section in context.sections]
    ...
    def _build_summary_section(
        ...
        return {
            ...
            "scan_date_range": {
                "earliest": min(
                    stats.get("scan_dates", [datetime.utcnow()])
                ).isoformat(),
    ...
    def _build_vulnerabilities_section(
