...
class HTMLReporter(TemplatedReporter):
    ...
    def __init__(
        self, output_dir: Optional[Path] = None, template_dir: Optional[Path] = None
    ):
    ...
    def _register_filters(self) -> None:
        ...
        def severity_badge(severity: str) -> str:
            ...
            return f'<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium text-white {color_class}">{
                severity.upper()}</span>'
        ...
        def code_highlight(code: str, language: str = "solidity") -> str:
            ...
            # syntax highlighter
            code = code.replace(
                "function", "<span class='text-blue-600 font-semibold'>function</span>"
            )
            code = code.replace(
                "contract",
                "<span class='text-purple-600 font-semibold'>contract</span>",
            )
            code = code.replace(
                "require", "<span class='text-red-600 font-semibold'>require</span>"
            )
            code = code.replace(
                "return", "<span class='text-green-600 font-semibold'>return</span>"
            )
    ...
    async def generate(
        ...
        **kwargs,
        ...
        template_context.update(
            {
                ...
                "chart_data_json": json.dumps(
                    template_context["charts"], default=self._json_serializer
                ),
                **kwargs,
            }
        )
        ...
        if include_charts:
            template_context["charts_js"] = self._generate_charts_js(
        ...
        try:
            ...
            if template_name != "default_report.html":
                ...
                html_content = self.render_template(
                    "default_report.html", template_context
                )
            else:
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (severityCtx) {
                new Chart(severityCtx, {
                    ...
                    data: {
                        labels: """
            + json.dumps(charts_data["severity_distribution"]["labels"])
            + """,
                        datasets: [{
                            data: """
            + json.dumps(charts_data["severity_distribution"]["values"])
            + """,
                            backgroundColor: """
            + json.dumps(charts_data["severity_distribution"]["colors"])
            + """,
                            borderWidth: 2,
                            borderColor: '#ffffff'
                        }]
            ...
            if (timelineCtx && """
            + json.dumps(len(charts_data["timeline"]) > 1)
            + """) {
                const timelineData = """
            + json.dumps(charts_data["timeline"])
            + """;
            ...
            if (riskCtx) {
                const riskData = """
            + json.dumps(charts_data["risk_scores"])
            + """;
