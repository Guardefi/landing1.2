    ...
    for theme in themes:
        if isinstance(theme, dict):
            ...
            description = (
                ...
                if hasattr(theme_details, "description")
                else "No description available"
            )
            display_name = (
                theme_details.display_name
                if hasattr(theme_details, "display_name")
                else theme
            )
...
def create_sample_command(args) -> None:
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(
            (
                sample_scan.model_dump()
                if hasattr(sample_scan, "model_dump")
                else sample_scan.__dict__
            ),
            f,
            indent=2,
            default=str,
...
Examples:
  ...
  python -m reporting.cli themes
    ...
    generate_parser.add_argument(
        "--bundle", "-b", action="store_true", help="Create audit bundle zip file"
    )
    generate_parser.add_argument(
        "--include-source", action="store_true", help="Include source code in bundle"
    )
    generate_parser.add_argument(
        "--sign", action="store_true", help="Include digital signature"
    )
    ...
    diff_parser.add_argument(
        "--output-dir", "-o", type=str, default="reports", help="Output directory"
    )
    diff_parser.add_argument(
        "--format", "-f", type=str, default="html", help="Output format"
    )
    ...
    validate_parser = subparsers.add_parser(
        "validate", help="Validate scan result file"
    )
    validate_parser.add_argument("scan_file", help="Scan result JSON file to validate")

    # Sample command
    sample_parser = subparsers.add_parser(
        "sample", help="Create sample scan result file"
    )
    sample_parser.add_argument(
        "--output", "-o", type=str, help="Output file path (default: sample_scan.json)"
    )
