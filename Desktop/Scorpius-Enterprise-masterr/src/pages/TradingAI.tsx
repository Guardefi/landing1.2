...
const TradingAI = () => {
  ...
  const [mevStats, setMevStats] = useState(() => {
    ...
    return {
      ...
      avgSuccessRate: tradingStats.totalTrades > 0 ? 92.8 : 0,
      gasEfficiency: 2.5,
      protectionLevel: 98.2,
      attacksPrevented: tradingStats.attacksPrevented,
      valueSaved: tradingStats.valueSaved,
    };
  });
  ...
  const fetchLatestTransaction =
    useCallback(async (): Promise<SimulatedTransaction | null> => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL || "http://localhost:8000"}/api/trading/transactions/latest`,
        );
        if (response.ok) {
        ...
        console.error("Failed to fetch trading transaction:", error);
      }
      return null;
    }, []);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        const botsResponse = await fetch(
          `${import.meta.env.VITE_API_BASE_URL || "http://localhost:8000"}/api/trading/bots/status`,
        );
        if (botsResponse.ok) {
          const botsData = await botsResponse.json();
          if (botsData.success && botsData.data) {
            setDeployedBots(botsData.data);
          }
        }

        // Fetch latest transaction
        const newTx = await fetchLatestTransaction();
        if (newTx) {
          setTransactions((prev) => [...prev.slice(-20), newTx]);

          // Persist trading activity if transaction was successful
          if (newTx.success) {
            StorageManager.incrementTradingStats(
              newTx.amount || 0,
              newTx.amount || 0,
            );

            // Update local stats from storage
            const updatedStats = StorageManager.getTradingStats();
            setMevStats((prev) => ({
              ...prev,
              totalTrades: updatedStats.totalTrades,
              totalProfit: updatedStats.totalProfit,
              attacksPrevented: updatedStats.attacksPrevented,
              valueSaved: updatedStats.valueSaved,
            }));
          }
        }

        // Fetch MEV stats from API (will merge with local stats)
        const statsResponse = await fetch(
          `${import.meta.env.VITE_API_BASE_URL || "http://localhost:8000"}/api/trading/mev/stats`,
        );
        if (statsResponse.ok) {
          const statsData = await statsResponse.json();
          if (statsData.success && statsData.data) {
            // Merge API stats with persistent stats
            const localStats = StorageManager.getTradingStats();
            setMevStats((prev) => ({
              ...prev,
              ...statsData.data,
              totalTrades: Math.max(
                prev.totalTrades,
                statsData.data.totalTrades || 0,
              ),
              totalProfit: Math.max(
                prev.totalProfit,
                statsData.data.totalProfit || 0,
              ),
            }));
          }
        }
      } catch (error) {
        console.error("Failed to fetch real-time trading data:", error);
      }
    }, 3000);

    return () => clearInterval(interval);
  }, [isLive, fetchLatestTransaction]);
  ...
  return (
    <PageLayout variant="trading">
      ...
      <motion.div
        ...
        className="flex items-center justify-between mb-6 p-4 bg-background/50 border rounded-lg"
      >
