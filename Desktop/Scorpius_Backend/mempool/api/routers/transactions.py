    ...
    try:
        ...
        async with db.acquire() as conn:
            ...
            tx_data['risk_analysis'] = {
                'score': risk_score,
                'factors': ['unusual_gas_price', 'new_address'] if risk_score > 0.5 else [],
    ...
    try:
        ...
        async with db.acquire() as conn:
            ...
            if not row:
                raise HTTPException(
                    status_code=404,
                    detail="Transaction trace not found"
                )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve transaction trace: {str(e)}"
        )
...
async def get_address_transactions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve address transactions: {str(e)}"
        )
...
async def get_transaction_stats(
    ...
    """
    try:
        time_filter = f"timestamp > extract(epoch from now() - interval '{hours} hours')"
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve transaction stats: {str(e)}"
        )
