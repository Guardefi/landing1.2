...
from ..models import PaginationParams, AlertResponse, AlertSeverity
...
async def update_alert(
    ...
    try:
        ...
        if update_data.status not in ["acknowledged", "dismissed"]:
            raise HTTPException(
                status_code=400,
                detail="Status must be 'acknowledged' or 'dismissed'"
            )
...
async def get_alert_stats(
    ...
    try:
        ...
        async with db.acquire() as conn:
            ...
            # Alerts by severity
            severity_stats = await conn.fetch(f"""
            ...
            # Alerts by chain
            chain_stats = await conn.fetch(f"""
            ...
            # Resolution rate
            resolution_stats = await conn.fetchrow(f"""
                SELECT 
                    COUNT(*) as total,
            ...
            resolution_rate = (
                ...
                if resolution_stats['total'] > 0 else 0
            )
            
            # Average response time (placeholder calculation)
            avg_response_time = await conn.fetchval(f"""
            ...
            return AlertStatsResponse(
                ...
                alerts_by_chain={
                    row['chain_id']: row['count'] for row in chain_stats
                },
...
async def get_live_alert_feed(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve live alert feed: {str(e)}"
        )
