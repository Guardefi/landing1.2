...
class TransactionResponse(BaseModel):
    ...
    risk_score: Optional[float] = Field(None, ge=0, le=1, description="Risk assessment score")
    mev_patterns: list[str] = Field(default_factory=list, description="Detected MEV patterns")
...
class RuleCondition(BaseModel):
    ...
    type: str = Field(..., description="Condition type (e.g., value_threshold, address_filter)")
    field: Optional[str] = Field(None, description="Transaction field to evaluate")
    operator: Optional[str] = Field(None, description="Comparison operator (gt, lt, eq, in, etc.)")
    value: Optional[Union[str, int, float, list]] = Field(None, description="Comparison value")
    addresses: Optional[list[str]] = Field(None, description="List of addresses for address-based conditions")
    chain_ids: Optional[list[int]] = Field(None, description="List of chain IDs for chain filtering")
    
    @validator('type')
    def validate_condition_type(cls, v):
        valid_types = [
            'value_threshold', 'gas_threshold', 'address_whitelist', 
            'address_blacklist', 'chain_filter', 'mev_pattern',
            'contract_interaction', 'token_transfer', 'time_window'
        ]
...
class RuleAction(BaseModel):
    """Rule action model"""
    type: str = Field(..., description="Action type (e.g., create_alert, webhook, email)")
    severity: Optional[AlertSeverity] = Field(None, description="Alert severity for alert actions")
    ...
    email_recipients: Optional[list[str]] = Field(None, description="Email recipients for email actions")
...
class RuleRequest(BaseModel):
    ...
    conditions: list[RuleCondition] = Field(..., min_items=1, description="Rule conditions")
...
class RuleResponse(BaseModel):
    ...
    trigger_count: Optional[int] = Field(None, description="Number of times rule has triggered")
    last_triggered: Optional[datetime] = Field(None, description="When rule last triggered")
...
class MEVOpportunityResponse(BaseModel):
    ...
    transaction_hash: str = Field(..., description="Associated transaction hash")
    chain_id: int = Field(..., description="Blockchain chain ID")
    mev_type: str = Field(..., description="Type of MEV (sandwich, arbitrage, liquidation, etc.)")
    estimated_profit: str = Field(..., description="Estimated profit in wei")
    confidence_score: float = Field(..., ge=0, le=1, description="Confidence in MEV detection")
    block_number: int = Field(..., description="Block number")
    timestamp: datetime = Field(..., description="When opportunity was detected")
    involved_addresses: list[str] = Field(default_factory=list, description="Addresses involved in MEV")
    metadata: dict[str, Any] = Field(default_factory=dict, description="Additional MEV metadata")
...
class UserResponse(BaseModel):
    ...
    created_at: Optional[datetime] = Field(None, description="When user account was created")
...
class SystemStatsResponse(BaseModel):
    ...
    alerts_by_severity: dict[str, int] = Field(..., description="Alert counts by severity")
    top_chains_by_volume: list[dict[str, Any]] = Field(..., description="Top chains by transaction volume")
