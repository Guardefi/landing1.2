...
class RustArbitrageEngine:
    ...
    async def find_arbitrage_paths(
        ...
        try:
            ...
            logger.info(
                f"Found {len(paths)} arbitrage paths in {execution_time_ms:.2f}ms"
            )
            return paths
    ...
    async def validate_arbitrage_path(
        ...
        try:
            input_data = {
                ...
                "config": {"slippage_tolerance": self.config.slippage_tolerance},
            }
    ...
    async def _execute_rust_engine(
        ...
        output_file = (
            self.temp_dir / f"output_{operation}_{int(time.time() * 1000)}.json"
        )
        ...
        try:
            ...
            try:
                ...
                await process.wait()
                raise Exception(
                    f"Rust engine execution timed out after {self.config.max_execution_time_seconds}s"
                ) from None
            ...
            if process.returncode != 0:
                ...
                raise Exception(
                    f"Rust engine failed with code {process.returncode}: {error_msg}"
                ) from None
