...
class ChainRegistry:
    def __init__(self, config_path=DEFAULT_CHAIN_CONFIG_PATH):
        ...
        try:
            ...
            logger.info(
                f"ChainRegistry initialized. Loaded {len(self.chains)} chain configs from '{config_path}'."
            )
            ...
            if filename.endswith(".json"):
                ...
                try:
                    with open(file_path) as f:
                        ...
                        chain_id_str = chain_config.get("chainId") or chain_config.get(
                            "chain_id"
                        )
                        if chain_id_str:
                    ...
                    logger.error(
                        f"Error loading chain config {filename}: {e}", exc_info=True
                    )
    ...
    def get_rpc_url(self, chain_id: int) -> str | None:
        ...
        if rpc_url:
            return rpc_url
        logger.error(
            f"Could not determine RPC URL for chain ID {chain_id} (Name: {chain_config.get('name', 'N/A')})."
        )
        return None
    ...
    def add_chain(self, chain_config: dict) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error saving chain config file {filepath}: {e}", exc_info=True
            )
...
class ProtocolRegistry:
    def __init__(self, config_path=DEFAULT_PROTOCOL_CONFIG_PATH):
        ...
        try:
            ...
            logger.info(
                f"ProtocolRegistry initialized. Loaded {len(self.protocols)} protocol configs from '{self.config_path}'."
            )
        except Exception as e:
        ...
        for filename in os.listdir(self.config_path):
            if filename.endswith(".json"):
                ...
                try:
                    ...
                    logger.error(
                        f"Error loading protocol config {filename}: {e}", exc_info=True
                    )
    ...
    def add_protocol(self, protocol_config: dict) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error saving protocol config file {filepath}: {e}", exc_info=True
            )
...
from web3 import Web3
    ...
    async def initialize(self, event_bus, ml_model):
        ...
        for chain_id, result in zip(enabled_chains, results, strict=False):
            if isinstance(result, Exception):
                self.logger.error(
                    f"Failed monitor for chain {chain_id}: {result}", exc_info=result
                )
            elif not result:
                self.logger.error(
    ...
    async def start_chain_monitor(self, chain_id: int) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to start monitor for chain {chain_id}: {e}", exc_info=True
            )
            return False
