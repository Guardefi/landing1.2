...
class LiquidationMonitor:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"Aave V3 contracts loaded: pool={self.aaveV3['poolAddress']} oracle={self.aaveV3['oracleAddress']}"
            )
    ...
    async def find_liquidatable_positions(self) -> list[dict]:
        ...
        logger.info(
            f"Checking {len(risky_users)} addresses for liquidation potential..."
        )
        ...
        if liquidatable_ops:
            logger.info(
                f"Found {len(liquidatable_ops)} potentially profitable Aave liquidations."
            )
        else:
            logger.info("No profitable Aave V3 liquidations found.")

        logger.debug(
            f"Liquidation scan completed in {time.monotonic()-start_time:.2f}s"
        )
