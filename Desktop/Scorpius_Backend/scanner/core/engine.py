...
from core.models import (
    ScanResult, ScanStatus, ScanType, VulnerabilityLevel,
    VulnerabilityFinding, Target, ScanConfig
)
from core.plugin_manager import PluginManager
from sandbox import SandboxManager, SandboxType
from reporting import generate_report, ReportFormat
...
class ScanEngine:
    ...
    async def start_scan(self, target: Target, scan_type: ScanType, 
                        config: Optional[ScanConfig] = None) -> str:
        """
        ...
        try:
            ...
            if self.config.get("sandbox_enabled", True):
                ...
                if not any(sandbox_available.values()):
                    logger.warning("No sandbox environments available, continuing without sandbox")
                    scan_result.warnings = ["No sandbox environments available"]
            
            # Determine which plugins to use based on scan type
            plugins_to_run = self._select_plugins(scan_result.scan_type, scan_result.config)
            ...
            if self.config.get("sandbox_enabled", True):
                for plugin_name, plugin in plugins_to_run.items():
                    ...
                    if ScanType.DYNAMIC in plugin.SUPPORTED_SCAN_TYPES or scan_result.scan_type == ScanType.DYNAMIC:
                        try:
                            sandbox_id = await self.create_sandbox_for_plugin(plugin_name, plugin)
                            if sandbox_id:
                                sandboxes[plugin_name] = sandbox_id
                                logger.info(f"Created sandbox {sandbox_id} for plugin {plugin_name}")
                        except Exception as e:
            ...
            for plugin_name, plugin in plugins_to_run.items():
                try:
                    ...
                    custom_config = scan_result.config.copy() if scan_result.config else ScanConfig()
                    if plugin_name in sandboxes:
                        custom_config.custom_options["sandbox_id"] = sandboxes[plugin_name]
                    
                    # Run the scan
                    plugin_findings = await plugin.scan(scan_result.target, custom_config)
                    
                    # Filter findings
                    enriched_findings = self._enrich_findings(plugin_findings, plugin_name)
                    all_findings.extend(enriched_findings)
                    
                    complete_plugins += 1
                    scan_result.progress = int((complete_plugins / total_plugins) * 100)
                    
                    logger.info(f"Plugin {plugin_name} completed with {len(plugin_findings)} findings")
                except Exception as e:
            ...
            logger.info(f"Scan {scan_id} completed successfully with {len(scan_result.findings)} findings in {scan_result.scan_duration:.2f} seconds")
    ...
    def _select_plugins(self, scan_type: ScanType, config: ScanConfig) -> Dict[str, Any]:
    ...
    def _enrich_findings(self, findings: List[VulnerabilityFinding], 
                         plugin_name: str) -> List[VulnerabilityFinding]:
    ...
    def _process_findings(self, findings: List[VulnerabilityFinding]) -> List[VulnerabilityFinding]:
    ...
    async def batch_scan(self, targets: List[Target], scan_type: ScanType, 
                         config: Optional[ScanConfig] = None) -> List[str]:
        """
    ...
    async def schedule_scan(self, target: Target, scan_type: ScanType,
                           schedule: Dict[str, Any], config: Optional[ScanConfig] = None) -> str:
        """
    ...
    async def create_sandbox_for_plugin(self, plugin_name: str, plugin: Any) -> Optional[str]:
        """
        ...
        try:
            ...
            if "slither" in plugin_name.lower():
                ...
                sandbox_options["command"] = ["tail", "-f", "/dev/null"]  # Keep container running
    ...
    async def generate_scan_reports(self, scan_id: str, scan_result: ScanResult) -> List[str]:
        """
        ...
        report_data = {
            ...
            "plugins_used": [scan_result.current_plugin] if scan_result.current_plugin else [],
        ...
        reports_dir = os.path.join(os.getcwd(), self.config.get("report_directory", "reports"))
        ...
        if scan_result.config.report_formats and ReportFormat.HTML.value in scan_result.config.report_formats:
            html_path = generate_report(report_data, f"{base_filename}", ReportFormat.HTML)
            report_files.append(html_path)
            
        # Generate Markdown report if requested
        if scan_result.config.report_formats and ReportFormat.MARKDOWN.value in scan_result.config.report_formats:
            md_path = generate_report(report_data, f"{base_filename}", ReportFormat.MARKDOWN)
