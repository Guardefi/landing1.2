...
class AccessControlStrategy(BaseStrategy):
    ...
    async def _find_unprotected_functions(self, context: StrategyContext) -> List[VulnerabilityFinding]:
                ...
                if func_content:
                    ...
                    if not has_access_control:
                        ...
                        finding = self.create_finding(
                            ...
                            exploit_scenario=self._generate_access_control_exploit(func_name),
                            remediation=self._generate_access_control_remediation(func_name)
                        )
                        findings.append(finding)
        
        return findings
    
    async def _check_initialization_vulnerabilities(self, context: StrategyContext) -> List[VulnerabilityFinding]:
        """Check for initialization function vulnerabilities"""
        findings = []
        source_code = context.source_code
        
        # Find initialization functions
        init_pattern = r'function\s+(initialize|init|setup)\s*\([^)]*\)\s*([^{]*)\s*\{([^}]+)\}'
        ...
        for match in re.finditer(init_pattern, source_code, re.IGNORECASE | re.DOTALL):
            ...
            if not re.search(r'initialized|_initialized|initializer', func_body, re.IGNORECASE):
            ...
            if any(re.search(pattern, func_body, re.IGNORECASE) for pattern in self.state_change_patterns):
            ...
            if vulnerability_issues:
                finding = self.create_finding(
                    ...
                    confidence=0.8,
                    affected_functions=[func_name],
                    exploit_scenario=self._generate_init_exploit(func_name, vulnerability_issues),
    ...
    async def _check_rbac_vulnerabilities(self, context: StrategyContext) -> List[VulnerabilityFinding]:
        """Check for role-based access control vulnerabilities"""
        findings = []
        source_code = context.source_code
        
        # Check for role assignment functions without proper protection
        role_assignment_pattern = r'function\s+(\w*grant\w*|\w*assign\w*|\w*add\w*)\s*\([^)]*role[^)]*\)'
    ...
    def _extract_function_content(self, source_code: str, start_pos: int) -> Optional[str]:
    ...
    def _determine_severity(self, func_name: str, func_content: str) -> str:
        ...
        # Critical functions
        if any(keyword in func_name_lower for keyword in ['withdraw', 'transfer', 'mint', 'burn', 'selfdestruct']):
            return "Critical"
        
        # High risk functions
        if any(keyword in func_name_lower for keyword in ['admin', 'owner', 'emergency', 'pause', 'upgrade']):
            return "High"
        
        # Check for dangerous operations in content
        if any(keyword in func_content_lower for keyword in ['selfdestruct', 'delegatecall', 'suicide']):
            return "Critical"
