...
async def start_scan(request: ScanRequest):
    ...
    try:
        ...
        asyncio.create_task(run_slither_scan(scan_id, request.target_path, request.options))
...
async def run_slither_scan(scan_id: str, target_path: str, options: Dict[str, Any]):
    ...
    try:
        ...
        if process.returncode == 0 or stdout:
                ...
                if "results" in output and "detectors" in output["results"]:
                    for detector in output["results"]["detectors"]:
                        finding = {
                            ...
                            "severity": map_slither_impact(detector.get("impact", "Low")),
                ...
                scan_results[scan_id].update({
                    ...
                    "metadata": {
                        **scan_results[scan_id]["metadata"],
                        ...
                        "detectors_run": len(output.get("results", {}).get("detectors", [])),
                        "compilation_success": True
                    }
                })
                
                logger.info(f"Slither scan {scan_id} completed with {len(findings)} findings")
            ...
            logger.error(f"Slither scan {scan_id} failed with return code {process.returncode}")
