...
async def list_marketplace_plugins(
    ...
    if category:
        plugins = [p for p in plugins if category.lower() in [tag.lower() for tag in p.manifest.tags]]
...
async def uninstall_plugin(plugin_id: str):
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Plugin {plugin_config['name']} uninstalled successfully"
        }
...
async def upload_plugin(
    ...
    try:
        ...
        # Generate plugin ID
        plugin_id = f"{manifest.name.lower().replace(' ', '-')}-{manifest.author.lower().replace(' ', '-')}"
        
        # Handle Dockerfile if provided
        dockerfile_content = None
        if dockerfile:
            dockerfile_content = (await dockerfile.read()).decode()
