...
from models.scheduler_models import (  # Pydantic models; Enums; Database models
    ClusterResponse,
    ClusterStatus,
    ComputeCluster,
    ExecutionResponse,
...
async def get_scheduler_health(
    ...
    try:
        ...
        active_jobs_result = await db.execute(
            select(func.count(ScheduledJob.id)).filter(
        ...
        clusters_result = await db.execute(
            select(func.count(ComputeCluster.id)).filter(
                ComputeCluster.status == ClusterStatus.ONLINE
            )
        )
        ...
        executions_result = await db.execute(
            select(func.count(JobExecution.id)).filter(
                func.date(JobExecution.start_time) == today
            )
        )
...
async def get_scheduled_job(
    job_id: str = Path(..., description="Job ID"), db: AsyncSession = Depends(get_db)
) -> JobResponse:
    """
...
async def create_scheduled_job(
    job_data: JobCreate, db: AsyncSession = Depends(get_db)
) -> JobResponse:
    """
...
async def trigger_job_execution(
    ...
    try:
        ...
        if job.status != JobStatus.ACTIVE:
            raise HTTPException(
                status_code=400, detail=f"Job {job_id} is not active"
            ) from e
...
async def get_job_executions(
    ...
    limit: int = Query(
        50, le=100, description="Maximum number of executions to return"
    ),
...
async def get_compute_clusters(
    status: ClusterStatus | None = Query(None, description="Filter by cluster status"),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch clusters: {str(e)}"
        )
...
async def get_compute_cluster(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch cluster: {str(e)}"
        )
...
async def get_resource_usage(
    ...
    try:
        ...
        if resource_type:
            query = query.filter(
                ResourceUsage.resource_type.ilike(f"%{resource_type}%")
            )
        ...
        return [
            ResourceUsageResponse.model_validate(resource) for resource in resources
        ]

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch resource usage: {str(e)}"
        )
