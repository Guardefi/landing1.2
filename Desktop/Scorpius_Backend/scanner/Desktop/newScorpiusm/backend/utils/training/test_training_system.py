...
class TrainingSystemTester:
                ...
                result.update(
                    {
                        ...
                        "modules": [
                            m["name"] for m in modules[:3]
                        ],  # First 3 module names
                    }
                )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if len(found_key_badges) >= 8:
                    print(
                        f"✅ Found {len(found_key_badges)}/{len(key_badges)} key badges"
                    )
                else:
                    print(
                        f"⚠️ Only found {len(found_key_badges)}/{len(key_badges)} key badges"
                    )
    ...
    async def test_leaderboard_endpoint(self) -> dict[str, Any]:
        ...
        try:
            response = await self.client.get(
                f"{self.base_url}/api/training/leaderboard"
            )
            result = {
    ...
    async def test_courses_for_module(self, module_id: str) -> dict[str, Any]:
        ...
        try:
            response = await self.client.get(
                f"{self.base_url}/api/training/modules/{module_id}/courses"
            )
            result = {
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ Courses endpoint failed for module {module_id}: {response.status_code}"
                )
    ...
    async def test_user_progress(self, user_id: str) -> dict[str, Any]:
        ...
        try:
            response = await self.client.get(
                f"{self.base_url}/api/training/users/{user_id}/progress"
            )
            result = {
    ...
    async def test_user_badges(self, user_id: str) -> dict[str, Any]:
        ...
        try:
            response = await self.client.get(
                f"{self.base_url}/api/training/users/{user_id}/badges"
            )
            result = {
    ...
    async def run_comprehensive_test(self) -> dict[str, Any]:
        ...
        if modules_result.get("success") and "modules_count" in modules_result:
            ...
            modules_response = await self.client.get(
                f"{self.base_url}/api/training/modules"
            )
            if modules_response.status_code == 200:
                modules = modules_response.json()
                if modules:
                    first_module_id = modules[0]["id"]
                    test_results["tests"]["courses"] = (
                        await self.test_courses_for_module(first_module_id)
                    )
