...
class SchedulerSystemTester:
    ...
    async def test_get_job_executions(self) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/api/scheduler/executions"
            ) as response:
                if response.status == 200:
    ...
    async def test_get_system_metrics(self) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/api/scheduler/metrics"
            ) as response:
                if response.status == 200:
    ...
    async def test_get_compute_clusters(self) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/api/scheduler/clusters"
            ) as response:
                if response.status == 200:
    ...
    async def test_get_resource_usage(self) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/api/scheduler/resources"
            ) as response:
                if response.status == 200:
    ...
    async def test_create_job(self) -> bool:
        ...
        job_data = {
            ...
            "timeout_minutes": 30,
            "targets": 50,
            "config": {"scan_types": ["basic", "advanced"], "depth": "normal"},
            "environment_vars": {"TEST_MODE": "true"},
        }

        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{self.base_url}/api/scheduler/jobs", json=job_data
            ) as response:
                if response.status == 200:
    ...
    async def test_job_filtering(self) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                f"{self.base_url}/api/scheduler/jobs?status=active"
            ) as response:
                if response.status != 200:
                    return False
                jobs = await response.json()
    ...
    async def test_trigger_job_execution(self) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/api/scheduler/jobs?status=active&limit=1"
            ) as response:
                if response.status != 200:
                    return False
                jobs = await response.json()
                ...
                # Trigger execution
                async with session.post(
                    f"{self.base_url}/api/scheduler/jobs/{job_id}/trigger"
                ) as response:
                    if response.status == 200:
    ...
    async def test_cluster_status_filtering(self) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/api/scheduler/clusters?status=online"
            ) as response:
                if response.status == 200:
                    clusters = await response.json()
    ...
    async def test_resource_usage_filtering(self) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/api/scheduler/resources?resource_type=CPU"
            ) as response:
                if response.status == 200:
                    resources = await response.json()
                    return isinstance(resources, list) and all(
                        "CPU" in resource.get("resource_type", "")
    ...
    async def test_job_details(self) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/api/scheduler/jobs?limit=1"
            ) as response:
                if response.status != 200:
                    return False
                jobs = await response.json()
                ...
                # Get job details
                async with session.get(
                    f"{self.base_url}/api/scheduler/jobs/{job_id}"
                ) as response:
                    if response.status == 200:
                        job = await response.json()
                        required_fields = [
    ...
    async def test_execution_details(self) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/api/scheduler/executions?limit=1"
            ) as response:
                if response.status != 200:
                    return False
                executions = await response.json()
                ...
                async with session.get(
                    f"{self.base_url}/api/scheduler/executions/{execution_id}"
                ) as response:
                    if response.status == 200:
    ...
    async def run_all_tests(self):
        ...
        await self.run_test(
            "5️⃣ Testing Compute Clusters", self.test_get_compute_clusters
        )
        ...
        await self.run_test(
            "9️⃣ Testing Job Execution Trigger", self.test_trigger_job_execution
        )

        # Advanced functionality tests
        await self.run_test(
            "🔟 Testing Cluster Status Filtering", self.test_cluster_status_filtering
        )
        await self.run_test(
            "1️⃣1️⃣ Testing Resource Usage Filtering", self.test_resource_usage_filtering
        )
        ...
        print(
            f"✅ Successful Tests: {self.test_results['passed_tests']}/{self.test_results['total_tests']}"
        )
