    ...
    try:
        ...
        if result.get("honeypot_detections"):
            ...
            for detection in result["honeypot_detections"][:2]:
                print(
                    f"    - {detection.get('type', 'Unknown')}: {detection.get('description', 'N/A')}"
                )
...
async def test_quick_analysis():
    ...
    for target in test_targets:
        try:
            ...
            print(
                f"  Honeypot Detected: {'🚨 YES' if result.get('honeypot_detected', False) else '✅ NO'}"
            )
            print(f"  Confidence: {result.get('confidence', 0.0):.2f}")
...
async def test_batch_analysis():
    ...
    try:
        ...
        print(
            f"  Honeypots Detected: {results.get('summary', {}).get('honeypots_detected', 0)}"
        )
        print(
            f"  High Confidence: {results.get('summary', {}).get('high_confidence', 0)}"
        )
        ...
        for result in results.get("results", []):
            ...
            print(
                f"    {status_icon} {result.get('target', 'unknown')}: "
...
async def test_scorpius_integration():
    ...
    try:
        ...
        print(
            f"    Honeypots Detected: {result.get('summary', {}).get('total_honeypots_detected', 0)}"
        )
        print(
            f"    Detection Rate: {result.get('summary', {}).get('detection_rate', 0.0):.1f}%"
        )
        print(
            f"    Analysis Time: {result.get('summary', {}).get('analysis_time', 0.0):.2f}s"
        )
...
async def test_edge_cases():
    ...
    try:
        ...
        print(
            f"    Empty batch result: {result.get('summary', {}).get('targets_analyzed', 0)} targets"
        )
    except Exception as e:
