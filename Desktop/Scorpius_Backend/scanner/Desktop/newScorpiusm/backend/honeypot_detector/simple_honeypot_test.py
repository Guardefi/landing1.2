...
import honeypot_detector
        ...
        batch_results = await detector.batch_analyze(
            targets=batch_targets, quick_scan=True, max_concurrent=1
        )

        print("  Batch Results:")
        print(f"  Targets Analyzed: {batch_results.get('targets_analyzed', 0)}")
        print(
            f"  Honeypots Detected: {batch_results.get('summary', {}).get('honeypots_detected', 0)}"
        )
        print(
            f"  Total Time: {batch_results.get('summary', {}).get('total_time', 0.0):.2f}s"
        )
