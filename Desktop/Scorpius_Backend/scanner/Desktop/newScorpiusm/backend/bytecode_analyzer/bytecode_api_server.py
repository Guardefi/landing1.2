...
async def analyze_bytecode_similarity(request: BytecodeSimilarityRequest):
    ...
    try:
        ...
        logger.info(
            f"Analysis complete. Risk score: {results.get('risk_score', 'N/A')}"
        )
        logger.info(
            f"Similarity matches found: {len(results.get('similarity_matches', []))}"
        )
        logger.info(
            f"Vulnerability patterns detected: {len(results.get('vulnerability_patterns', []))}"
        )
