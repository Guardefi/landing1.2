...
async def test_bytecode_similarity_engine():
    ...
    print(
        f"✅ Engine initialized with {len(engine.reference_patterns)} reference patterns"
    )
    ...
    try:
        ...
        for vuln in results["vulnerability_patterns"]:
            print(
                f"   • {vuln['pattern_type']}: {vuln['severity']} ({vuln['confidence']:.2f})"
            )
        ...
        if opcode_analysis.get("suspicious_opcodes"):
            print(
                f"   • Suspicious Opcodes: {', '.join(opcode_analysis['suspicious_opcodes'])}"
            )
...
async def test_convenience_function():
    ...
    try:
        # Test the convenience function
        quick_results = await analyze_contract_bytecode(
...
from modules.bytecode_similarity_engine import (  # Test 1: Main engine functionality
    1,
    2,
    +=,
    :,
    ...
    if,
    success_count,
    test_bytecode_similarity_engine,
    total_tests,
)
    ...
    if success_count == total_tests:
        print(
            "🎉 ALL TESTS PASSED! Bytecode similarity engine is ready for production."
        )
    else:
