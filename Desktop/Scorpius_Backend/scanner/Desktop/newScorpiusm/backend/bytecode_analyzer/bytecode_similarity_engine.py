...
class BytecodeSimilarityEngine:
    ...
    async def analyze_bytecode_similarity(
        ...
        """
        logger.info(
            f"Starting comprehensive bytecode similarity analysis for {len(bytecode)} bytes"
        )
        ...
        results["similarity_matches"] = await self._analyze_pattern_similarity(
            clean_bytecode
        )

        # Vulnerability pattern detection
        if include_vulnerability_patterns:
            results["vulnerability_patterns"] = (
                await self._detect_vulnerability_patterns(clean_bytecode)
            )
    ...
    async def _analyze_pattern_similarity(self, bytecode: str) -> list[dict[str, Any]]:
        ...
        for pattern_name, pattern_code in self.reference_patterns.items():
            ...
            if similarity_ratio > 0.3:  # Only analyze diffs for meaningful similarities
                diff_analysis = await self._calculate_detailed_diff(
                    bytecode, pattern_code
                )
            ...
            similarities.append(
                {
                    ...
                    "confidence": self._calculate_confidence(
                        similarity_ratio, len(bytecode), len(pattern_code)
                    ),
    ...
    async def _calculate_detailed_diff(
        ...
        def run_diff():
                ...
                diff_blocks.append(
                    {
                        ...
                        "target_range": [j1, j2],
                        "source_content": (
                            bytecode1[i1:i2] if i1 < len(bytecode1) else ""
                        ),
                        "target_content": (
                            bytecode2[j1:j2] if j1 < len(bytecode2) else ""
                        ),
                    }
                )
            ...
            return {
                ...
                "total_changes": len(
                    [block for block in diff_blocks if block["operation"] != "equal"]
                ),
    ...
    async def _find_pattern_matches(
        ...
        def find_matches():
            ...
            while True:
                ...
                if pos == -1:
                    break
                matches.append({"position": pos, "length": len(pattern)})
                start = pos + 1
            return matches
    ...
    async def _analyze_opcodes(self, bytecode: str) -> dict[str, Any]:
        ...
        def analyze_opcodes_sync():
            ...
            complexity_score = (
                unique_opcodes / total_opcodes if total_opcodes > 0 else 0
            )
            ...
            return {
                ...
                "opcode_distribution": dict(
                    sorted(named_opcodes.items(), key=lambda x: x[1], reverse=True)[:20]
                ),
    ...
    def _calculate_confidence(
        self, similarity_ratio: float, len1: int, len2: int
    ) -> float:
...
async def analyze_contract_bytecode(
    bytecode: str, engine: BytecodeSimilarityEngine | None = None
) -> dict[str, Any]:
    """
