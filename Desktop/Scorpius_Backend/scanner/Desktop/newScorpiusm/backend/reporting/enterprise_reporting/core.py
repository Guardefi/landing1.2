...
class ReportEngine:
    ...
    def _register_template_functions(self) -> None:
        ...
        def format_datetime(
            value: datetime, format_str: str = "%Y-%m-%d %H:%M:%S"
        ) -> str:
    ...
    async def generate_report(
        ...
        try:
            ...
            output_path = await self._generate_output(
                rendered_content, job.format, job_id, job.config
            )
            ...
            if job.watermark_config:
                output_path = await self._add_watermark(
                    output_path, job.watermark_config
                )

            if job.signature_required:
                output_path = await self._add_signature_block(
                    output_path, job.signature_data
                )
            ...
            await self._log_audit_event(
                job_id, "report_generation_failed", {"error": str(e)}
            )
            raise e from e

    async def _load_job(self, job_id: str) -> ReportJob | None:
    ...
    async def _load_template(self, template_id: str) -> ReportTemplate | None:
    ...
    async def _render_template(
        self, template: ReportTemplate, context: ReportContext, config: dict[str, Any]
    ) -> str:
        """
    ...
    async def _generate_output(
        self, content: str, format: str, job_id: str, config: dict[str, Any]
    ) -> Path:
        """
        ...
        if format == "html":
            ...
            if not WEASYPRINT_AVAILABLE and not PLAYWRIGHT_AVAILABLE:
                raise RuntimeError(
                    "Neither WeasyPrint nor Playwright available for PDF generation"
                )
    ...
    async def _generate_pdf_playwright(
        self, html_content: str, output_path: Path, config: dict[str, Any]
    ) -> None:
    ...
    async def _convert_to_csv(self, content: str, config: dict[str, Any]) -> str:
        ...
        # Write headers
        writer.writerow(
            ["ID", "Severity", "Title", "Description", "Function", "Risk Score"]
        )
    ...
    async def _add_watermark(
        self, file_path: Path, watermark_config: dict[str, Any]
    ) -> Path:
    ...
    async def _add_signature_block(
        self, file_path: Path, signature_data: dict[str, Any]
    ) -> Path:
    ...
    async def _update_job_completion(
        self, job_id: str, file_path: Path, file_hash: str, file_size: int
    ) -> None:
    ...
    async def _log_audit_event(
        self, job_id: str, action: str, details: dict[str, Any], user_id: str = "system"
    ) -> None:
        """Log audit event"""
...
class ReportBuilder:
    ...
    async def create_draft_report(
        self, template_id: str, title: str, config: dict[str, Any], user_id: str
    ) -> str:
        """
    ...
    async def update_report_config(
        self, job_id: str, config: dict[str, Any], user_id: str
    ) -> bool:
        """
...
class ReportExporter:
    ...
    async def generate_signed_url(
        self, job_id: str, expires_in: int = 3600, recipient_email: str | None = None
    ) -> str:
        """
    ...
    async def revoke_access(self, job_id: str, user_id: str) -> bool:
        ...
        # Log revocation
        await self.engine._log_audit_event(job_id, "access_revoked", {}, user_id)
