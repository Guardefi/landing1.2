...
async def list_report_templates(
    ...
    if active_only:
        query = query.where(ReportTemplate.is_active)
...
async def create_report_job(
    ...
    try:
        ...
        background_tasks.add_task(
            generate_report_background, job.id, template, job_request.parameters
        )
...
async def download_report(
    ...
    if job.status != ReportStatus.COMPLETED:
        raise HTTPException(
            status_code=400, detail="Report not ready for download"
        ) from e

    if not job.output_path or not Path(job.output_path).exists():
        raise HTTPException(status_code=404, detail="Report file not found") from e
...
async def publish_report(
    ...
    if job.status != ReportStatus.COMPLETED:
        raise HTTPException(
            status_code=400, detail="Report not ready for publishing"
        ) from e
    ...
    try:
        ...
        return {
            ...
            + timedelta(days=publish_request.expires_days),
        }

    except Exception as e:
        await db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Failed to publish report: {str(e)}"
        )
...
async def compare_reports(
    ...
    try:
        ...
        if not base_job or not compare_job:
            raise HTTPException(
                status_code=404, detail="One or both jobs not found"
            ) from e

        if (
            base_job.status != ReportStatus.COMPLETED
            or compare_job.status != ReportStatus.COMPLETED
        ):
            raise HTTPException(
                status_code=400, detail="Both reports must be completed"
            ) from e
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to compare reports: {str(e)}"
        )
...
@router.get("/widgets")
async def list_widgets(current_user: dict = Depends(get_current_user)):
    """
...
async def portal_access(access_token: str, db: AsyncSession = Depends(get_db)):
    """
    ...
    try:
        ...
        if not report_data:
            raise HTTPException(
                status_code=404, detail="Invalid or expired access token"
            ) from e
        ...
        # Return portal HTML
        return HTMLResponse(content=generate_portal_html(report_data), status_code=200)
...
async def generate_report_background(
    job_id: str, template: ReportTemplate, parameters: dict[str, Any]
):
    """
