...
async def start_scan(request: ScanRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if response.status == ScanStatus.FAILED:
            raise HTTPException(status_code=500, detail=response.message) from e
...
async def download_report(
    scan_id: str, report_type: str, db: AsyncSession = Depends(get_db)
):
    """
    ...
    try:
        if report_type not in ["html", "pdf", "json"]:
            raise HTTPException(
                status_code=400, detail="Invalid report type. Use: html, pdf, json"
            ) from e
        ...
        if not report_path or not Path(report_path).exists():
            raise HTTPException(
                status_code=404, detail=f"{report_type.upper()} report not found"
            )
        ...
        return FileResponse(
            path=report_path, media_type=media_types[report_type], filename=filename
        )
...
async def get_scan_history(
    ...
    try:
        ...
        return ScanHistoryResponse(
            scans=scan_data, total=total, page=page, per_page=per_page
        )
...
async def get_scan_stats(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        completed_scans = sum(
            1 for s in scans if s.status == ScanStatus.COMPLETED.value
        )
        ...
        avg_scan_duration = (
            sum(s.scan_duration for s in completed_with_duration)
...
async def delete_scan(
    scan_id: str, delete_reports: bool = False, db: AsyncSession = Depends(get_db)
):
    """
            ...
            for report_path in [
                ...
                if report_path and Path(report_path).exists():
                    try:
                        ...
                        logger.warning(
                            f"Failed to delete report file {report_path}: {e}"
                        )
