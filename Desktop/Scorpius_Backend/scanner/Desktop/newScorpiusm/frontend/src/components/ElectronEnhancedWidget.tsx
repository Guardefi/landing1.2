...
declare global {
  interface Window {
    ...
    electronPerformance?: {
      startPerformanceMonitoring: (callback: (data: PerformanceData) => void) => void;
    };
  }
}
...
const ElectronEnhancedWidget: React.FC = () => {
  ...
  const getMemoryUsagePercentage = (): number => {
    ...
    return (
      (performanceData.systemMemory.used / performanceData.systemMemory.total) * 100
    );
  };
          ...
          {performanceData && (
            <div className="space-y-3">
              <div className="space-y-2">
                ...
                <Progress value={getCpuUsagePercentage()} className="h-1 bg-gray-800" />
              </div>
