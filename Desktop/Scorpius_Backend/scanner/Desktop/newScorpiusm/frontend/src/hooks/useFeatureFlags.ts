...
class FeatureFlagService {
  ...
  private tierHierarchy: UserTier[] = ['community', 'starter', 'pro', 'enterprise'];
  ...
  private hasRequiredTier(userTier: UserTier, requiredTier: UserTier | 'any'): boolean {
  ...
  checkFeatureLimits(
    ...
    if (!this.isFeatureEnabled(featureName, userTier, userPermissions)) {
      const requiredTierIndex = this.getTierLevel(flag.tierRequired as UserTier);
    ...
    if (flag.limits && usage) {
      if (flag.limits.dailyUsage && usage.dailyUsage >= flag.limits.dailyUsage) {
        return {
          allowed: false,
  ...
  async recordFeatureUsage(featureName: string, authToken: string): Promise<void> {
    try {
...
export const useFeatureFlags = () => {
  ...
  const getFeatureFlag = useCallback((name: string): FeatureFlag | undefined => {
