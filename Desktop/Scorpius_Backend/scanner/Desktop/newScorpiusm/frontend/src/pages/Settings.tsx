    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        toast({
          ...
          description:
            result.message || `Failed to connect to ${type}. Check your configuration.`,
          variant: 'destructive',
        });
      }
    } catch (error) {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h1 className="text-3xl font-bold text-red-400">SYSTEM CONFIGURATION</h1>
              ...
              <CardContent className="space-y-6">
                {Object.entries(rpcConfig).map(([network, url]) => (
                  <div key={network} className="space-y-2">
                    <Label className="text-red-400 capitalize">{network} RPC URL</Label>
              ...
              <CardContent className="space-y-6">
                {Object.entries(apiConfig).map(([service, key]) => (
                  <div key={service} className="space-y-2">
                    <Label className="text-red-400 capitalize">{service} API Key</Label>
                    <div className="flex gap-2">
              ...
              <CardContent className="space-y-4">
                ...
                {notificationConfig.slack.enabled && (
                  <div className="space-y-4">
                    ...
                    <Button
                      variant="outline"
                      onClick={() => testConnection('Slack', notificationConfig.slack)}
                      className="bg-black/50 border-red-500/30 text-red-400 hover:bg-red-500/10"
                      disabled={isLoading}
                    >
                      Test Slack
