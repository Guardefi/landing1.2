...
// Reducer
function teamChatReducer(state: TeamChatState, action: TeamChatAction): TeamChatState {
  switch (action.type) {
    ...
    case 'MARK_ROOM_READ':
      return {
        ...
        unreadTotal: state.rooms.reduce(
          (total, room) => total + (room.id === action.payload ? 0 : room.unreadCount),
          0,
        ),
      };
...
interface TeamChatContextType extends TeamChatState {
  sendMessage: (roomId: string, content: string, type?: ChatMessage['type']) => void;
...
export function TeamChatProvider({ children }: TeamChatProviderProps) {
  ...
  const canAccessFeature = (feature: string): boolean => {
    ...
    const rolePermissions = {
      admin: ['*'],
      manager: ['invite', 'remove', 'update_roles', 'create_rooms', 'delete_rooms'],
