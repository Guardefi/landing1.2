...
class AttackConfig:
    ...
    enabled_strategies: List[str] = field(default_factory=lambda: [
        "flashloan_arbitrage", "sandwich_attack", "liquidation_bot"
    ])
    
    # Flash Loan Providers
    flashloan_providers: List[str] = field(default_factory=lambda: [
...
class MevGuardianConfig:
    ...
    def from_env(cls) -> 'MevGuardianConfig':
        ...
        database = DatabaseConfig(
            url=os.getenv('DATABASE_URL', 'postgresql://user:pass@localhost/mev_guardian'),
        ...
        guardian = GuardianConfig(
            threat_confidence_threshold=float(os.getenv('THREAT_DETECTION_THRESHOLD', '0.8')),
    ...
    def validate(self) -> List[str]:
        ...
        if self.mode in [OperatingMode.GUARDIAN, OperatingMode.HYBRID]:
            if not self.chains:
                errors.append("At least one chain configuration required for Guardian mode")
        
        if self.mode in [OperatingMode.ATTACK, OperatingMode.HYBRID]:
            if not self.security.private_key and not self.security.use_kms:
                errors.append("Private key or KMS configuration required for Attack mode")
