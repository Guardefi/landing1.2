...
from mev_guardian.config import load_config, get_config, OperatingMode
from mev_guardian.guardian_engine import GuardianEngine
from mev_guardian.types import (
    ThreatDetection, AttackSimulation, HoneypotDetection,
    MEVOpportunity, SystemMetrics, ThreatType, SimulationStatus
)
...
async def get_system_status(authenticated: bool = Depends(verify_token)):
    ...
    if guardian_engine and guardian_engine.start_time:
        uptime = (datetime.now(timezone.utc) - guardian_engine.start_time).total_seconds()
...
@app.get("/api/v1/guardian/simulations/{simulation_id}", response_model=SimulationResponse)
async def get_simulation(simulation_id: str, authenticated: bool = Depends(verify_token)):
...
async def get_mev_opportunities(authenticated: bool = Depends(verify_token)):
    ...
    return [
        OpportunityResponse(
            ...
            discovered_at=datetime.fromtimestamp(opp.timestamp, timezone.utc).isoformat(),
