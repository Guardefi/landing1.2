...
class Exploit(Base, TimestampMixin):
    ...
    # Relationships
    replay_sessions = relationship("ReplaySession", back_populates="exploit", cascade="all, delete-orphan")
    forensic_reports = relationship("ForensicReport", back_populates="exploit", cascade="all, delete-orphan")
...
class ReplaySession(Base, TimestampMixin):
    ...
    session_type = Column(String(50), nullable=False)  # exploit, transaction, custom, batch, forensic
    ...
    transaction_traces = relationship("TransactionTrace", back_populates="session", cascade="all, delete-orphan")
    child_sessions = relationship("ReplaySession", backref="parent_session", remote_side=[id])
    analysis_results = relationship("AnalysisResult", back_populates="session", cascade="all, delete-orphan")
    timeline_frames = relationship("TimelineFrame", back_populates="session", cascade="all, delete-orphan")
...
class Transaction(Base, TimestampMixin):
    ...
    # Relationships
    transaction_traces = relationship("TransactionTrace", back_populates="transaction", cascade="all, delete-orphan")
    storage_changes = relationship("StorageChange", back_populates="transaction", cascade="all, delete-orphan")
    event_logs = relationship("EventLog", back_populates="transaction", cascade="all, delete-orphan")
...
class ForensicReport(Base, TimestampMixin):
    ...
    # Report metadata
    report_type = Column(String(50), nullable=False)  # incident, vulnerability, compliance
    ...
    report_status = Column(String(20), default="draft")  # draft, review, approved, published
...
def create_analysis_patch(
    ...
    if patch_type == "storage":
        ...
        base_patch.update({
            "gas_price": parameters.get("gas_price")
        })
...
ENHANCED_HISTORICAL_EXPLOITS = {
    "dao_hack_2016": {
        **{
        ...
        "forensic_metadata": {
            "attack_timeline": [
                {"step": 1, "description": "Attacker creates malicious contract", "block": 1718496},
                {"step": 2, "description": "Calls withdrawBalance recursively", "block": 1718497},
