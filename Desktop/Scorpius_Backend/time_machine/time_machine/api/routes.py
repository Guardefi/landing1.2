...
from fastapi import APIRouter, HTTPException, WebSocket, WebSocketDisconnect, Depends, BackgroundTasks, Query
...
from .schemas import (
    ReplayRequest, ReplayResponse, PatchRequest, PatchResponse,
    DiffRequest, DiffResponse, BranchInfo, JobInfo, ForensicSessionInfo,
    TimelineEventResponse, SnapshotInfo, AnalysisResult, EngineStats,
    ForensicSessionRequest, BookmarkRequest, FindingRequest, AnalysisRequest,
    ErrorResponse, WSMessage, WSTimelineEvent, WSJobStatus, ExportRequest, ExportResponse,
    MacroInfo, ValidationResult
)
...
async def apply_patches(
    ...
    try:
        ...
        if validation_errors and request.validation_level == "strict":
            raise HTTPException(
                status_code=400, 
                detail=f"Patch validation failed: {validation_errors}"
            )
...
async def generate_diff(
    ...
    try:
        ...
        summary = {
            ...
            "total_changes": (
                len(diff.storage_changes) + len(diff.balance_changes) +
                len(diff.code_changes) + len(diff.nonce_changes) + len(diff.log_changes)
            )
        }
...
async def create_forensic_session(
    request: ForensicSessionRequest,
    engine: TimeMachineEngine = Depends(get_engine)
):
...
async def list_forensic_sessions(
    ...
    for session in sessions:
        result.append(ForensicSessionInfo(
            session_id=session.session_id,
            name=session.name,
            session_type=session.session_type,
            description=session.description,
...
async def execute_analysis(branch_id: str, session_type: SessionType, engine: TimeMachineEngine):
