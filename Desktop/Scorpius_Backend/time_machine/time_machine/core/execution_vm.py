...
from .models import TimelineEvent, Patch, VMBackend, FrameType, PatchType
...
class AnvilAdapter(BaseVMAdapter):
    ...
    async def apply_patch(self, patch: Patch) -> None:
        ...
        if patch.patch_type == PatchType.STORAGE:
            ...
            logger.info(f"Applied storage patch: {patch.target_address}[{patch.key}] = {patch.value}")
            ...
            logger.info(f"Applied balance patch: {patch.target_address} = {patch.value}")
            ...
            await self._rpc_call("anvil_setNonce", [
                patch.target_address, 
                patch.value
            ])
    ...
    async def trace_transaction(self, tx_hash: str) -> List[Dict[str, Any]]:
        """Get detailed transaction trace."""
        trace = await self._rpc_call("debug_traceTransaction", [tx_hash, {
            "tracer": "callTracer",
            "tracerConfig": {
                "withLog": True
            }
