...
class DiffEngine:
    ...
    async def generate_diff(self, from_branch_id: str, to_branch_id: str,
                          from_state: Dict[str, Any], to_state: Dict[str, Any],
                          format: DiffFormat = DiffFormat.JSON) -> Diff:
        """
    ...
    def _analyze_state_changes(self, from_state: Dict[str, Any], 
                             to_state: Dict[str, Any]) -> List[StateChange]:
        ...
        for key in all_keys:
            if (key in from_state and key in to_state and
                isinstance(from_state[key], dict) and isinstance(to_state[key], dict)):
    ...
    def _analyze_nested_changes(self, base_path: str, from_dict: Dict[str, Any],
                              to_dict: Dict[str, Any], depth: int) -> List[StateChange]:
        ...
        for key in all_keys:
            ...
            change = self._compare_values(full_path, from_dict.get(key), to_dict.get(key))
            
            if change:
                changes.append(change)
            
            # Recurse for nested dictionaries
            if (key in from_dict and key in to_dict and
                isinstance(from_dict[key], dict) and isinstance(to_dict[key], dict)):
                nested_changes = self._analyze_nested_changes(
    ...
    def _compare_values(self, path: str, old_value: Any, new_value: Any) -> Optional[StateChange]:
    ...
    def _extract_storage_changes(self, changes: List[StateChange]) -> Dict[str, Dict[str, Any]]:
    ...
    def _extract_balance_changes(self, changes: List[StateChange]) -> Dict[str, Dict[str, Any]]:
            ...
            if "balance" in change.path.lower():
                ...
                if address:
                    balance_changes[address] = {
                        ...
                        "change_type": change.change_type.value,
                        "delta": self._calculate_balance_delta(change.old_value, change.new_value),
                        "metadata": change.metadata,
                    }
        
        return balance_changes
    
    def _extract_code_changes(self, changes: List[StateChange]) -> Dict[str, Dict[str, Any]]:
    ...
    def _extract_nonce_changes(self, changes: List[StateChange]) -> Dict[str, Dict[str, Any]]:
            ...
            if "nonce" in change.path.lower():
                ...
                if address:
                    nonce_changes[address] = {
                        ...
                        "change_type": change.change_type.value,
                        "nonce_delta": self._calculate_nonce_delta(change.old_value, change.new_value),
                        "metadata": change.metadata,
                    }
        
        return nonce_changes
    
    def _extract_log_changes(self, changes: List[StateChange],
                           from_state: Dict[str, Any], to_state: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _extract_gas_changes(self, changes: List[StateChange],
                           from_state: Dict[str, Any], to_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if from_gas != to_gas:
            gas_changes = {
                ...
                "percentage_change": ((to_gas - from_gas) / from_gas * 100) if from_gas > 0 else 0,
            }
    ...
    def _calculate_diff_statistics(self, changes: List[StateChange]) -> DiffStatistics:
        ...
        unchanged_items = sum(1 for c in changes if c.change_type == ChangeType.UNCHANGED)
    ...
    def _format_html(self, diff: Diff) -> str:
        ...
        html = html_template.format(
            ...
            total_changes=len(diff.storage_changes) + len(diff.balance_changes) + len(diff.code_changes)
        )
        ...
        for addr, changes in diff.storage_changes.items():
            for slot, change in changes.items():
                sarif["runs"][0]["results"].append({
                    ...
                    "message": {
                        "text": f"Storage change at {addr} slot {slot}"
                    },
                    "locations": [{
                        "physicalLocation": {
                            "artifactLocation": {
    ...
    def _format_text(self, diff: Diff) -> str:
        ...
        if diff.storage_changes:
            ...
            for addr, changes in diff.storage_changes.items():
                ...
                for slot, change in changes.items():
                    lines.append(f"  Slot {slot}: {change['old_value']} → {change['new_value']}")
            lines.append("")
        ...
        if diff.balance_changes:
            ...
            for addr, change in diff.balance_changes.items():
                lines.append(f"Address: {addr}")
                lines.append(f"  Balance: {change['old_balance']} → {change['new_balance']}")
            lines.append("")
    ...
    # Custom comparators
    def _compare_numeric_hex(self, old_val: Any, new_val: Any) -> Tuple[bool, Dict[str, Any]]:
    ...
    def _compare_numeric(self, old_val: Any, new_val: Any) -> Tuple[bool, Dict[str, Any]]:
    ...
    def _compare_timestamp(self, old_val: Any, new_val: Any) -> Tuple[bool, Dict[str, Any]]:
    ...
    def _calculate_balance_delta(self, old_balance: Any, new_balance: Any) -> str:
        ...
        try:
            old_val = int(old_balance, 0) if isinstance(old_balance, str) else (old_balance or 0)
            new_val = int(new_balance, 0) if isinstance(new_balance, str) else (new_balance or 0)
    ...
    def _calculate_code_size_change(self, old_code: Any, new_code: Any) -> Dict[str, int]:
