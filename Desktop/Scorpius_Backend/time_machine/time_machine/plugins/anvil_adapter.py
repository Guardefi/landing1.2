...
from ..core.models import TimelineEvent, EventType
...
class AnvilAdapter(BaseVMAdapter):
    ...
    async def load_block_range(self, start_block: int, end_block: int) -> List[Dict]:
        ...
        for block_num in range(start_block, end_block + 1):
            block_data = await self._rpc_call('eth_getBlockByNumber', [
                hex(block_num), True
            ])
            if block_data:
    ...
    async def trace_transaction(self, tx_hash: str) -> Dict:
        ...
        return await self._rpc_call('debug_traceTransaction', [
            tx_hash,
            {
                'tracer': 'callTracer',
                'tracerConfig': {
    ...
    async def stream_events(self, start_block: int, end_block: Optional[int] = None) -> AsyncIterator[TimelineEvent]:
        ...
        while end_block is None or current_block <= end_block:
            ...
            if current_block <= latest_num:
                # Process blocks
                for block_num in range(current_block, min(latest_num + 1, (end_block or latest_num) + 1)):
                    block_data = await self._rpc_call('eth_getBlockByNumber', [
                        hex(block_num), True
                    ])
        ...
        for tx in transactions:
            try:
                ...
                estimates.append({
                    ...
                    'gas_price': int(tx.get('gasPrice', '0x0'), 16) or await self._get_gas_price()
                })
            except Exception as e:
                estimates.append({
                    'tx': tx,
    ...
    async def _set_balance(self, op: Dict) -> None:
        """Set account balance"""
        await self._rpc_call('anvil_setBalance', [
            op['target_address'],
            hex(int(op['value']))
        ])
    ...
    async def _set_storage(self, op: Dict) -> None:
        """Set storage slot"""
        await self._rpc_call('anvil_setStorageAt', [
            op['target_address'],
            op['storage_key'],
            op['value']
        ])
    ...
    async def _set_code(self, op: Dict) -> None:
        """Set account code"""
        await self._rpc_call('anvil_setCode', [
            op['target_address'],
            op['code']
        ])
