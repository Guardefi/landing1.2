...
async def ensure_directories():
    ...
    for directory in directories:
        path = Path(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) / directory
        if not path.exists():
...
async def initialize_database():
    ...
    try:
        ...
        await db["analyses"].create_index([("contract_address", 1), ("chain_id", 1)], unique=True)
        ...
        await db["contracts"].create_index([("address", 1), ("chain_id", 1)], unique=True)
...
async def initialize_ml_models():
    ...
    try:
        ...
        model_path = Path(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) / "models/ml/honeypot_model.joblib"
...
async def create_prometheus_config():
    ...
    prometheus_dir = Path(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) / "docker"
