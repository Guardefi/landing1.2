...
class ContractAnalysisRequest(BaseModel):
    ...
    chain_id: int = Field(default=1, description="Blockchain network ID (1=Ethereum, 56=BSC)")
    deep_analysis: bool = Field(default=False, description="Enable comprehensive analysis")
...
async def analyze_contract(
    ...
    """
    try:
        logger.info(f"Analysis requested for address: {request.address} on chain {request.chain_id}")
        ...
        # Server errors
        logger.error(f"Error analyzing contract {request.address}: {str(e)}", exc_info=True)
