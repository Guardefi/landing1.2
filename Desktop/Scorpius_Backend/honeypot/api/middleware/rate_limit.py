...
class RateLimiter:
    ...
    async def check_rate_limit(self, request: Request):
        ...
        if client_id in self.blocked_clients:
            ...
            block_remaining = int(self.block_duration_seconds - (current_time - block_time))
        ...
        recent_requests = [ts for ts in self.request_records[client_id] if ts >= window_start]
        
        # Check if limit exceeded
        if len(recent_requests) >= self.requests_limit:
            # Block client
            self.blocked_clients[client_id] = current_time
            logger.warning(f"Rate limit exceeded for {client_id}, blocked for {self.block_duration_seconds}s")
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
