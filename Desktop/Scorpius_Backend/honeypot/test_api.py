...
async def test_api():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.get(f"{API_BASE}/api/v1/dashboard/stats", headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
                    print(f"   ✅ Dashboard stats retrieved")
                    print(f"      Total analyses: {data.get('total_analyses', 0)}")
                    print(f"      Honeypots detected: {data.get('honeypots_detected', 0)}")
                else:
        ...
        try:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    print(f"   ⚠️  Analysis rejected (expected for dummy address): {response.status}")
                else:
