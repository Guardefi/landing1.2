    ...
    async def test_dashboard_data_structure(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                data = response.json()
                expected_fields = [
                    "total_analyses", "honeypots_detected", "false_positives",
                    "detection_rate", "recent_analyses", "risk_distribution",
            ...
            response = await client.get(
                f"{self.base_url}/api/v1/dashboard/trends?days=7",
                headers=self.headers
            )
            ...
            if response.status_code == 200:
                data = response.json()
                expected_fields = ["dates", "honeypot_counts", "analysis_counts", "detection_rates"]
                
                for field in expected_fields:
                    if field in data:
                        print(f"  ✅ {field}: {type(data[field])} (length: {len(data[field]) if isinstance(data[field], list) else 'N/A'})")
                    else:
                        print(f"  ❌ Missing field: {field}")
    ...
    async def test_contract_analysis_flow(self):
        ...
        async with httpx.AsyncClient() as client:
            for address in test_addresses:
                ...
                if response.status_code == 200:
                    data = response.json()
                    required_fields = ["address", "is_honeypot", "confidence", "risk_level"]
                    ...
                    print(f"    ⚠️  Server error (may be due to missing blockchain connection)")
                
                # Test getting analysis history
                response = await client.get(
                    f"{self.base_url}/api/v1/history/{address}",
                    headers=self.headers
                )
    ...
    async def generate_sample_data_for_react(self):
        ...
        sample_data = {
            ...
            "analysis_response": {
                ...
                "engine_results": {
                    "static_engine": {"confidence": 0.85, "techniques": ["Hidden State Update"]},
                    "ml_engine": {"confidence": 0.98, "prediction": True}
                }
            },
            "trends_data": {
                "dates": ["2025-06-17", "2025-06-18", "2025-06-19", "2025-06-20", "2025-06-21", "2025-06-22", "2025-06-23"],
