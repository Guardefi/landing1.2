...
class MLEngine:
    ...
    async def _extract_features(self, contract_data: dict) -> List[float]:
        ...
        suspicious_count = sum(1 for func in abi if func.get("name", "").lower() in suspicious_functions)
        ...
        incoming_value = sum(tx.get("value", 0) for tx in transactions if tx.get("to") == contract_data.get("address"))
        outgoing_value = sum(tx.get("value", 0) for tx in transactions if tx.get("from") == contract_data.get("address"))
    ...
    def _generate_synthetic_data(self, n_samples=1000):
        ...
        honeypot_idx = np.where(
            (X[:, 0] > 0.7) &  # High bytecode length
            (X[:, 1] > 0.7) &  # High control flow complexity
            (X[:, 4] > 0.5)    # Selfdestruct present
        )[0]
        
        # Another honeypot pattern: suspicious functions + imbalanced tx ratio
        honeypot_idx2 = np.where(
            (X[:, 7] > 0.7) &  # High suspicious function count
            (X[:, 8] < 0.3)    # Low tx volume ratio (funds coming in but not out)
        )[0]
