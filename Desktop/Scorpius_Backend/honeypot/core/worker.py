...
def analyze_contract(contract_address: str, chain_id: int = 1, deep_analysis: bool = False) -> Dict[str, Any]:
    """
    ...
    # Run in event loop
    loop = asyncio.get_event_loop()
    return loop.run_until_complete(_analyze_contract_async(contract_address, chain_id, deep_analysis))


async def _analyze_contract_async(contract_address: str, chain_id: int, deep_analysis: bool) -> Dict[str, Any]:
    """Async implementation of contract analysis"""
    try:
        # Fetch contract data
        contract_data = await contract_fetcher.fetch_contract_details(contract_address, chain_id)
        ...
        if contract_data.get("transactions"):
            tx_results = transaction_analyzer.analyze_transactions(contract_data["transactions"])
...
async def _store_analysis_results(contract_address: str, chain_id: int, results: Dict[str, Any]):
