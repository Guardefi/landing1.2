...
from api.metrics import track_analysis_metrics, HONEYPOT_DETECTED
...
class HoneypotDetector:
    ...
    async def analyze_contract(
        ...
        # Fetch contract data
        try:
            contract_data = await self.contract_fetcher.fetch_contract(address, chain_id)
            ...
            if is_token:
                token_data = await self.contract_fetcher.fetch_token_metadata(address, chain_id)
                contract_data.update({"token_data": token_data})
                contract_data["is_token"] = True
                
            # Fetch transactions
            transactions = await self.contract_fetcher.fetch_transactions(address, chain_id)
        ...
        try:
            ...
            bytecode_analysis = await self.bytecode_analyzer.analyze_bytecode(contract_data.get("bytecode", ""))
            contract_data["bytecode_metrics"] = bytecode_analysis.get("bytecode_metrics", {})
            
            analysis_tasks = [
                self.static_engine.analyze(contract_data),
                self.ml_engine.predict(contract_data),
                self.transaction_analyzer.analyze_transactions(contract_data.get("transactions", []))
            ]
            ...
            if deep_analysis:
                ...
                symbolic_result = {"confidence": 0, "techniques": [], "message": "Symbolic analysis not performed"}
            ...
            risk_level = self._calculate_risk_level(confidence, len(detected_techniques))
            ...
            result = AnalysisResponse(
                ...
                engine_results={
                    ...
                    "symbolic": symbolic_result if deep_analysis else {"performed": False}
                },
                contract_metadata={
                ...
                transaction_history={
                    ...
                    "first_seen": contract_data.get("created_at") if "created_at" in contract_data else None,
                    "unique_senders": len(set(tx.get("from") for tx in contract_data.get("transactions", [])))
                }
            )
            ...
            logger.info(f"Analysis completed for {address}: honeypot={is_honeypot}, confidence={confidence:.2f}")
            return result
    ...
    def _calculate_risk_level(self, confidence: float, technique_count: int) -> RiskLevel:
        """
    ...
    async def get_analysis_history(self, address: str, limit: int = 10) -> List[Dict[str, Any]]:
        """
