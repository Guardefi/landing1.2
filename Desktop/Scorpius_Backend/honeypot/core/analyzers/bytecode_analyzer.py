...
class BytecodeAnalyzer:
    ...
    async def analyze_bytecode(self, bytecode: str) -> Dict[str, Any]:
        ...
        try:
            ...
            detected_techniques = list(set(
                pattern["technique"] for pattern in detected_patterns
            ))
            ...
            return {
                ...
                "opcode_stats": {k: v for k, v in sorted(opcode_counts.items(), key=lambda x: x[1], reverse=True)},
    ...
    def _calculate_bytecode_metrics(self, bytecode: str, opcode_counts: Dict[str, int]) -> Dict[str, Any]:
        ...
        control_flow_count = sum(opcode_counts.get(op, 0) for op in control_flow_opcodes)
        ...
        if sum(opcode_counts.values()) > 0:
            metrics["storage_op_ratio"] = storage_ops_count / sum(opcode_counts.values())
        else:
            metrics["storage_op_ratio"] = 0
            
        # External call ratio
        external_call_opcodes = ["CALL", "CALLCODE", "DELEGATECALL", "STATICCALL"]
        external_call_count = sum(opcode_counts.get(op, 0) for op in external_call_opcodes)
        
        if sum(opcode_counts.values()) > 0:
            metrics["external_call_ratio"] = external_call_count / sum(opcode_counts.values())
        else:
