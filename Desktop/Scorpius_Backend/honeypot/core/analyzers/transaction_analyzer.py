...
class TransactionAnalyzer:
    ...
    async def _detect_quick_drain(self, transactions: List[Dict]) -> Dict[str, Any]:
        ...
        deposits = [tx for tx in transactions if tx.get("value", 0) > 0 and tx.get("to") == transactions[0].get("address")]
        withdrawals = [tx for tx in transactions if tx.get("value", 0) > 0 and tx.get("from") == transactions[0].get("address")]
    ...
    async def _detect_owner_only_withdrawals(self, transactions: List[Dict]) -> Dict[str, Any]:
    ...
    async def _detect_failed_withdrawals(self, transactions: List[Dict]) -> Dict[str, Any]:
