...
def test_analyze_honeypot_contract():
    ...
    assert results is not None
    assert "confidence" in results
    assert results["confidence"] > 0.5  # Should have higher confidence of being honeypot
    assert len(results["techniques"]) > 0
    assert "malicious_fallback" in results["techniques"] or "selfdestruct" in results["techniques"]
    assert results["patterns_detected"] > 0
