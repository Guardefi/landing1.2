...
from .. import initialize_scorpius, get_engine, ScorpiusEngine
from ..exceptions import ScorpiusError, LicenseError
from .models import *
from .websockets import ConnectionManager
from .dashboard import dashboard_router
from .dependencies import get_scorpius_engine, get_current_user, set_scorpius_initialized, is_scorpius_initialized
...
async def scorpius_exception_handler(request, exc):
    return JSONResponse(
        status_code=500,
        content={"detail": f"Scorpius Error: {str(exc)}"}
    )
...
async def license_exception_handler(request, exc):
    return JSONResponse(
        status_code=403,
        content={"detail": f"License Error: {str(exc)}"}
    )
