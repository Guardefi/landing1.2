...
from .dependencies import get_scorpius_engine, get_current_user
    ...
    try:
        ...
        for i in range(limit):
            entry = ActivityLogEntry(
                ...
                action=action_type or ["encryption", "scan", "key_generation", "login"][i % 4],
                resource=f"resource_{i}",
                status=["success", "failed"][i % 10 > 8],  # 90% success rate
                details=f"Operation completed successfully" if i % 10 <= 8 else "Operation failed"
            )
