...
from .. import initialize_scorpius, get_engine, __version__
...
def server(ctx, port, host, workers):
    ...
    async def start_server():
        ...
        click.echo(f"Scorpius server would start on {host}:{port} with {workers} workers")
...
def status(ctx, module):
    ...
    async def check_status():
        ...
        if module:
            module_info = next(
                (m for m in status_info['modules'] if m['name'] == module),
                None
            )
            if module_info:
