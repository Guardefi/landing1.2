...
from utils.metrics import PerformanceMonitor, MetricsCollector
...
class SimilarityEngine:
    ...
    async def compare_bytecodes(self, bytecode1: str, bytecode2: str, 
                               use_neural_network: bool = True) -> SimilarityResult:
        ...
        try:
            ...
            md_result = await self.comparison_engine.compute_similarity(bytecode1, bytecode2)
            ...
            # Combine scores
            final_score = self._combine_scores(md_result['final_score'], nn_score, use_neural_network)
    ...
    async def find_similar_bytecode(self, query_bytecode: str, 
                                   top_k: int = None, 
        ...
        try:
            ...
            similar_embeddings = await self._vector_search(query_embedding, top_k * 2)  # Get more for filtering
            ...
            self.monitor.end_measurement("similarity_search", {
    ...
    async def batch_index_bytecodes(self, bytecodes: List[Tuple[str, Dict]]) -> List[str]:
        ...
        try:
            ...
            for i in range(0, len(bytecodes), batch_size):
                ...
                batch_embeddings = await self._get_batch_embeddings([bc for bc, _ in batch])
            ...
            self.monitor.end_measurement("batch_indexing", {
                'total_bytecodes': len(bytecodes),
    ...
    def _features_to_vector(self, features: Dict) -> np.ndarray:
        ...
        common_opcodes = ['PUSH1', 'POP', 'MSTORE', 'MLOAD', 'SSTORE', 'SLOAD', 'JUMP', 'JUMPI', 'JUMPDEST', 'ADD']
    ...
    async def _load_siamese_model(self):
        ...
        try:
            ...
            if os.path.exists(model_path):
                self._siamese_model.load_state_dict(torch.load(model_path, map_location=self._device))
    ...
    async def _vector_search(self, query_embedding: np.ndarray, top_k: int) -> List[Tuple[str, float]]:
