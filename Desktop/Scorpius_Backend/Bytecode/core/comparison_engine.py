...
class MultiDimensionalComparison:
    ...
    def __init__(self, config: Dict):
        ...
        self.opcode_categories = {
            'arithmetic': {'ADD', 'SUB', 'MUL', 'DIV', 'MOD', 'ADDMOD', 'MULMOD', 'EXP'},
            'comparison': {'LT', 'GT', 'SLT', 'SGT', 'EQ', 'ISZERO'},
            'bitwise': {'AND', 'OR', 'XOR', 'NOT', 'BYTE', 'SHL', 'SHR', 'SAR'},
            'memory': {'MLOAD', 'MSTORE', 'MSTORE8', 'MSIZE', 'MCOPY'},
            'storage': {'SLOAD', 'SSTORE', 'TLOAD', 'TSTORE'},
            'stack': {'POP', 'PUSH1', 'PUSH2', 'PUSH32', 'DUP1', 'DUP16', 'SWAP1', 'SWAP16'},
    ...
    async def compute_similarity(self, bytecode1: str, bytecode2: str) -> Dict[str, float]:
    ...
    def _extract_control_flow_dimension(self, opcodes: List[Dict]) -> Set[str]:
        ...
        for i, opcode in enumerate(opcodes):
            if opcode['mnemonic'] in ['JUMP', 'JUMPI']:
                ...
                context_after = [op['mnemonic'] for op in opcodes[i+1:min(len(opcodes), i+3)]]
    ...
    def _is_address(self, operand: str) -> bool:
        ...
        return len(operand) == 40 and all(c in '0123456789abcdefABCDEF' for c in operand)
