...
class SCORPIUSDemo:
    ...
    async def test_bytecode_comparison(self, bytecode1: str, bytecode2: str, description: str = "") -> Dict[str, Any]:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.api_base_url}/api/v1/stats") as response:
                    ...
                    if response.status == 200:
                        ...
                        print(f"   Total Indexed: {stats.get('total_indexed_bytecodes', 0)}")
                        ...
                        error_text = await response.text()
                        print(f"❌ Stats retrieval failed: {response.status} - {error_text}")
                        return {}
            ...
            async with websockets.connect(self.ws_url) as websocket:
                ...
                while time.time() - start_time < duration:
                    try:
                        ...
                        print(f"   📨 {data.get('type', 'unknown')}: {data.get('timestamp', 'no-time')}")
