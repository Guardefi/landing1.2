...
def example_feature_extraction():
    ...
    print(f"Normalized bytecode length: {len(normalized['normalized_bytecode'])} characters")
...
def example_advanced_comparison():
    ...
    print("\nIndividual Analysis:")
    print(f"Jaccard Similarity: {comparison_engine.jaccard_similarity(bytecode1, bytecode2):.4f}")
    print(f"N-gram Similarity:  {comparison_engine.ngram_similarity(bytecode1, bytecode2):.4f}")
    print(f"Structural Similarity: {comparison_engine.structural_similarity(bytecode1, bytecode2):.4f}")
    print(f"Opcode Frequency Similarity: {comparison_engine.opcode_frequency_similarity(bytecode1, bytecode2):.4f}")
...
def example_performance_analysis():
    ...
    if 'metrics' in result:
        print(f"Engine reported time: {result['metrics']['computation_time']:.4f} seconds")
        print(f"Memory usage: {result['metrics'].get('memory_usage', 'N/A')} MB")
    
    # Batch comparison timing
    print("\nBatch Comparison Performance:")
    pairs = [(contract_list[i][1], contract_list[j][1]) 
