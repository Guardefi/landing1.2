...
class BytecodeNormalizer:
    ...
    async def _normalize_constants(self, bytecode: str) -> str:
        ...
        normalized = re.sub(push_pattern, replace_constant, normalized, flags=re.IGNORECASE)
        return normalized
    ...
    async def _normalize_addresses(self, bytecode: str) -> str:
        ...
        def replace_address(match):
            ...
            if address == '00' * 20:
                ...
                return '73' + 'ADDR' + hashlib.md5(address.encode()).hexdigest()[:8].upper()
    ...
    async def _normalize_push_sequences(self, bytecode: str) -> str:
        ...
        return re.sub(push_sequence_pattern, normalize_push_sequence, bytecode, flags=re.IGNORECASE)
...
class InstructionLevelNormalizer:
    ...
    def __init__(self):
        self.noise_instructions = {
            'INVALID', 'REVERT', 'SELFDESTRUCT'  # Instructions that add noise
        }
...
class FeatureExtractor:
    ...
    async def extract_features(self, bytecode: str) -> Dict:
        ...
        normalized_opcodes = await self.instruction_normalizer.normalize_instructions(opcodes)
        ...
        features = {
            ...
            'control_flow_features': self._extract_control_flow_features(normalized_opcodes),
            'data_dependency_features': self._extract_data_dependency_features(normalized_opcodes),
            'structural_features': self._extract_structural_features(normalized_opcodes)
        }
    ...
    def _hex_to_opcode(self, byte_val: int) -> str:
        ...
        opcode_map = {
            ...
            0x04: 'DIV', 0x05: 'SDIV', 0x50: 'POP', 0x51: 'MLOAD',
            0x52: 'MSTORE', 0x54: 'SLOAD', 0x55: 'SSTORE',
    ...
    def _extract_structural_features(self, opcodes: List[Dict]) -> Dict:
        ...
        return {
            ...
            'complexity_ratio': len(set(op['mnemonic'] for op in opcodes)) / len(opcodes) if opcodes else 0
        }
