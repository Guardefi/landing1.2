...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, client_ip: str, user_id: str = None) -> str:
        ...
        logger.info(f"WebSocket connection established: {connection_id} from {client_ip}")
    ...
    async def unsubscribe(self, connection_id: str, event_types: List[str]):
        ...
        for event_type in event_types:
            ...
            # Update connection info
            if connection_id in self.connection_info:
                self.connection_info[connection_id].subscription_types.discard(event_type)
    ...
    async def broadcast_event(self, event: AnalysisEvent):
        ...
        logger.debug(f"Broadcasted {event.event_type} to {len(subscriber_ids)} connections")
    
    async def send_analysis_result(self, result: Dict[str, Any], analysis_type: str = "similarity"):
...
class RealtimeAnalysisQueue:
        ...
        try:
            ...
            if request_data.get("type") == "similarity_analysis":
                ...
                result = await similarity_engine.compare_bytecodes(
                    request_data["bytecode1"],
                    request_data["bytecode2"]
                )
                ...
                threat_score = await self._analyze_threat_level(request_data, similarity_engine)
    ...
    async def _analyze_threat_level(self, request_data: Dict[str, Any], similarity_engine) -> float:
