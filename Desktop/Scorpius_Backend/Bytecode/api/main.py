...
class BytecodeComparisonRequest(BaseModel):
    ...
    use_neural_network: Optional[bool] = Field(True, description="Use neural network for comparison")
...
class BytecodeComparisonResponse(BaseModel):
    ...
    is_similar: bool = Field(..., description="Whether bytecodes are considered similar")
    dimension_scores: Dict[str, float] = Field(..., description="Scores for each dimension")
...
class SimilaritySearchRequest(BaseModel):
    ...
    min_similarity: Optional[float] = Field(0.7, ge=0.0, le=1.0, description="Minimum similarity threshold")
...
class BatchIndexingRequest(BaseModel):
    bytecodes: List[Dict[str, Any]] = Field(..., description="List of bytecodes with metadata to index")

class BatchIndexingResponse(BaseModel):
    indexed_count: int = Field(..., description="Number of successfully indexed bytecodes")
...
class EngineStatsResponse(BaseModel):
    total_indexed_bytecodes: int = Field(..., description="Total number of indexed bytecodes")
    cache_size: int = Field(..., description="Current cache size")
    device: str = Field(..., description="Device being used for computation")
    model_loaded: bool = Field(..., description="Whether neural network model is loaded")
...
async def compare_bytecodes(
    request: BytecodeComparisonRequest,
    ...
    try:
        ...
        if not request.bytecode1 or not request.bytecode2:
            raise HTTPException(status_code=400, detail="Both bytecodes must be provided")
        
        # Perform comparison
        result = await engine.compare_bytecodes(
            request.bytecode1,
            request.bytecode2,
            request.use_neural_network
        )
        ...
        monitor.end_measurement("compare_bytecodes", {
            'similarity_score': result.similarity_score,
            'is_similar': is_similar
        })
...
async def search_similar_bytecodes(
    ...
    try:
        ...
        if not request.query_bytecode:
            raise HTTPException(status_code=400, detail="Query bytecode must be provided")
        
        # Perform search
        results = await engine.find_similar_bytecode(
            request.query_bytecode,
            request.top_k,
...
async def batch_index_bytecodes(
    ...
    try:
        ...
        if not request.bytecodes:
            raise HTTPException(status_code=400, detail="Bytecodes list must be provided")
