...
class TestSimilarityEngineCore:
    ...
    async def test_compare_bytecodes_basic(self):
        ...
        with patch.object(self.engine.comparison_engine, 'multi_dimensional_compare', 
                         return_value={'jaccard_similarity': 0.85, 'opcode_similarity': 0.90, 
                                      'control_flow_similarity': 0.80, 'structural_similarity': 0.75}) as mock_compare:
            with patch.object(self.engine.neural_model, 'compare_async',
                             return_value=0.92) as mock_neural:
                
                result = await self.engine.compare_bytecodes(
                    self.sample_bytecode1, 
                    self.sample_bytecode2
                )
    ...
    async def test_identical_bytecodes(self):
        ...
        with patch.object(self.engine.comparison_engine, 'multi_dimensional_compare', 
                         return_value={'jaccard_similarity': 1.0, 'opcode_similarity': 1.0,
                                      'control_flow_similarity': 1.0, 'structural_similarity': 1.0}):
            with patch.object(self.engine.neural_model, 'compare_async',
                             return_value=1.0):
    ...
    async def test_batch_comparison(self):
        ...
        with patch.object(self.engine, 'compare_bytecodes',
                         return_value={'combined_score': 0.85}):
    ...
    async def test_search_similar(self):
        ...
        with patch.object(self.engine, 'compare_bytecodes',
                         return_value={'combined_score': 0.85}):
            results = await self.engine.search_similar(target, candidates, threshold=0.8, top_k=5)
...
class TestComparisonEngineCore:
    ...
    def test_multi_dimensional_compare(self):
        ...
        with patch.object(self.engine.normalizer, 'normalize',
                         side_effect=lambda x: x):
            with patch.object(self.engine.normalizer, 'extract_opcodes',
                             return_value=['PUSH1', 'MSTORE', 'RETURN']):
                with patch.object(self.engine.normalizer, 'extract_control_flow',
                                 return_value=['JUMPI', 'JUMP']):
                    
                    result = self.engine.multi_dimensional_compare("bytecode1", "bytecode2")
...
class TestBytecodeNormalizerCore:
    ...
    def test_remove_metadata(self):
        ...
        bytecode_with_metadata = "608060405234801561001057600080fd5b50a165627a7a72305820"
...
class TestSiameseNetworkCore:
    ...
    async def test_compare_async(self):
        ...
        with patch.object(self.model, 'preprocess_bytecode',
                         return_value=torch.randn(256)):
            with patch.object(self.model, 'forward',
                             return_value=torch.tensor([0.85])):
...
class TestCoreModulesIntegration:
    ...
    async def test_end_to_end_comparison(self):
        ...
        with patch.object(engine.neural_model, 'compare_async',
                         return_value=0.85):
            result = await engine.compare_bytecodes(bytecode1, bytecode2)
            
            # Verify complete result structure
            expected_keys = ['jaccard_similarity', 'neural_similarity', 'combined_score', 
                           'opcode_similarity', 'control_flow_similarity', 'structural_similarity']
