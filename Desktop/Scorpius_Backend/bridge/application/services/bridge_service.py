...
from ...domain.models.bridge_tx import BridgeTransaction, TransferStatus, SecurityLevel
from ...domain.policy import TransferPolicy, SecurityPolicy
from ...domain.events import (
    BridgeTransferInitiated, 
    BridgeTransferCompleted, 
    BridgeTransferFailed
)
from ...domain.errors import InvalidTransferError, InsufficientLiquidityError
...
class BridgeService:
    ...
    async def initiate_transfer(self, command: InitiateBridgeTransferCommand) -> str:
        ...
        logger.info(f"Initiating transfer: {command.amount} from {command.source_chain} to {command.destination_chain}")
        ...
        try:
            ...
            sender_history = await self.transfer_repository.get_sender_history(command.sender_address)
            ...
            final_security = max(command.security_level, recommended_security, key=lambda x: x.value)
            
            # Calculate fees
            bridge_fee = TransferPolicy.calculate_bridge_fee(
                command.amount,
                self.config.base_bridge_fee_percentage,
                final_security
            )
            ...
            pool = await self.pool_repository.get_by_chains(
                command.source_chain, 
                command.destination_chain,
    ...
    async def validate_transfer(self, command: ValidateTransferCommand) -> bool:
        ...
        logger.info(f"Validating transfer {command.transfer_id} by validator {command.validator_id}")
        ...
        try:
            ...
            if is_valid:
                ...
                transfer.add_validator_signature(command.validator_id, command.signature)
    ...
    async def cancel_transfer(self, command: CancelTransferCommand) -> bool:
        ...
        try:
            ...
            if transfer.status not in [TransferStatus.PENDING, TransferStatus.INITIATED]:
    ...
    async def get_transfer(self, query: GetBridgeTransferQuery) -> Optional[Dict[str, Any]]:
    ...
    async def get_transfer_history(self, query: GetTransferHistoryQuery) -> Dict[str, Any]:
    ...
    async def _verify_validator_signature(
        ...
        return (
            len(signature) > 0 and
            validator.supports_chain(transfer.source_chain) and
            validator.supports_chain(transfer.destination_chain)
        )
