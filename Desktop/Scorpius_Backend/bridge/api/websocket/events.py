...
class EventBroadcaster:
    ...
    async def _broadcast_event(self, event: BridgeEvent):
        ...
        await connection_manager.broadcast_to_topic(message, f"events:{event.event_type}")
        
        # Broadcast to chain-specific topics if applicable
        if event.source_chain:
            await connection_manager.broadcast_to_topic(message, f"chain:{event.source_chain}")
        
        if event.target_chain:
            await connection_manager.broadcast_to_topic(message, f"chain:{event.target_chain}")
        
        # Broadcast to user-specific topic if applicable
        if event.user_id:
            await connection_manager.broadcast_to_topic(message, f"user:{event.user_id}")
        
        # Broadcast to transaction-specific topic if applicable
        if event.transaction_id:
            await connection_manager.broadcast_to_topic(message, f"transaction:{event.transaction_id}")
