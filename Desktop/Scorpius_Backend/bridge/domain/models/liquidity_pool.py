...
class LiquidityPool:
    ...
    def _validate(self) -> None:
        ...
        if self.source_chain == self.destination_chain:
            raise InvalidTransferError("Source and destination chains cannot be the same")
    ...
    def add_liquidity(self, provider_address: str, amount: Decimal) -> Decimal:
        ...
        if self.status not in [PoolStatus.ACTIVE, PoolStatus.INACTIVE]:
            raise InvalidTransferError(f"Cannot add liquidity to pool with status {self.status.value}")
        ...
        if (self.status == PoolStatus.INACTIVE and 
            self.total_liquidity >= self.min_liquidity):
    ...
    def remove_liquidity(self, provider_address: str, shares: Decimal) -> Decimal:
        ...
        if withdrawal_amount > self.available_liquidity:
            raise InsufficientLiquidityError("Insufficient available liquidity for withdrawal")
    ...
    def execute_transfer(self, amount: Decimal) -> Decimal:
        ...
        if self.status != PoolStatus.ACTIVE:
            raise InvalidTransferError(f"Pool is not active (status: {self.status.value})")
        ...
        if net_amount > self.reserved_liquidity:
            raise InsufficientLiquidityError("Insufficient reserved liquidity for transfer")
        ...
        self.metrics.avg_transfer_size = (
            self.metrics.total_volume / Decimal(str(self.metrics.transfer_count))
        )
        
        return net_amount
    
    def get_provider_position(self, provider_address: str) -> Optional[LiquidityPosition]:
