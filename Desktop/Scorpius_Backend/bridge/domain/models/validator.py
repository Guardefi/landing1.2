...
from ..errors import ValidatorError, SecurityViolationError
...
class ValidatorNode:
    ...
    def activate(self) -> None:
        ...
        if self.status != ValidatorStatus.JOINING:
            raise ValidatorError(f"Cannot activate validator with status {self.status.value}")
    ...
    def deactivate(self) -> None:
        ...
        if self.status not in [ValidatorStatus.ACTIVE, ValidatorStatus.JOINING]:
            raise ValidatorError(f"Cannot deactivate validator with status {self.status.value}")
    ...
    def add_stake(self, amount: Decimal) -> None:
        ...
        if (self.status == ValidatorStatus.SLASHED and 
            self.stake_amount >= self.minimum_stake):
            self.status = ValidatorStatus.INACTIVE
    ...
    def record_validation(self, success: bool, response_time: float) -> None:
        ...
        if self.performance.total_validations == 1:
            ...
            self.performance.response_time_avg = (
                (self.performance.response_time_avg * (self.performance.total_validations - 1) + response_time) 
        ...
        return {
            ...
            "performance": {
                ...
                "last_activity": (
                    ...
                    if self.performance.last_activity else None
                ),
            },
