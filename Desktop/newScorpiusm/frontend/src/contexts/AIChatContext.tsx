...
export function AIChatProvider({ children }: AIChatProviderProps) {
  ...
  // Mock AI API call
  const callAI = async (messages: AIMessage[], model: AIModelKey): Promise<string> => {
  ...
  const sendMessage = async (content: string, context = 'general') => {
    ...
    try {
      ...
      dispatch({
        type: 'UPDATE_USAGE',
        payload: {
          tokensUsed: state.usage.tokensUsed + (assistantMessage.metadata?.tokens || 0),
  ...
  const canUseModel = (model: AIModelKey): boolean => {
    ...
    const hasApiKey = modelInfo.provider === 'openai' ? !!openaiKey : !!anthropicKey;
