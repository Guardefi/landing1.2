...
export function useNotifications() {
  ...
  useEffect(() => {
    const loadPreferences = () => {
      try {
        ...
        const savedHistory = localStorage.getItem('scorpius_notification_history');
        if (savedHistory) {
  ...
  useEffect(() => {
    ...
    if (settings.customApiKeys?.telegram) {
      updatedPreferences.channels.telegram.token = settings.customApiKeys.telegram;
    }
  ...
  // Get unread count
  const unreadCount = history.filter(item => !item.read && !item.archived).length;
  ...
  const notifyMEVOpportunity = useCallback(
    (data: { mevType: string; estimatedProfit: string; transactionHash: string }) => {
      return sendNotification({
