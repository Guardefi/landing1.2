...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("time_machine_api.log"), logging.StreamHandler()],
)
...
async def system_health() -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "available_tasks": {
                "replay": ["start_exploit_replay", "start_transaction_replay"],
                "analysis": [
                    "analyze_exploit",
...
async def trigger_session_cleanup(
    ...
    try:
        ...
        task_result = cleanup_tasks.cleanup_session.apply_async(
            args=[session_id], kwargs={"web3_url": web3_url, "redis_url": redis_url}
        )
...
async def trigger_old_sessions_cleanup(
    age_hours: int = 24, web3_url: str | None = None, redis_url: str | None = None
) -> dict[str, Any]:
    """Trigger cleanup for old sessions"""
    try:
        # Send cleanup task to Celery
        task_result = cleanup_tasks.cleanup_old_sessions.apply_async(
            args=[age_hours], kwargs={"web3_url": web3_url, "redis_url": redis_url}
        )
...
async def trigger_temp_files_cleanup(
    temp_dir: str = "/tmp/time_machine", age_hours: int = 12
) -> dict[str, Any]:
...
def start_server():
    ...
    logger.info(
        f"Starting Time Machine API Server on {SERVER_CONFIG['host']}:{SERVER_CONFIG['port']}"
    )
