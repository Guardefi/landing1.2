...
class Config:
    ...
    def __init__(self):
        ...
        # Rate limiting
        self.max_notifications_per_hour = int(
            os.getenv("MAX_NOTIFICATIONS_PER_HOUR", "1000")
        )
...
class RateLimiter:
    ...
    async def is_rate_limited(
        self, channel: str, recipient: str, limit_per_hour: int
    ) -> bool:
...
class EmailNotifier:
    ...
    async def send_notification(
        self, alert: Alert, recipient: str, template: NotificationTemplate
...
class SlackNotifier:
            ...
            async with aiohttp.ClientSession() as session:
                if self.config.slack_webhook_url:
                    ...
                    async with session.post(
                        self.config.slack_webhook_url, json=message
                    ) as response:
    ...
    def _create_slack_message(
        self, alert: Alert, template: NotificationTemplate
    ) -> dict[str, Any]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    ...
                    if (
                        response.status == 204
                    ):  # Discord returns 204 for successful webhook
                        delivery.status = NotificationStatus.SENT
                        delivery.sent_at = datetime.now()
                    else:
                        delivery.status = NotificationStatus.FAILED
    ...
    def _create_discord_embed(self, alert: Alert) -> dict[str, Any]:
        ...
        return {
            ...
            "fields": [
                ...
                {"name": "Chain ID", "value": str(alert.chain_id), "inline": True},
                {
            ...
            "footer": {"text": "Scorpius Mempool Elite"},
...
class NotifierService:
    ...
    async def process_alert_notification(self, alert: Alert):
        ...
        for config_name, notification_config in applicable_configs.items():
            ...
            if not self._meets_severity_threshold(
                alert.severity, notification_config.severity_threshold
            ):
                continue
            ...
            for recipient in recipients:
                ...
                if await self.rate_limiter.is_rate_limited(
                    ...
                    logger.warning(
                        f"Rate limited: {notification_config.channel} to {recipient}"
                    )
                    continue

                # Get template
                template = await self.get_template(
                    notification_config.channel, alert.severity
                )

                # Send notification
                delivery = await self.send_notification(
                    alert, recipient, notification_config, template
                )
                ...
                await self.rate_limiter.increment_rate_limit(
                    notification_config.channel, recipient
                )
    ...
    async def send_notification(
        ...
        try:
            if config.channel == NotificationChannel.EMAIL:
                return await self.email_notifier.send_notification(
                    alert, recipient, template
                )
            elif config.channel == NotificationChannel.SLACK:
                return await self.slack_notifier.send_notification(
                    alert, recipient, template
                )
            elif config.channel == NotificationChannel.DISCORD:
                return await self.discord_notifier.send_notification(
                    alert, recipient, template
                )
            else:
    ...
    def _meets_severity_threshold(
        self, alert_severity: AlertSeverity, threshold: AlertSeverity
    ) -> bool:
    ...
    async def retry_failed_notifications(self):
        ...
        while True:
            try:
                ...
                retry_threshold = datetime.now() - timedelta(
