...
ACTIVE_CONNECTIONS = Gauge(
    "scorpius_api_active_websockets", "Active WebSocket connections"
)
ALERT_COUNT = Counter(
    "scorpius_api_alerts_sent", "Alerts sent via WebSocket", ["severity"]
)
    ...
    async def initialize(self):
        ...
        try:
            self.pool = await asyncpg.create_pool(
                config.database_url, min_size=5, max_size=20
            )
...
class ConnectionManager:
    ...
    def disconnect(self, client_id: str):
        ...
        if client_id in self.active_connections:
            ...
            ACTIVE_CONNECTIONS.set(self.connection_count)
            logger.info(
                f"Client {client_id} disconnected. Total: {self.connection_count}"
            )
...
async def create_rule(
    rule_data: RuleRequest, current_user: dict = Depends(get_current_user)
):
    """Create a new rule"""
    try:
