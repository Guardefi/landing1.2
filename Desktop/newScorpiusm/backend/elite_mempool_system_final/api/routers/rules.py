...
from ..models import PaginationParams, RuleRequest, RuleResponse, RuleCondition, RuleAction
...
async def test_rule(
    ...
    try:
        ...
        confidence = len(matched_conditions) / total_conditions if total_conditions > 0 else 0
...
async def get_rule_performance(
    ...
    try:
        async with db.acquire() as conn:
            ...
            # Total triggers
            total_triggers = await conn.fetchval(f"""
            ...
            # Last triggered
            last_triggered = await conn.fetchval(f"""
...
async def _validate_rule(rule_data: RuleRequest):
    ...
    if not rule_data.conditions:
        raise HTTPException(
            status_code=400,
            detail="Rule must have at least one condition"
        )
    
    if not rule_data.actions:
        raise HTTPException(
            status_code=400,
            detail="Rule must have at least one action"
        )
    ...
    for condition in rule_data.conditions:
        if condition.type not in valid_condition_types:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid condition type: {condition.type}"
            )
