                ...
                "data": [
                    {
                        ...
                        "avg_gas_price": float(row['avg_gas_price']) if row['avg_gas_price'] else 0,
...
async def get_alert_timeseries(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve alert timeseries: {str(e)}"
        )
...
async def get_system_performance(
    ...
    try:
        async with db.acquire() as conn:
            # Processing times
            processing_stats = await conn.fetchrow(f"""
                SELECT 
            ...
            # Throughput
            throughput_stats = await conn.fetchrow(f"""
                SELECT 
            ...
            # Error rates
            error_stats = await conn.fetchrow(f"""
                SELECT 
            ...
            return {
                "processing_performance": {
                    "avg_processing_time_ms": round(float(processing_stats['avg_processing_ms'] or 0), 2),
                    "min_processing_time_ms": round(float(processing_stats['min_processing_ms'] or 0), 2),
                    "max_processing_time_ms": round(float(processing_stats['max_processing_ms'] or 0), 2),
                    "p95_processing_time_ms": round(float(processing_stats['p95_processing_ms'] or 0), 2)
                },
                "throughput": {
                    "avg_transactions_per_hour": round(float(throughput_stats['avg_transactions_per_hour'] or 0), 2),
                    "total_processed": throughput_stats['total_processed'] or 0
                },
                "reliability": {
                    "error_rate_percent": round(error_rate, 2),
                    "failed_count": error_stats['failed_count'] or 0,
                    "success_count": (error_stats['total_count'] or 0) - (error_stats['failed_count'] or 0)
                },
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve system performance: {str(e)}"
        )
...
async def generate_summary_report(
    ...
    try:
        ...
        async with db.acquire() as conn:
            ...
            # Transaction metrics
            tx_stats = await conn.fetchrow(f"""
                SELECT 
            ...
            # Alert metrics
            alert_stats = await conn.fetchrow(f"""
                SELECT 
            ...
            # MEV metrics
            mev_stats = await conn.fetchrow(f"""
                SELECT 
            ...
            if include_charts:
                ...
                tx_chart_data = await conn.fetch(f"""
                    SELECT 
                ...
                severity_data = await conn.fetch(f"""
...
async def execute_custom_query(
    ...
    try:
        ...
        if current_user.get('role') != 'admin':
            raise HTTPException(
                status_code=403,
                detail="Admin access required for custom queries"
            )
