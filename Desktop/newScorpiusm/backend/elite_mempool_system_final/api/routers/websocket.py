        ...
        for client_id, websocket in self.active_connections.items():
            try:
                ...
                if "all" in subscribed_events or event_type in subscribed_events:
                    ...
                    if self._message_matches_filters(message, subscription.get("filters", {})):
...
async def websocket_endpoint(
    ...
    try:
        ...
        user_info = await verify_websocket_token(token) if token else {"user_id": "anonymous"}
...
async def handle_client_message(client_id: str, message: dict):
    ...
    if message_type == "subscribe":
        ...
        await connection_manager.update_client_subscription(client_id, subscription_data)
        ...
        if client_id in connection_manager.connection_subscriptions:
            current_events = connection_manager.connection_subscriptions[client_id].get("subscribed_events", [])
            updated_events = [e for e in current_events if e not in events_to_remove]
            await connection_manager.update_client_subscription(client_id, {"subscribed_events": updated_events})
