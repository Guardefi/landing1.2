...
class ConfigManager:
    ...
    def _load_config(self):
        try:
            ...
            print(
                f"Warning: config file '{self.config_path}' not found. Using defaults and environment variables."
            )
    ...
    def _get_default_config(self):
        ...
        return {
            ...
            "SIMULATION_RPC": os.getenv(
                "SIMULATION_RPC", "http://localhost:8545"
            ),  # RPC for Anvil/REVM simulator
            ...
            if env_value is not None:
                ...
                try:
                    if isinstance(default_value, bool):
                        ...
                        print(
                            f"Warning: Environment variable override for dict key '{key}' not automatically handled."
                        )
                    ...
                    print(
                        f"Warning: Could not convert environment variable {env_var}='{env_value}' for key '{key}'. Using default."
                    )
