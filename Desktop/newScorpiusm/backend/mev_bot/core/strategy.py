...
class AbstractStrategy(ABC):
    ...
    def __init__(
        self, strategy_type: StrategyType, config: StrategyConfig, explain: bool = False
    ):
        """
        Initialize strategy
    ...
    async def start(self) -> None:
        ...
        if self.explain:
            self.logger.info(
                "Teaching mode enabled - detailed explanations will be logged"
            )
        ...
        while self._running:
            try:
                ...
                opportunity = await asyncio.wait_for(
                    self._opportunities.get(), timeout=1.0
                )
...
class StrategyRegistry:
    ...
    def register_strategy(
        self, name: str, strategy: AbstractStrategy, file_path: str | None = None
    ) -> None:
    ...
    def get_strategy(self, name: str) -> AbstractStrategy | None:
