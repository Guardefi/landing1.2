...
class BackPressureManager:
    ...
    def record_input(self) -> None:
        ...
        self._input_timestamps = [
            ts for ts in self._input_timestamps if ts > cutoff_time
        ]

        # Update smoothed input rate
        instant_rate = (
            len(self._input_timestamps) / 5.0
        )  # Items per second over 5s window
    ...
    def record_processed(self) -> None:
        ...
        self._processed_timestamps = [
            ts for ts in self._processed_timestamps if ts > cutoff_time
        ]
    ...
    def should_throttle(self) -> bool:
        ...
        """
        if self.metrics.current_state in [
    ...
    def add_state_change_callback(
        self, callback: Callable[[BackPressureState], None]
    ) -> None:
        """
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            self.metrics.current_state != BackPressureState.CRITICAL
            and self.metrics.queue_utilization < 0.9
            and self.metrics.backlog_ratio < 3.0
        )
