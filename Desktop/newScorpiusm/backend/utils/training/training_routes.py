...
async def get_training_modules(
    ...
    try:
        query = (
            ...
            .options(
                selectinload(TrainingModule.courses), selectinload(TrainingModule.badge)
            )
            .filter(TrainingModule.is_active)
...
async def get_module_courses(module_id: str, db: AsyncSession = Depends(get_db)):
    """Get all courses for a specific module"""
    try:
        result = await db.execute(
            select(Course)
            .filter(and_(Course.module_id == module_id, Course.is_active))
...
async def get_user_progress(user_id: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        result = await db.execute(
            select(UserProgress, TrainingModule)
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch user progress: {str(e)}"
        )
...
async def update_user_progress(
    ...
    try:
        ...
        result = await db.execute(
            select(UserProgress).filter(
                and_(
                    UserProgress.user_id == user_id, UserProgress.module_id == module_id
                )
            )
        )
...
async def get_all_badges(
    badge_type: BadgeType | None = Query(None), db: AsyncSession = Depends(get_db)
):
    """Get all available badges"""
    try:
        query = select(Badge).filter(Badge.is_active)
...
async def get_user_badges(user_id: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch user badges: {str(e)}"
        )
...
async def submit_assessment_attempt(
    ...
    try:
        ...
        if not assessment:
            raise HTTPException(status_code=404, detail="Assessment not found") from e
        ...
        if attempt_count >= assessment.max_attempts:
            raise HTTPException(
                status_code=400, detail="Maximum attempts exceeded"
            ) from e
        ...
        await db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Failed to submit attempt: {str(e)}"
        )
...
async def start_simulation(
    user_id: str, simulation_data: SimulationStart, db: AsyncSession = Depends(get_db)
):
        ...
        if category == "overall":
            ...
            result = await db.execute(
                select(
                    ...
                    .filter(UserProgress.status == ProgressStatus.COMPLETED)
            ...
            result = await db.execute(
                select(
                    TrainingUser.username,
                    TrainingUser.full_name,
                    func.coalesce(func.sum(UserBadge.points_earned), 0).label(
                        "weekly_points"
                    ),
                )
                .outerjoin(
        ...
        return [
            LeaderboardEntry(
                ...
                total_points=getattr(
                    ranking, "total_points", getattr(ranking, "weekly_points", 0)
                ),
...
async def get_user_analytics(user_id: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        user_result = await db.execute(
            select(TrainingUser).filter(TrainingUser.id == user_id)
        )
        ...
        return {
            ...
            "achievement_stats": {
                ...
                "completion_rate": round(
                    (
                        (progress_stats.modules_completed or 0)
                        / max(progress_stats.modules_started or 1, 1)
                    )
                    * 100,
                    1,
                ),
...
async def check_and_award_badges(user_id: str, module_id: str, db: AsyncSession):
    ...
    try:
        ...
        result = await db.execute(
            select(TrainingModule)
            .options(selectinload(TrainingModule.badge))
        ...
        if module and module.badge:
            ...
            existing_badge = await db.execute(
                select(UserBadge).filter(
                    and_(
                        ...
                        UserBadge.badge_id == module.badge.id,
                    )
                )
            )
            ...
            if not existing_badge.scalar_one_or_none():
                ...
                # Update user points
                await award_points(
                    user_id, module.badge.points_awarded, "badge_earned", db
                )
