...
async def get_mev_opportunities():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch MEV opportunities: {str(e)}"
        )
...
async def get_mev_strategies():
    ...
    try:
            ...
            strategy_status[strategy_type.value] = {
                ...
                "success_rate": (
                    ...
                    / max(
                        1,
                        strategy.stats.successful_executions
                        + strategy.stats.failed_executions,
                    )
                )
                * 100,
            }
        ...
        return JSONResponse(
            {
                ...
                "success_rate": strategy_status.get("flashloan_arbitrage", {}).get(
                    "success_rate", 0.0
                ),
            }
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch strategy status: {str(e)}"
        )
...
async def get_mev_performance():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch MEV performance: {str(e)}"
        )
...
async def pause_strategy(strategy_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to pause strategy: {str(e)}"
        )
...
async def stop_strategy(strategy_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to stop strategy: {str(e)}"
        )
...
async def deploy_strategy(request_data: dict):
    ...
    try:
        ...
        if strategy_enum in mev_engine.strategies:
            ...
            raise HTTPException(
                status_code=400, detail=f"Strategy {strategy_type} not available"
            ) from e

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to deploy strategy: {str(e)}"
        )
...
async def execute_mev_opportunity(opportunity_id: str):
    ...
    try:
        ...
        if result is None:
            raise HTTPException(
                status_code=404, detail="Opportunity not found or already executed"
            ) from e
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to execute opportunity: {str(e)}"
        )
...
async def start_mev_engine():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to start MEV engine: {str(e)}"
        )
...
async def stop_mev_engine():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to stop MEV engine: {str(e)}"
        )
...
from mev_strategies import StrategyType
                ...
                "active_strategies": sum(
                    1 for strategy in mev_engine.strategies.values() if strategy.enabled
                ),
