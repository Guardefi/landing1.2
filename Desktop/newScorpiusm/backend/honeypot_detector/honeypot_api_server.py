...
async def get_scan_status(scan_id: str):
    ...
    try:
        ...
        completed_scan = next(
            (scan for scan in scan_history if scan["scan_id"] == scan_id), None
        )
        if completed_scan:
...
async def run_honeypot_scan(scan_id: str, request: HoneypotScanRequest):
    ...
    try:
        ...
        results = await detector.scan_target_async(
            request.target, ports=request.ports, timeout=request.timeout
        )
        ...
        # Analyze results
        honeypot_detected = any(
            detection.confidence > 0.5 for detection in results.detections
        )
        overall_confidence = max(
            (d.confidence for d in results.detections), default=0.0
        )
        honeypot_types = [
            d.honeypot_type for d in results.detections if d.confidence > 0.3
        ]
        ...
        recommendations = generate_recommendations(
            results, honeypot_detected, risk_score
        )
...
def generate_recommendations(
    results, honeypot_detected: bool, risk_score: float
) -> list[str]:
