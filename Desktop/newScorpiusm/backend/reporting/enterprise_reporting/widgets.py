...
class CallGraphWidget(BaseWidget):
    ...
    def render(self, data: dict[str, Any], **kwargs) -> str:
        ...
        for source, target in G.edges():
            vis_edges.append({"from": source, "to": target, "arrows": "to"})
...
class OpcodeHistogramWidget(BaseWidget):
    ...
    def render(self, data: dict[str, Any], **kwargs) -> str:
        ...
        fig = go.Figure(data=[go.Bar(x=opcodes, y=counts, marker_color="#3b82f6")])
        ...
        plot_html = fig.to_html(
            include_plotlyjs="cdn", div_id=f"histogram_{self.widget_id}"
        )
