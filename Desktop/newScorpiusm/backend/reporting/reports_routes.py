...
# Mock database functions and models for compatibility
class ScorpiusScan:
    """Mock ScorpiusScan model"""
...
async def get_db() -> AsyncSession:
    """Mock database session getter"""
    # This should be replaced with actual database session
    return AsyncSession()


# Mock database instance for backward compatibility
db = AsyncSession()
...
class ReportAnalytics(BaseModel):
    ...
    avg_vulnerabilities: float = Field(
        ..., description="Average vulnerabilities per scan"
    )
    ...
    most_common_vuln: str | None = Field(
        None, description="Most common vulnerability type"
    )
...
class ReportGenerationRequest(BaseModel):
    ...
    include_ai_analysis: bool = Field(
        default=True, description="Include AI-powered insights"
    )
...
async def list_reports(
    ...
    try:
        ...
        query = select(ScorpiusScan).filter(
            ScorpiusScan.status == "completed", ScorpiusScan.report_paths.isnot(None)
        )
        ...
        query = (
            query.order_by(desc(ScorpiusScan.created_at)).offset(offset).limit(per_page)
        )
        ...
        for scan in scans:
            try:
                # Parse report paths
                report_paths = (
                    json.loads(scan.report_paths) if scan.report_paths else {}
                )
...
async def get_report_analytics(db: AsyncSession = Depends(get_db)) -> ReportAnalytics:
    ...
    try:
        ...
        avg_vulnerabilities = (
            total_vulnerabilities / total_reports if total_reports > 0 else 0
        )
        ...
        unique_contracts = len({s.contract_address for s in scans})

        # Most common vulnerability type (simplified)
        most_common_vuln = (
            "Access Control Vulnerability"  # Could be enhanced with actual analysis
        )
...
async def download_report(
    scan_id: str, format: str = "pdf", db: AsyncSession = Depends(get_db)
) -> FileResponse:
    """
    ...
    try:
        ...
        if not scan.report_paths:
            raise HTTPException(
                status_code=404, detail=f"No reports available for scan {scan_id}"
            ) from e
        ...
        if not os.path.exists(file_path):
            raise HTTPException(
                status_code=404, detail=f"Report file not found: {file_path}"
            ) from e
    ...
    try:
        ...
        return {
            ...
            "report_metadata": {
                ...
                "size_estimate": sum(
                    info.get("size", 0) for info in file_info.values()
                ),
            },
        }
